...
class mm_tedor extends cl_pbase implements MouseListener
{
	...
	private Hashtable hstTBLDT;    /** Hashtable to save the vendor table details in a vector against Indent /Item code */
	private Hashtable hstDODTL;    /** Hashtable to save the vendor/ARC wise Details for D.O to be generated.           */
	private Vector vtrDORCT = new Vector();
	private Vector vtrDORQT = new Vector();
		...
		if(M_objSOURC == cl_dat.M_cmbOPTN_pbst)
		{
			try
			{
				...
				if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPADD_pbst))
				{
					...
					+"from MM_INMST,MM_POMST,CO_CTMST WHERE IN_STRTP = PO_STRTP AND IN_MATCD = PO_MATCD AND IN_MATCD = CT_MATCD "
					+"AND IN_INDTP ='01' AND IFNULL(IN_AUTQT,0)-IFNULL(IN_ORDQT,0)-IFNULL(IN_FCCQT,0) >0 AND IN_STSFL ='4'"
					+" AND PO_PORTP ='03' and IFNULL(PO_PORQT,0)-IFNULL(PO_ORDQT,0)-IFNULL(PO_FRCQT,0) >0 AND PO_STSFL NOT IN('X','O')";
				...
				for(int i=0;i<tblITDTL.getRowCount();i++)
				{
					if(tblITDTL.getValueAt(i,TBL_CHKFL).toString().equals("true"))
				    {
						...
						M_strSQLQRY ="Select distinct PO_PORNO,PO_VENCD,PT_PRTNM,PO_PORQT,ifnull(PO_PORQT,0)-ifnull(PO_ORDQT,0)-ifnull(PO_FRCQT,0) L_PENQT"
									+" FROM MM_POMST,CO_PTMST WHERE PO_VENCD = PT_PRTCD and PT_PRTTP ='S' and PO_STRTP ='"+M_strSBSCD.substring(2,4) +"'"
		 ...
		 String L_strDORNO = strPORNO;
		  ...
		  while(M_rstRSSET.next())
		  {
			 ...
			 // Generate single P.O. for each Entry in Vector
			  ...
			  M_strSQLQRY = "INSERT INTO MM_POMST(PO_MMSBS,PO_STRTP,PO_PORTP,PO_PORNO,PO_INDNO,PO_MATCD,"
						...
						+"PO_LUSBY,PO_LUPDT)"
						+" SELECT '"+M_strSBSCD +"','"+M_strSBSCD.substring(2,4) +"','01','"
						+ strPORNO+"',"	
						...
						+" FROM MM_POMST WHERE PO_STRTP ='"+M_strSBSCD.substring(2,4)+"'"
			...
			{
			  M_strSQLQRY ="INSERT INTO CO_TXDOC (TX_SYSCD,TX_SBSTP,TX_DOCTP,TX_SBSCD,TX_DOCNO,TX_TRNTP,TX_PRDCD,TX_EDCFL,TX_EDCVL,"
				  ...
				  + " select  TX_SYSCD,TX_SBSTP,TX_DOCTP,TX_SBSCD,"
				  +"'"+strPORNO +"',"			 
				  ...
				  + "'"+cl_dat.M_strUSRCD_pbst +"',"
			  	  +" current_date FROM CO_TXDOC WHERE TX_SBSTP ='"+M_strSBSCD.substring(2,4)+"'"
		  ...
		  if(cl_dat.M_flgLCUPD_pbst)
		  {
			   M_strSQLQRY ="UPDATE CO_CDTRN SET CMT_CCSVL = '"+strPORNO.substring(3) +"',CMT_CHP01 =''";
				M_strSQLQRY += " where CMT_CGMTP = 'DOC' and CMT_CGSTP = 'MM"+M_strSBSCD.substring(2,4)+"POR'";
	...
	private boolean genPORNO()
	{
		try
		{
			...
			M_strSQLQRY += " where CMT_CGMTP = 'DOC' and CMT_CGSTP = 'MM"+M_strSBSCD.substring(2,4)+"POR' and ";
