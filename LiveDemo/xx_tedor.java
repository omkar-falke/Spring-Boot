...
{	/**Thread to prepare object of mr_rpdor for DO printing  */
	...
	private String strYREND = "31/03/2008";
	...
	private Vector vtrMKTTP;
	private Vector vtrMOTCD;
	private Vector vtrDTPCD;
	
	private Hashtable hstCDTRN;
	private Hashtable hstCODDS;
	private Hashtable hstPRMST;			// Product Master details
	private Hashtable hstINTRN;
	private Hashtable hstINTRN_1;
	private Hashtable hstGRTRN;
	private Hashtable hstDOTRN;
	private Hashtable hstDOTRN_1;
	///private Hashtable hstDODEL;
	///private Hashtable hstINDEL;
	private Hashtable hstINSRL;

	private Hashtable hstPRDDT;
	...
	mr_tedor()
	{
		...
		try
		{
			...
			hstINTRN = new Hashtable();
			hstINTRN_1 = new Hashtable();
			hstGRTRN = new Hashtable();
			hstDOTRN = new Hashtable();
			hstDOTRN_1 = new Hashtable();
			///hstDODEL = new Hashtable();
			///hstINDEL = new Hashtable();
			hstINSRL = new Hashtable();
			chkAUTOCF = new JCheckBox("Auto C/F");

			hstCDTRN.clear();
            hstCODDS.clear();
			crtCDTRN("'SYSCOXXDST','STSMRXXDOR','MSTCOXXMKT','DOCMR01DOR','DOCMR02DOR','DOCMR03DOR','DOCMR04DOR','DOCMR05DOR','DOCMR12DOR','SYSMRXXDTP', 'SYSMR01MOT','SYSFGXXPKG'","",hstCDTRN);
			crtPRMST();
			...
			add(cmbMKTTP=new JComboBox(),1,2,1,1.75,this,'L');
			add(new JLabel("Order No."),1,4,1,1,this,'L');
			add(txtINDNO=new TxtLimit(9),1,5,1,1,this,'L');
			add(new JLabel("D.O. No."),1,6,1,1,this,'L');
			add(txtDORNO=new TxtLimit(8),1,7,1,1,this,'L');
			//add(txtDLCDS=new TxtLimit(45),2,8,1,3,this,'L');
			add(rdbAUTH_Y=new JRadioButton("Authorise"),1,8,1,1.5,this,'L');
			add(rdbAUTH_N=new JRadioButton("DoNot Authorise"),1,10,1,1.5,this,'R');
	...
	public void itemStateChanged(ItemEvent L_IE)
	{
		...
		try
		{
			...
			{
				inlCDTRN();
				crtCDTRN("'MSTCOXXMKT'"," and CMT_CHP01 like '%"+M_strSBSCD.substring(0,2)+"%'  and CMT_CHP02 like '%"+M_strSBSCD.substring(2,4)+"%'",hstCDTRN);
	...
	private void exeRLSE_DO()
	{
		...
		{
			strMKTTP = getCODCD("MSTCOXXMKT"+cmbMKTTP.getSelectedItem().toString());
			cl_dat.exeSQLUPD("update co_cdtrn set cmt_chp01 = 'Y' where cmt_cgmtp='DOC' and cmt_cgstp = 'MR"+strMKTTP+"DOR' and cmt_chp01='N'","");
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		try
		{
			...
			if(M_objSOURC==txtDORNO)
			{
				...
				{
					String L_strSQLQRY = "Select distinct DOT_DORNO,DOT_DORDT,DOT_AMDNO,DOT_AMDDT,DOT_INDNO from MR_DOTRN where DOT_MKTTP='"+strMKTTP+"' and DOT_STSFL<>'X' and DOT_LADQT<DOT_DORQT and DOT_INDNO in (select IN_INDNO from MR_INMST where substr(IN_SBSCD,3,2) = '"+M_strSBSCD.substring(2,4)+"') order by DOT_DORDT desc,DOT_DORNO desc";
			...
			else if(M_objSOURC==txtINDNO)
			{
				...
				if(L_KE.getKeyCode()==L_KE.VK_F1 && cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPADD_pbst))
					...
					cl_hlp("Select IN_INDNO,IN_BKGDT,IN_AUTTM,sum((ifnull(int_indqt,0)-ifnull(int_fcmqt,0))-INT_DORQT) from MR_INMST,MR_INTRN where IN_INDNO=INT_INDNO and IN_STSFL='1' and IN_MKTTP='"+strMKTTP+"' and IN_SALTP = '"+M_strSBSCD.substring(2,4)+"' group by IN_INDNO,IN_BKGDT,IN_AUTTM having sum(int_INDQT-INT_DORQT)>0 order by IN_BKGDT desc, IN_INDNO desc " ,1,1,new String[] {"Indent No","Indent Date","Time"},3,"CT");
			...
			else if(M_objSOURC==txtDORNO && cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPMOD_pbst) && cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPAUT_pbst))
			{
				...
				//	cl_hlp("Select DOT_DORNO,DOT_DORDT from MR_DOTRN where  DOT_STSFL not in ('2','D','X') and DOT_MKTTP='"+strMKTTP+"' and substr(DOT_SBSCD,3,2) = '"+M_strSBSCD.substring(2,4)+"' order by DOT_DORNO " ,1,1,new String[] {"DO No","DO Date"},2,"CT");
				M_strHLPFLD = "txtDORNO";
				if(L_KE.getKeyCode()==L_KE.VK_F1)
				{
					String L_strSQLQRY = "Select distinct DOT_DORNO,DOT_DORDT,DOT_AMDNO,DOT_AMDDT,DOT_INDNO from MR_DOTRN where DOT_MKTTP='"+strMKTTP+"' and DOT_STSFL<>'X'  and substr(DOT_SBSCD,3,2) = '"+M_strSBSCD.substring(2,4)+"' and DOT_LADQT<DOT_DORQT order by DOT_DORDT desc,DOT_DORNO desc";
					//System.out.println(L_strSQLQRY);
					cl_hlp(L_strSQLQRY ,1,1,new String[] {"DO.No","DO.Date","Amd.No.","Amd.Date","Ind.No."},5,"CT");//	
				}
			}
	...
	void exeSAVE()
	{
		try
		{
			...
			for(int LP_ROWNO=0;LP_ROWNO<tblDOTRN.getRowCount();LP_ROWNO++)
			{
				...
				if((fltTBL_DORQT+fltINT_DORQT-fltDOT_DORQT) > fltINT_INDQT)
					strVLDMSG = "Against Ind:"+txtINDNO.getText()+"  Grade:"+tblDOTRN.getValueAt(LP_ROWNO,intTB1_PRDDS).toString()+"     Available  = "+setNumberFormat(fltINT_INDQT - fltINT_DORQT+fltDOT_DORQT,3)+"  Entered = "+setNumberFormat(fltTBL_DORQT,3);
			...
			if(!vldDATA())
				...
				if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPMOD_pbst) || cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPAUT_pbst) || cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPADD_pbst))
				{
            		...
            		{
                			String L_strSQLQRY  ="Insert into MR_DOTAM Select * from MR_DOTRN where DOT_MKTTP = '"+strMKTTP +"' AND DOT_DORNO = '"+txtDORNO.getText()+"'";
                			cl_dat.exeSQLUPD(L_strSQLQRY,"" );
                			L_strSQLQRY  ="Insert into MR_DODAM Select * from MR_DODEL where DOD_MKTTP = '"+strMKTTP +"' AND DOD_DORNO = '"+txtDORNO.getText()+"'";
                			cl_dat.exeSQLUPD(L_strSQLQRY,"");
                   	 }
				...
				else if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPDEL_pbst))
				{
					...
					cl_dat.exeSQLUPD("Update MR_DOTRN set DOT_STSFL='X',DOT_LUPDT='"+M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_txtCLKDT_pbst.getText()))+"' ,DOT_DORQT=0 where DOT_MKTTP='"
						+strMKTTP+"' and DOT_SBSCD='"+M_strSBSCD+"' and DOT_INDNO='"
						+txtINDNO.getText()+"' and DOT_DORNO='"+txtDORNO.getText()+"'","setLCLUPD");
					
					if(cl_dat.M_flgLCUPD_pbst)
					cl_dat.exeSQLUPD("Update MR_DODEL set DOD_STSFL='X',DOD_LUPDT='"+M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_txtCLKDT_pbst.getText()))+"' ,DOD_DORQT=0 where DOD_MKTTP='"
...
private boolean chkDORNO()
{
	try
	{
		if(!cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPADD_pbst))
			return true;
		M_rstRSSET=cl_dat.exeSQLQRY("Select * from co_CDTRN where CMT_CGMTP='DOC' and CMT_CGSTP='MR"+strMKTTP+"DOR' and cmt_CODCD='"+strYRDGT+strMKTTP+"'");
		...
		cl_dat.exeSQLUPD("Update co_cdtrn set cmt_CHP01='N' where CMT_CGMTP='DOC' and CMT_CGSTP='MR"+strMKTTP+"DOR' and cmt_CODCD='"+strYRDGT+strMKTTP+"'","setLCLUPD");
...
private boolean setDOMST(String LP_MKTTP, String LP_DORNO)
{
	...
	try
	{
		...
		L_rstRSSET=cl_dat.exeSQLQRY("Select * from MR_DOTRN where DOT_MKTTP = '"+LP_MKTTP+"' and DOT_DORNO='"+LP_DORNO+"'");
...
private void saveDOTRN(int LP_ROWNO)
{
	try
	{
		...
		strWHRSTR =  "DOT_MKTTP = '" +strMKTTP+"' and "
		...
		if(!flgCHK_EXIST)
		{
			M_strSQLQRY= "insert into MR_DOTRN (DOT_MKTTP ,DOT_DORNO ,DOT_PRDCD ,DOT_PKGTP ,DOT_DORDT ,DOT_INDNO ,DOT_AMDNO,DOT_AMDDT,DOT_TMOCD ,DOT_DLCCD ,DOT_LRYNO ,DOT_FRTRT ,DOT_PRDDS ,DOT_DORPK ,DOT_PKGWT ,DOT_DORQT ,DOT_ORDUM ,DOT_LADQT ,DOT_INVQT ,DOT_FCMQT ,DOT_DELTP ,DOT_STSFL ,DOT_LUSBY ,DOT_LUPDT ,DOT_TRPCD ,DOT_AUTBY,DOT_AUTDT,DOT_AUTTM,DOT_SBSCD,DOT_TRNFL) values (" 
			...
			+setINSSTR("DOT_DORPK",tblDOTRN.getValueAt(LP_ROWNO,intTB1_DORPK).toString(),"N")
			...
			+setINSSTR("DOT_LUPDT",cl_dat.M_txtCLKDT_pbst.getText(),"D")
		...
		else if(flgCHK_EXIST)
		{
			...
			+setUPDSTR("DOT_LUPDT",cl_dat.M_txtCLKDT_pbst.getText(),"D")
		...
		{
			strWHRSTR =   "DOD_MKTTP = '" +strMKTTP+"' and "
		...
		if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPADD_pbst) && flgFIRSTREC)
		{
			...
			M_strSQLQRY="update co_cdtrn set cmt_ccsvl='"+txtDORNO.getText().substring(3)+"',CMT_CHP01='Y' where cmt_cgmtp='DOC' and cmt_cgstp='MR"+strMKTTP+"DOR'";
			cl_dat.exeSQLUPD(M_strSQLQRY ,"setLCLUPD");
		}
		flgFIRSTREC=false;
	}
	    ...
	    if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPADD_pbst))
	    {
    		for(int i=0;i<tblDODEL.getRowCount();i++)
    		{
    		    ...
    		    if((tblDODEL.getValueAt(i,intTB2_CHKFL).toString().equals("true"))&&(tblDODEL.getValueAt(i,intTB2_PRDCD).toString().equals(LP_PRDCD))&&(tblDODEL.getValueAt(i,intTB2_PKGTP).toString().equals(LP_PKGTP)))
    		    {
        		    ...
        		    M_strSQLQRY="Insert into MR_DODEL (DOD_MKTTP,DOD_DORNO,DOD_PRDCD,DOD_SRLNO,DOD_DSPDT,DOD_DORQT,DOD_DELTP,DOD_PKGTP,DOD_SBSCD,DOD_STSFL,DOD_LUSBY,DOD_LUPDT,DOD_TRNFL) values ("
        			...
        			+setINSSTR("DOD_LUPDT",cl_dat.M_txtCLKDT_pbst.getText(),"D")
        			+"'0')";
        			//System.out.println(M_strSQLQRY);
        			cl_dat.exeSQLUPD(M_strSQLQRY ,"setLCLUPD");
    		    }
    	       ...
    	       //if((tblDODEL.getValueAt(i,intTB2_CHKFL).toString().equals("true"))&&(tblDODEL.getValueAt(i,intTB2_PRDCD).toString().equals(LP_PRDCD))&&(tblDODEL.getValueAt(i,intTB2_PKGTP).toString().equals(LP_PKGTP)))
    		    if((tblDODEL.getValueAt(i,intTB2_CHKFL).toString().equals("true"))&&(tblDODEL.getValueAt(i,intTB2_PRDCD).toString().equals(LP_PRDCD)))
    		    {
    		        ...
    		        else if(flgCHK_EXIST)
            		{
            			...
            			+setUPDSTR("DOD_LUPDT",cl_dat.M_txtCLKDT_pbst.getText(),"D")
...
private void saveRMMST(String LP_DOCTP, String LP_REMDS)
{
	try
	{
		...
		strWHRSTR = " RM_MKTTP = '"+strMKTTP+"' and "
		...
		if(!flgCHK_EXIST)
		{
			...
			M_strSQLQRY="Insert into MR_RMMST (RM_MKTTP,RM_TRNTP,RM_DOCNO,RM_REMDS,RM_SBSCD,RM_TRNFL,RM_LUSBY,RM_LUPDT,RM_STSFL) values ("
...
private void setINT_DORQT()
{
	try
	{
		cl_dat.exeSQLUPD("update MR_INTRN set INT_DORQT=0,INT_LUPDT= '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_strLOGDT_pbst))+"' where INT_MKTTP = '"+strMKTTP+"' and INT_INDNO = '"+txtINDNO.getText()+"'","");
		String L_strDORQT ="";
		strWHRSTR = " DOT_MKTTP = '"		+strMKTTP			+"' and "
		...
		while(true)
		{
				...
				strWHRSTR = " INT_MKTTP = '"		+strMKTTP			+"' and "
		...
		public boolean verify(JComponent input) 
		{
			try
			{
				...
				else if(input == txtTRPCD)
				{
					...
					ResultSet L_rstRSSET=cl_dat.exeSQLQRY2("Select * from co_ptmst where pt_prttp='T' and pt_prtcd='"+txtTRPCD.getText()+"'");
					if(L_rstRSSET!=null)
					{
						if(L_rstRSSET.next())
						{
		...
		try
		{
			...
			for(int i=0;i<tblDOTRN.getRowCount()&&tblDOTRN.getValueAt(i,intTB1_CHKFL).toString().equals("true");i++)
			{
               ...
               if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPADD_pbst) || cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPMOD_pbst))
                if(getTOT_DODQT(tblDOTRN.getValueAt(i,intTB1_PRDCD).toString(),tblDOTRN.getValueAt(i,intTB1_PKGTP).toString()) != Double.parseDouble(tblDOTRN.getValueAt(i,intTB1_DORQT).toString()))			    	
                {
                    ...
                    setMSG("Quantity mismatch in Delivery Schedule.., Check for "+tblDOTRN.getValueAt(i,intTB1_PRDDS).toString(),'E'); 
                    return false;
                }
    		}
		String L_strTEMP ="";
...
private boolean vldDOT_DORQT(int LP_ROWNO)
{
	...
	try
	{
		...
		if(tblDOTRN.isEditing())
			...
			String strVLDMSG = "";
			String strVLDMSG1 = "";
			if(((fltTBL_DORQT-fltDOT_DORQT)+fltINT_DORQT) > fltINT_INDQT)
				strVLDMSG = "Against Ind:"+txtINDNO.getText()+"  Grade:"+tblDOTRN.getValueAt(LP_ROWNO,intTB1_PRDDS).toString()+"     Available  = "+setNumberFormat(fltINT_INDQT - (fltINT_DORQT-fltDOT_DORQT),3)+"  Entered = "+setNumberFormat(fltTBL_DORQT,3);
...
*/
	...
	private void dspADDR(Component P_cmpSOURC,String P_strPRTCD,String P_strPRTTP)
	{
		try
		{
			ResultSet L_rstRSSET=cl_dat.exeSQLQRY3("Select PT_PRTNM,PT_ADR01,PT_ADR02,PT_ADR03 from CO_PTMST where PT_PRTTP='"+P_strPRTTP+"' and PT_PRTCD='"+P_strPRTCD+"'");
			if(L_rstRSSET!=null)
				if(L_rstRSSET.next())
				{
...
private boolean setPRTNM_1(String LP_PRTTP, JTextField LP_CDFLD, JTextField LP_NMFLD, String LP_MSGDS, String LP_PTADR)
{
	try
	{
		ResultSet T_rstRSSET=cl_dat.exeSQLQRY1("Select PT_PRTCD, PT_PRTNM, PT_ADR01, PT_ADR02, PT_ADR03 from co_ptmst where pt_prttp = '"+LP_PRTTP+"' and pt_prtcd = '"+LP_CDFLD.getText()+"'");
...
private void setRMMST()
{
	...
	try
	{
		...
		if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPADD_pbst)) // During addition, Fetch Indent Remark
			L_rstRSSET=cl_dat.exeSQLQRY3("Select * from MR_RMMST where RM_MKTTP = '"+strMKTTP+"' and RM_DOCNO='"+txtINDNO.getText()+"'");
		else   // Modification / Deltion / Enquiry ...  Fetch from D.O. Remark
			L_rstRSSET=cl_dat.exeSQLQRY1("Select * from mr_rmmst where rm_mkttp = '"+strMKTTP+"' and rm_trntp='DO' and rm_docno='"+txtDORNO.getText()+"'");
		if(L_rstRSSET!=null && L_rstRSSET.next())
...
private void setCAPTIVE_1()
{
	...
	private boolean setINMST(String LP_MKTTP, String LP_INDNO, String LP_ADDSTR, String LP_CLRMSG)
	{
		...
		try
		{
			L_rstRSSET=cl_dat.exeSQLQRY2("Select * from MR_INMST where IN_MKTTP='"+LP_MKTTP+"' and IN_INDNO = '"+LP_INDNO+"'"+LP_ADDSTR);
			if(L_rstRSSET==null || (!L_rstRSSET.next()))
			...
			if (!L_rstRSSET.getString("IN_SALTP").equals(M_strSBSCD.substring(2,4)))
			{setMSG("Invalid Sale Type   IN_SALTP : "+L_rstRSSET.getString("IN_SALTP")+" M_strSBSCD.substring(2,4) : "+M_strSBSCD.substring(2,4),'E'); return false;}
			...
			if(txtTRPCD.getText().length()== 0)
			{
    	        L_rstRSSET = null;
        		String L_strDORNO="";
    			L_rstRSSET=cl_dat.exeSQLQRY2("Select max(DOT_DORNO) DOT_DORNO from MR_DOTRN where DOT_MKTTP = '"+strMKTTP+"' and DOT_INDNO in (select IN_INDNO from MR_INMST where IN_CNSCD = '"+txtCNSCD.getText()+"' and IN_MKTTP = '"+strMKTTP+"')");
    			if(L_rstRSSET!=null)
    			{
    			    if(L_rstRSSET.next())
    			...
    			{	
        			L_rstRSSET=cl_dat.exeSQLQRY2("Select DOT_TRPCD from MR_DOTRN where DOT_MKTTP = '"+strMKTTP+"' and DOT_DORNO = '"+L_strDORNO+"'");
        			if(L_rstRSSET!=null)
        			{   
        			    if(L_rstRSSET.next())
        			        txtTRPCD.setText(getRSTVAL(L_rstRSSET,"DOT_TRPCD","C"));
	 ...
	 private boolean crtINTRN(String LP_MKTTP, String LP_INDNO, String LP_ADDSTR)
	{
	    try
	    {
			hstINTRN.clear();
			hstINSRL.clear();
			String L_strSQLQRY = "select INT_SRLNO,INT_PRDCD,INT_PKGTP,INT_PRDDS,ifnull(INT_INDQT,0) INT_INDQT,ifnull(INT_DORQT,0) INT_DORQT,INT_BASRT,ifnull(INT_PKGWT,0) INT_PKGWT,ifnull(INT_INVQT,0) INT_INVQT,sum(ifnull(ST_STKQT,0)-ifnull(ST_ALOQT,0)) INT_STKQT from MR_INTRN left outer join FG_STMST on INT_PRDCD=ST_PRDCD AND INT_PKGTP=ST_PKGTP "
	                                                 + " where INT_MKTTP ='"+LP_MKTTP+"' and INT_INDNO = '"+LP_INDNO+"'"+LP_ADDSTR+" group by INT_SRLNO,INT_PRDCD,INT_PKGTP,INT_PRDDS,INT_INDQT,INT_DORQT,INT_BASRT,INT_PKGWT,INT_INVQT";
			//System.out.println(L_strSQLQRY);
	        ResultSet L_rstRSSET = cl_dat.exeSQLQRY2(L_strSQLQRY);
	        if(L_rstRSSET == null || !L_rstRSSET.next())
	        {
	        ...
	        while(true)
	        {
	                ...
	                staINTRN[intAE_INT_BALQT] = setNumberFormat(Double.parseDouble(getRSTVAL(L_rstRSSET,"INT_INDQT","C"))-Double.parseDouble(getRSTVAL(L_rstRSSET,"INT_DORQT","C")),3);
	 ...
	 private boolean crtGRTRN(String LP_MKTTP, String LP_INDNO, String LP_ADDSTR)
	{
	    ...
	    {
			hstGRTRN.clear();
			M_strSQLQRY = "select ST_PRDCD,sum(ifnull(ST_STKQT,0)-ifnull(ST_ALOQT,0)) GR_QLHQT from FG_STMST where ST_PRDTP||ST_LOTNO||ST_RCLNO in (select LT_PRDTP||LT_LOTNO||LT_RCLNO from PR_LTMST where LT_RESFL in ('Q','H') and LT_PRDCD in (select INT_PRDCD from MR_INTRN  where INT_MKTTP ='"+LP_MKTTP+"' and INT_INDNO = '"+LP_INDNO+"'"+LP_ADDSTR+")) group by ST_PRDCD" ;
			//System.out.println(M_strSQLQRY);
	 ...
	 private void setDODEL_NEW(String LP_MKTTP, String LP_DORNO, String LP_ADDSTR)
	{
	    try
	    {
            ...
            +" where DOT_MKTTP = DOD_MKTTP AND DOT_DORNO = DOD_DORNO AND DOT_PRDCD = DOD_PRDCD AND DOT_PKGTP = DOD_PKGTP AND dod_mkttp='"+LP_MKTTP+"' and DOD_DORNO = '"+LP_DORNO+"'"+LP_ADDSTR;
			//System.out.println(L_strSQLQRY);
           /// hstDODEL.clear();
	        ResultSet L_rstRSSET = cl_dat.exeSQLQRY2(L_strSQLQRY);
	        if(L_rstRSSET == null || !L_rstRSSET.next())
	 ...
	 private void crtINDEL(String LP_MKTTP, String LP_INDNO)
	{
	    try
	    {
			...
			///hstINDEL.clear();
	        ResultSet L_rstRSSET = cl_dat.exeSQLQRY2("select INT_SRLNO,INT_PRDCD,INT_PRDDS,IND_PRDCD,IND_PKGTP,IND_SRLNO,IND_DELTP,IND_INDQT,IND_DSPDT from MR_INTRN,MR_INDEL where IND_MKTTP = INT_MKTTP AND IND_INDNO = INT_INDNO AND IND_PRDCD = INT_PRDCD AND IND_PKGTP = INT_PKGTP AND ind_mkttp='"+LP_MKTTP+"' and IND_INDNO = '"+LP_INDNO+"' and ifnull(INT_STSFL,'')<>'X' and ifnull(IND_STSFL,'')<>'X' ORDER BY INT_SRLNO,IND_PRDCD,IND_SRLNO ");
	...
	*/
	 ...
	 */
	private void setCDLST(Vector LP_VTRNM, String LP_CODCT, String LP_FLDNM)
	{
		try
		{
			Enumeration enmCODKEYS=hstCDTRN.keys();
			...
			while(enmCODKEYS.hasMoreElements())
			{
				...
				if(L_strCODCD.substring(0,10).equals(LP_CODCT))
				{
					...
					//System.out.println(getCDTRN(L_strCODCD,LP_FLDNM,hstCDTRN));
				}
			}
...
private boolean chkPARTY(String LP_PRTTP, String LP_PRTCD, String LP_MSGDS, String LP_CMPFL)
{
    ...
    private void rlsINDNT()
    {
		...
		{
		strWHRSTR =  "DOT_MKTTP = '" +strMKTTP+"' and "
					 +"DOT_INDNO = '" +txtINDNO.getText()+"' and ifnull(DOT_DORQT,0) > 0";
		flgCHK_EXIST =  chkEXIST("MR_DOTRN", strWHRSTR);
		if(flgCHK_EXIST)
		{
		...
		else
		{
			...
			if(L_intOPTN==0)
			{
				M_strSQLQRY="update MR_INMST set IN_STSFL='0',IN_LUSBY ='"+cl_dat.M_strUSRCD_pbst+"',IN_LUPDT='"+M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_strLOGDT_pbst))+"' where IN_MKTTP='"+strMKTTP+"' and IN_INDNO='"+txtINDNO.getText()+"'";
				cl_dat.M_flgLCUPD_pbst=true;
				cl_dat.exeSQLUPD(M_strSQLQRY ,"setLCLUPD");
				M_strSQLQRY="update MR_INTRN set INT_INDQT=0, INT_STSFL='0',int_lusby ='"+cl_dat.M_strUSRCD_pbst+"',INT_LUPDT='"+M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_strLOGDT_pbst))+"' where INT_MKTTP='"+strMKTTP+"' and INT_INDNO='"+txtINDNO.getText()+"'";
				if(cl_dat.M_flgLCUPD_pbst)
					cl_dat.exeSQLUPD(M_strSQLQRY ,"setLCLUPD");
				if(cl_dat.exeDBCMT("exeSAVE"))
