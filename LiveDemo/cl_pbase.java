...
public class cl_pbase extends JPanel implements WindowListener,ActionListener,FocusListener,KeyListener,MouseListener,DocumentListener,SwingConstants,ItemListener, Runnable
{
	...
	protected final String	M_strCPI10 ="CPI10",
							...
							M_strFILNM = "File",
							M_strEMLNM = "E-Mail";
	String M_strPRGCD_fr,M_strSYSCD_fr;
	boolean M_flgBIGHLP=false;
	protected final int M_intAE_SBSCD = 0,
						M_intAE_SHRDS = 1,
						M_intAE_CHP01 = 2,
						M_intAE_CHP02 = 3,
						M_intAE_ADDFL = 4,
						...
						M_intAE_DELFL = 6,
						M_intAE_ENQFL = 7,
						M_intAE_AUTFL = 8;/** Filter of Subsystem Code for SQL Query.*/
	protected String M_strSBSLS;/** SybSystem code equivallent to current selection of SBS comboboxes at the top of the screen	 Ideally, should be equal to concatination of selected indices of subsystem combos.*/
	...
	protected String M_strHLPFLD;/**Flags to remember operations carried out by the user. To update SA_PPUTR	 */
	private String strFLADD="'F'",strFLMOD="'F'",strFLDEL="'F'",strFLENQ="'F'",strFLRPT="'F'";	/** Global variable for ResutSet*/
	...
	protected Vector<Object> M_vtrSCCOMP;	/** Global variable for ratio of currnet screen resolution to standard resolution i e  800 x 600	 */
	...
	public cl_pbase()//DEFAULT CONSTRUCTOR, NEVER CALLED
	{
		...
		this.setLayout(null);
		M_vtrSCCOMP=new Vector<Object>(20,10);
//		addLSTN();
	}
	...
	public cl_pbase(int P_intSBSCT)
	{
		try
		{
			...
			M_vtrSCCOMP=new Vector<Object>(20,10);//VECTOR FOR SCREEN COMPONENTS
		}catch(Exception e)
...
//				cl_dat.M_btnEXT_pbst.setToolTipText(cl_dat.M_MSGEXT);
	...
	public  void actionPerformed(ActionEvent L_AE)
	{
		...
		try
		{
			...
			//	((JTextField)input).setDisabledTextColor(Color.black);
			...
			if(M_objSOURC==cl_dat.M_cmbOPTN_pbst)
			{
				...
				System.out.println(L_STRTMP+" "+ cl_dat.M_OPENQ_pbst);
				...
				{
					System.out.println("L_STRTMP"+L_STRTMP);
			...
			{
				cl_dat.M_strCMPCD_pbst = cl_dat.M_strCMPCD_EXT_pbst;
			...
			{
				String L_strTEMP=cl_dat.M_btnSAVE_pbst.getText().trim();
	...
	public void itemStateChanged(ItemEvent L_IE)
	{
		...
		if(M_objSOURC==cl_dat.M_cmbSBSL1_pbst)//SBS LEVEL -1 SELECTION CHANGED
		{
			...
			{//POPULATING SBS LEVE - 2 COMBO
				...
				for (int j=cl_dat.M_cmbSBSL2_pbst.getItemCount()-1;j>0;j--)
						cl_dat.M_cmbSBSL2_pbst.removeItemAt(j);
			...
			else//IF LEVEL - 2 DOESN'T EXIST
			{
				for(int i=0;i<M_staUSRRT.length&&M_staUSRRT[i][0]!=null;i++)
				{
					for (int j=cl_dat.M_cmbOPTN_pbst.getItemCount()-1;j>0;j--)
						cl_dat.M_cmbOPTN_pbst.removeItemAt(j);
					//cl_dat.M_cmbOPTN_pbst.removeAllItems();
					if(M_staUSRRT[i][1].equals(cl_dat.M_cmbSBSL1_pbst.getSelectedItem().toString()))
					{
						M_strSBSCD=M_staUSRRT[i][0].substring(2);
						if(cl_dat.M_btnSAVE_pbst.getText().trim().equalsIgnoreCase("Run")
							||cl_dat.M_btnSAVE_pbst.getText().trim().equalsIgnoreCase("Send")
							||cl_dat.M_btnSAVE_pbst.getText().trim().equalsIgnoreCase("Display")
							||cl_dat.M_btnSAVE_pbst.getText().trim().equalsIgnoreCase("Print"))
						{
		...
		else if(M_objSOURC==cl_dat.M_cmbSBSL2_pbst)//SBS LEVEL - 2 SELECTION CHANGED
		{
			...
			{//POPULATING LEVEL - 3 COMBO
				...
				for (int j=cl_dat.M_cmbSBSL3_pbst.getItemCount()-1;j>0;j--)
						cl_dat.M_cmbSBSL3_pbst.removeItemAt(j);
		...
		{//POPULATING OPTION COMBO
			...
			//System.out.println("M_strSBSCD : in cl_pbase "+M_strSBSCD);
				
			if(cl_dat.M_strEINFL_pbst.equals("I"))
				cl_dat.M_strCMPCD_pbst = M_strSBSCD.substring(0,2);
	...
	private void setOPTN(int i)
	{
		if(i==-1)
		{
			...
			cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPFAX_pbst);
		}
		else
		{
			if(M_staUSRRT[i][M_intAE_ADDFL].equalsIgnoreCase("Y"))
				cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPADD_pbst);
			if(M_staUSRRT[i][M_intAE_MODFL].equalsIgnoreCase("Y"))
				cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPMOD_pbst);
			if(M_staUSRRT[i][M_intAE_DELFL].equalsIgnoreCase("Y"))
				cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPDEL_pbst);
			if(M_staUSRRT[i][M_intAE_ENQFL].equalsIgnoreCase("Y"))
				cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPENQ_pbst);
			//ADDED ON 02/04/2004 BY AAP FOR AUTHORISATION OPTION
			if(M_staUSRRT[i][M_intAE_AUTFL].equalsIgnoreCase("Y"))
	...
	protected  void cl_hlp(Object P_objSQLQRY,int P_intCLSCH,int P_intCLRTN,String[] P_staHEADR,int P_intNUMCL,String P_strHLPOS)
	{
		try
		{
			...
			cl_dat.M_pnlDYNTBL_pbst = crtHLPPNL(P_objSQLQRY,P_intNUMCL,P_staHEADR,0,0,(M_flgBIGHLP ? 1028 : 550),145);
...
//			cl_dat.M_txtHLPPOS_pbst.addKeyListner(this);
			...
			cl_dat.M_wndHLP_pbst.setBounds(cl_dat.M_intXPOS_pbst,cl_dat.M_intYPOS_pbst,(M_flgBIGHLP ? 1056 : 600),230);
			cl_dat.M_wndHLP_pbst.toFront();
			cl_dat.M_wndHLP_pbst.setVisible(true);
	...
	//ADDED BY AAP 02/06/2004
	/**
	 * Dummy method
	 * 
	 * Dummy method
	 * 
	 * <p>Introduced as orignal method revised to Object as first aurgument.<br>This method is added to avoid recompiling of existing classes compiled with method having String as first aurgument. Typcasts String to Object and calls revised method.
	 */
	protected  void cl_hlp(String P_strSQLQRY,int P_intCLSCH,int P_intCLRTN,String[] P_staHEADR,int P_intNUMCL,String P_strHLPOS)
	{
		cl_hlp((Object)P_strSQLQRY,P_intCLSCH,P_intCLRTN,P_staHEADR,P_intNUMCL,P_strHLPOS);
	}
/**
 ...
 */
	protected  JPanel crtHLPPNL(Object P_objSQLQRY,int P_intNUMCL,String[] LP_COLHD,int P_intXPOSN,int P_intYPOSN,int L_intWIDTH,int L_intHIGHT){
		...
		try{
			...
			cl_dat.M_hstHELP_pbst = new Hashtable<String,String>();
				...
				if(cl_dat.M_rstHELP_pbst !=null){
					while (cl_dat.M_rstHELP_pbst.next()){
						...
						for(int j = 0;j < P_intNUMCL;j++)
						{
							...
							//System.out.println(LM_SPLTP);
							if(!LM_SPLTP.equalsIgnoreCase("DATE")){
								if(LM_SPLTP.equalsIgnoreCase("Float")){
									L_FLTVL = cl_dat.M_rstHELP_pbst.getFloat(j+1);

									L_COLVAL= Integer.toString((int)L_FLTVL);//shubham
									//System.out.println(L_COLVAL);
									//shubham L_COLVAL = String.valueOf(L_FLTVL);
								}else
									L_COLVAL = cl_dat.M_rstHELP_pbst.getString(j+1);
								if(LM_SPLTP.equalsIgnoreCase("TIMESTAMP")){
									if(L_COLVAL != null)
										L_COLVAL = M_fmtLCDTM.format(cl_dat.M_rstHELP_pbst.getTimestamp(j+1));
								}
							}
							else if(LM_SPLTP.equalsIgnoreCase("DATE")){
 ...
 */
	 ...
	 * 
	 * @deprecated since v2.0.0 as report folder is chcked in fr_log at entry point.
	 */
	...
	void setENBL(boolean L_STAT)
	{
		...
		System.out.println("setENBL 2");
		System.out.println("setENBL 3");
	}
 ...
 */
	...
	public void setUSRRT(String[][] LP_USRRT)
	{
		...
		try
		{
			...
			{
				M_staUSRRT=LP_USRRT;
				for (int j=cl_dat.M_cmbSBSL1_pbst.getItemCount()-1;j>0;j--)
						cl_dat.M_cmbSBSL1_pbst.removeItemAt(j);
				//cl_dat.M_cmbSBSL1_pbst.removeAllItems();
				//cl_dat.M_cmbSBSL1_pbst.addItem("select");
				for(int i=0;i<LP_USRRT.length&&LP_USRRT[i][0]!=null;i++)
				{//SCANNING SBS RIGHTS
					if(LP_USRRT[i][1]!=null && M_intSBSCT >= 1)//cl_dat.M_cmbSBSL1_pbst!=null)
					{
						boolean flag=true;
			...
			///***   ADDED BY AAP ON 11/01/2004 FOR SEPARATE STRING OF SBS FILTER. API HBP
				M_strSBSLS="(";
				for(int i=0;i<M_staUSRRT.length;i++)
					M_strSBSLS+="'"+M_staUSRRT[i][0].substring(2)+"',";
				M_strSBSLS=M_strSBSLS.substring(0,M_strSBSLS.length()-1);
				M_strSBSLS+=")";
			///***
			}
			else
			{//FOR SYSTEM WITHOUT SBS CODES. NOT TO BE USED. CODED FOR FUTURE MODIFICATION, IF REQUIRED
				...
				if(cl_dat.M_btnSAVE_pbst.getText().trim().equals("Run"))
				{
					//cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPRSL_pbst);
					cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPFIL_pbst);
					cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPEML_pbst);
				...
				{
					//cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPSEL_pbst);
					if(LP_USRRT[0][M_intAE_ADDFL].equals("Y"))
						cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPADD_pbst);
					if(LP_USRRT[0][M_intAE_MODFL].equals("Y"))
						cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPMOD_pbst);
					if(LP_USRRT[0][M_intAE_DELFL].equals("Y"))
						cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPDEL_pbst);
					if(LP_USRRT[0][M_intAE_ENQFL].equals("Y"))
						cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPENQ_pbst);
					//ADDED ON 02/04/2004 BY AAP FOR AUTHORISATION OPTION
					if(LP_USRRT[0][M_intAE_AUTFL].equals("Y"))
						cl_dat.M_cmbOPTN_pbst.addItem(cl_dat.M_OPAUT_pbst);
				}
				cl_dat.M_cmbOPTN_pbst.requestFocus();
			}
 ...
 String getCODVL(String P_strHSKEY,int P_intCOLID) throws Exception
    {
        if(cl_dat.M_hstMKTCD_pbst == null)
            return " ";
        if(cl_dat.M_hstMKTCD_pbst.containsKey((String)P_strHSKEY))    
        {
            return ((String[])cl_dat.M_hstMKTCD_pbst.get(P_strHSKEY))[P_intCOLID];
        }
        return " ";
    }
    boolean vldMKTCD(String P_strHSKEY) throws Exception
    {
        if(cl_dat.M_hstMKTCD_pbst == null)
            return false;
        return cl_dat.M_hstMKTCD_pbst.containsKey((String)P_strHSKEY);    
    }
	
	public String vldDATE(String LP_STR) /** Validates date. 
	                         ...
	                         */
	{
		...
		/** Returning product type for Specified Product Code
	 */
	public String getPRDTP(String LP_PRDCD)
	{
		String L_strPRDTP = "";
		try
		{
			if(LP_PRDCD.trim().length() != 10)
				return L_strPRDTP;
			else if(LP_PRDCD.substring(0,3).equals("511") || LP_PRDCD.substring(0,3).equals("519"))  // Polystyrene (including scrap)
			   L_strPRDTP = "01";
			else if(LP_PRDCD.substring(0,3).equals("512"))  //  EPS
			   L_strPRDTP = "03";
			else if(LP_PRDCD.substring(0,2).equals("52"))   // Speciality Polystyrene
			   L_strPRDTP = "02";
			else if(LP_PRDCD.substring(0,2).equals("54"))   // Masterbatch
			   L_strPRDTP = "05";
			else if(LP_PRDCD.substring(0,2).equals("53"))   // Article of Polystyrene (Wood Profile, XPS etc.)
			   L_strPRDTP = "04";
			else if(LP_PRDCD.substring(0,2).equals("SX"))   // Article of Polystyrene (Wood Profile, XPS etc.)
			   L_strPRDTP = "07";
			else if(LP_PRDCD.substring(0,4).equals("6805"))   // Styrene
			   L_strPRDTP = "11";
		}
		catch(Exception e)
		{setMSG(e,"getPRDTP");}
