...
{			
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		if(L_KE.getKeyCode() == L_KE.VK_F1)
		{
			if(M_objSOURC == txtPRTCD)
			{
				...
				M_strSQLQRY += " AND PL_PRTTP=PT_PRTTP AND PL_PRTCD=PT_PRTCD  AND ifnull(PT_STSFL,'')<> 'X' AND PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' ";
				if(txtPRTCD.getText().trim().length()>0)
				{
		...
		if(L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
			...
			if(M_objSOURC == txtPRTCD)
			{
				...
				if(txtPRTCD.getText().trim().length() ==5)
				{
					try
					{
						M_strSQLQRY="Select distinct PT_PRTCD,PT_PRTNM from CO_PTMST,MR_PLTRN  where  PT_PRTTP='"+(cmbPRTTP.getSelectedItem().toString()).substring(0,1)+"' AND PL_PRTCD=PT_PRTCD AND PL_PRTCD=PT_PRTCD  AND ifnull(PT_STSFL,'')<> 'X' AND PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' ";
	...
	private void getDATA()
	{
		try
		{
			...
			M_strSQLQRY +=" AND PT_PRTCD='"+txtPRTCD.getText().trim().toUpperCase()+"' and ifnull(PT_STSFL,'')<> 'X'"; 
		...
		//	System.out.println("DATE 1 ="+L_datTMPDT);
			...
			if(flgTEMP)
			{
				...
				M_strSQLQRY +=" AND PL_PRTCD='"+txtPRTCD.getText().trim().toUpperCase()+"' AND PL_DOCDT >='"+M_fmtDBDAT.format(M_fmtLCDAT.parse(txtFMDAT.getText().trim()))+ "' and ifnull(PL_STSFL,'')<> 'X' order by PL_SEQNO"; 
				...
				if(intSEQNO>1)
				{
					...
					M_strSQLQRY +=" AND PL_PRTCD='"+txtPRTCD.getText().trim().toUpperCase()+"'  AND ifnull(PL_STSFL,'')<> 'X'"; 
			...
			M_strSQLQRY="select PL_DOCDT,PL_DOCNO,PL_SEQNO,PL_DOCVL,PL_BALVL,PL_BALFL,substr(PL_DOCTP,1,1)PL_DOCTP from MR_PLTRN ";
			...
			M_strSQLQRY +="and ifnull(PL_STSFL,'')<> 'X' order by PL_SEQNO"; 
