...
class hr_teapr_fn extends JFrame implements ActionListener,Runnable
{
   	private File file;
	...
	private static String M_strCMPLS = "('01','02','03','04','05','11')";
    hr_teapr_fn()
    {
    ...
    public void run()
    {
        try
        {
            ...
            cstEMPCT = conSPDBA.prepareCall("{call hr_ephct_fn()}");
        }
 ...
 public void actionPerformed(ActionEvent L_AE)
{
    ...
    public static void main(String args[])
    {
        try
        {
        hr_teapr_fn ohr_teapr_fn = new hr_teapr_fn();
        if(thrCONNECT!=null)
			thrCONNECT.join();
        hr_teapr_fn_thrData thr1 = new hr_teapr_fn_thrData(conSPDBA);
        thr1.start();
        thr1.join();
        if(conSPDBA !=null)
        {
   ...
   private static void prcLOG()
	{
		try
	    {
			...
			int L_intSRLNO =0;
            cstSWTRN = conSPDBA.prepareCall("{call updSWTRN_SLT_fn()}");
            cstSWTRN.executeUpdate();
			//--------------------------------------  added on 31-03-2011
			strSQLQRY = "update hr_sltrn1 a set slt_stsfl='2' where slt_cmpcd in "+M_strCMPLS+" and slt_stsfl='1' and  SLT_EMPCD||CHAR(SLT_PNCTM) IN (select EP_SAPNO||CHAR(SLT_PNCTM) from HO_SLTRN,HR_EPMST  where slt_cmpcd=ep_cmpcd and slt_empcd=ep_sapno and slt_stsfl='1')";
            if(stmSPDBA.executeUpdate(strSQLQRY) >0);
                conSPDBA.commit();
            //---------------------------------------
			strSQLQRY = "DELETE FROM HO_SLTRN A WHERE A.SLT_CMPCD in "+M_strCMPLS+" AND A.SLT_EMPNO||CHAR(A.SLT_PNCTM) NOT IN (SELECT B.SLT_EMPNO||MAX(CHAR(B.SLT_PNCTM)) FROM HO_SLTRN B WHERE B.SLT_CMPCD in "+M_strCMPLS+" GROUP BY B.SLT_EMPNO)";				//System.out.println(strSQLQRY);	 				if (conSPDBA != null)
			if(stmSPDBA.executeUpdate(strSQLQRY)>0)
			{
				conSPDBA.commit();
...
class hr_teapr_fn_thrData extends Thread
{
   // Hashtable hstData;
    String M_strCMPLS= "('01','02','03','04','05','11')";
    ...
    hr_teapr_fn_thrData(Connection P_conSPDBA)
    {
    ...
    public void run()
    {
		 ...
		 String L_strACCRF="",L_strTRMID="",L_strCMPCD ="",L_strEMPNO="",L_strEMPCD="",L_strPNCDT="",L_strPNCDTM="",L_strPNCDT1="",L_strPNCTM="",L_strPNCTM1="",L_strINOCD="";		 
		 ...
		 try
          {
         	...
         	stmSPDBA2 = conSPDBA.createStatement();
   			String L_strTMP = "Select distinct slt_cmpcd, ep_empno, ep_accrf, slt_empcd, slt_pnctm, slt_inocd, slt_trmid, slt_stsfl from hr_sltrn1,hr_epmst where slt_cmpcd in  "+M_strCMPLS+" and ep_cmpcd=slt_cmpcd and ep_sapno=slt_empcd and slt_stsfl='1'  order by slt_pnctm desc";
            ...
            if(L_rstRSSET !=null)
            {
                ...
                while((L_rstRSSET.next())&& L_flgRUN)
                {
                    ...
                    L_strINOCD = L_rstRSSET.getString("slt_inocd");
		   			L_STRQRY1 = "Select count(*) slt_count from ho_sltrn where slt_cmpcd in "+M_strCMPLS+" and slt_empno = '"+L_strEMPNO+"' and slt_pnctm='"+L_strPNCDTM+"' and slt_inocd='"+L_strINOCD+"'";
   			...
   			L_strTMP = "Select distinct slt_cmpcd,  slt_empcd, slt_pnctm, slt_inocd, slt_trmid, slt_stsfl from hr_sltrn1 where slt_cmpcd in "+M_strCMPLS+" and slt_stsfl='1'  order by slt_pnctm desc";
            System.out.println(L_strTMP);
            ...
            if(L_rstRSSET !=null)
            {
                ...
                while((L_rstRSSET.next())&& L_flgRUN)
                {
					...
					L_strPNCDTM = L_rstRSSET.getString("slt_pnctm");
                    L_strINOCD = L_rstRSSET.getString("slt_inocd");
                	L_STRQRY1 = "Select count(*) slt_count from hr_slhst where slt_cmpcd in "+M_strCMPLS+" and slt_empcd = '"+L_strEMPCD+"' and slt_pnctm='"+L_strPNCDTM+"' and slt_inocd='"+L_strINOCD+"'";
		            System.out.println(L_STRQRY1);
		            L_rstRSSET2 = stmSPDBA2.executeQuery(L_STRQRY1);
		            if(L_rstRSSET2==null)
		               continue;
   			    ...
   			    //--------------------------------------  commented on 31-03-2011
				//L_STRSQL = "update hr_sltrn1 a set slt_stsfl='2' where slt_cmpcd in "+M_strCMPLS+" and slt_stsfl='1'";
				//System.out.println(L_STRSQL);
