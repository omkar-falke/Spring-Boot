...
create procedure spldata.updPLTRN_PMR(IN LP_CMPCD char(2),IN LP_MKTTP char(2), IN LP_DOCNO char(8))  language sql  modifies sql data 
...
begin
  ...
  declare C_PMR cursor for  select  PR_PRTTP,PR_PRTCD, PR_DOCTP, PR_DOCDT, PR_SBSCD, PR_GRPCD,PR_CNSCD,sum(PR_RCTVL) PR_RCTVL from spldata.mr_prtrn   where   PR_CMPCD = LP_CMPCD and PR_MKTTP = LP_MKTTP and PR_DOCNO=LP_DOCNO and  PR_DOCTP in ('11','12','13','19')  group by PR_PRTTP,PR_PRTCD, PR_DOCTP, PR_DOCDT, PR_SBSCD, PR_GRPCD,PR_CNSCD;
  ...
  while END_TABLE = 0 DO 
      ...
      select isnull(PL_SEQNO,0) PL_SEQNO into L_SEQNO from spldata.MR_PLTRN where PL_CMPCD = LP_CMPCD and PL_PRTTP = L_PRTTP and PL_PRTCD = L_PRTCD and PL_DOCTP = L_DOCTP and PL_DOCNO = LP_DOCNO;
       ...
       select  max(isnull(PL_SEQNO,0)) PL_SEQNO into L_SEQNO from spldata.MR_PLTRN where PL_CMPCD = LP_CMPCD and PL_PRTTP = L_PRTTP and PL_PRTCD = L_PRTCD;
      ...
      select count(*) into L_RECCT from spldata.MR_PLTRN where PL_CMPCD= LP_CMPCD and PL_PRTTP = L_PRTTP  and PL_PRTCD = L_PRTCD and PL_DOCTP = L_DOCTP and PL_DOCNO = LP_DOCNO;
...
-- Old logic (only for reference)
  ...
  while END_TABLE = 0 DO 
     ...
     if L_SEQNO = 0 then 
       ...
       insert into spldata.mr_pltrn (PL_CMPCD,PL_PRTTP,PL_PRTCD,PL_DOCTP,PL_DOCNO,PL_SEQNO,PL_DOCDT,PL_DOCVL,PL_BALVL,PL_BALFL,PL_ADJVL,PL_ACCRF,PL_ACCDT,PL_MKTTP,PL_GRPCD,PL_CNSCD,PL_SBSCD,PL_TRNFL,PL_STSFL,PL_LUSBY,PL_LUPDT) values  (LP_CMPCD,L_PRTTP,L_PRTCD,L_DOCTP,LP_DOCNO,L_SEQNO+1,L_DOCDT,round(L_DOCVL,0),round(L_BALVL1,0),L_BALFL1,round(L_ADJVL,0),L_ACCRF,L_ACCDT,LP_MKTTP,L_GRPCD,L_CNSCD,L_SBSCD,L_TRNFL,L_STSFL,L_LUSBY,L_LUPDT);
      update spldata.co_ptmst set pt_ytdvl = round(L_BALVL1,0), pt_ytdfl = L_BALFL1  where pt_prttp = L_PRTTP and PT_PRTCD = L_PRTCD;
