...
drop procedure splDATA.rwkPLTRN_PRT_MM;
commit;

create procedure splDATA.rwkPLTRN_PRT_MM(IN LP_CMPCD char(2),IN LP_PRTTP char(1), IN LP_PRTCD char(5),IN LP_YSTDT char(10), IN LP_YOPVL decimal(12),IN LP_YOPFL char(2))  language sql  modifies sql data 
P1:
begin
  ...
  declare C_SRL cursor for  select  PL_DOCDT, CMT_CCSVL PL_DOCSQ, PL_DOCNO, PL_DOCTP  from  splDATA.mm_pltrn, splDATA.co_cdtrn  where   cmt_cgmtp='SYS' and  cmt_cgstp='MMXXPTT' and cmt_codcd = SUBSTRING(PL_DOCNO,2,2)  and pl_cmpcd = LP_CMPCD  and pl_prttp = LP_PRTTP and pl_prtcd = LP_PRTCD and pl_docdt >= L_STRDT  and pl_doctp <>  '21'  union all 
select  PL_DOCDT, CMT_CCSVL PL_DOCSQ, PL_DOCNO, PL_DOCTP  from  splDATA.mm_pltrn, splDATA.co_cdtrn   where   cmt_cgmtp='SYS' and  cmt_cgstp='MMXXPTT' and cmt_codcd = PL_DOCTP  and pl_cmpcd = LP_CMPCD and pl_prttp = LP_PRTTP and pl_prtcd = LP_PRTCD and pl_docdt >= L_STRDT and   pl_doctp  = '21'  order by PL_DOCDT, PL_DOCSQ,PL_DOCNO;

  declare C_PLD cursor for  select  PL_SEQNO, PL_DOCTP, round(PL_DOCVL,0) PL_DOCVL from  splDATA.mm_pltrn  where   pl_cmpcd = LP_CMPCD and pl_prttp = LP_PRTTP and pl_prtcd = LP_PRTCD and pl_docdt >= L_STRDT order by pl_seqno;
...
--end if;
  ...
  fetch C_SRL into  L_DOCDT, L_DOCSQ, L_DOCNO, L_DOCTP;
   while END_TABLE = 0 do
      ...
      update splDATA.mm_pltrn set PL_SEQNO = L_SEQNO_RUN  where PL_CMPCD = LP_CMPCD and PL_PRTTP = LP_PRTTP and PL_PRTCD = LP_PRTCD and PL_DOCDT = L_DOCDT and PL_DOCTP = L_DOCTP and PL_DOCNO = L_DOCNO;
...
--          if SUBSTRING(L_DOCTP,1,1)  in (L_DBSER1,L_DBSER2) then
          if SUBSTRING(L_DOCTP,1,1) in (L_CRSER1,L_CRSER2) then  	
             -- set L_TRNFL_NEW = 'DB';
             set L_TRNFL_NEW = 'CR';	
          end if;
  ...
  if LP_YOPVL >= 0  and LP_YOPFL = 'DB' then
       ...
       select count(*) into L_RECCT from splDATA.Mm_PLTRN where PL_CMPCD = LP_CMPCD and PL_PRTTP = LP_PRTTP and PL_PRTCD = LP_PRTCD and PL_DOCTP = L_DOCTP  and PL_DOCNO = L_DOCNO;

      -- select sum(ifnull(PL_ADJVL,0)) into L_ADJVL from splDATA.Mm_PLTRN where PL_CMPCD = LP_CMPCD and PL_PRTTP = LP_PRTTP and PL_PRTCD = LP_PRTCD and PL_DOCTP = L_DOCTP and PL_DOCDT < LP_YSTDT and PL_DOCNO <> L_DOCNO;

       if L_RECCT > 0 then
           update spldata.Mm_PLTRN set PL_DOCVL = LP_YOPVL  where PL_CMPCD = LP_CMPCD and PL_PRTTP = LP_PRTTP and PL_PRTCD = LP_PRTCD and PL_DOCTP = L_DOCTP and PL_DOCNO = L_DOCNO;
       else
...
select  PL_SEQNO,PL_DOCDT, CMT_CCSVL PL_DOCSQ, PL_DOCNO, PL_DOCTP  from  spldata.mm_pltrn, spldata.co_cdtrn  where   cmt_cgmtp='SYS' and cmt_cgstp='MRXXPTT' and cmt_codcd = PL_DOCTP  and pl_cmpcd = '01' and pl_prttp = 'C' and pl_prtcd = 'I0079' and pl_docdt >= '07/01/2006' order by PL_DOCDT, CMT_CCSVL, PL_DOCNO;
...
DELETE FROM SPLDATA.MM_PLTRN WHERE PL_DOCTP='39';

select * from splDATA.co_cdtrn where CMT_CGSTP='MMXXPTT';cmt_codcd='21';
UPDATE SPLDATA.CO_CDTRN SET CMT_CCSVL='10' WHERE CMT_CGSTP='MMXXPTT' AND CMT_CODCD='19';
COMMIT;
select * from spldata.mm_WBTRN; where WB_DOCNO LIKE('90107117');
UPDATE SPLDATA.MM_WBTRN SET WB_TPRCD='P0106',WB_TPRDS='PUNEET TRANSPORT COMPANY' where WB_DOCNO='90107117';
SELECT * FROM SPLDATA.CO_PTMST WHERE PT_PRTCD='P0106' AND PT_PRTTP='T';
