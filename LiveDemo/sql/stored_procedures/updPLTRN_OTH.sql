...
-- procedure to create/update data in Party Ledger transaction using Other Credit Note details - miscelleneous cat
-- This procedure is to be executed when other credit note is added

-- FEATURES INTRODUCED
-- New record generated will have
--  Credit category,Party Type,Paty code, document number from parameters supplied. 
...
drop procedure spldata.updPLTRN_OTH;
commit;

create procedure spldata.updPLTRN_OTH(IN LP_CMPCD char(2),IN LP_MKTTP char(2),IN LP_CRDTP char(2),IN LP_PRTTP CHAR(1),IN LP_PRTCD char(5), LP_DOCRF char(8), IN LP_DOCDT char(10),  IN LP_PNTVL decimal (12,2))  language sql  modifies sql data
  P1:
 begin   declare L_PRTTP char(1);   declare L_PRTCD char(5); 
 ...
 declare L_DOCNO char(8); 
  ...
  declare L_ADJVL decimal(12,2);
   declare L_ACCRF char(8);   declare L_ACCDT date;
  ...
  declare L_PLTRN_PUSH char(1);
   ...
   declare continue handler for not found 
        set END_TABLE = 1;  
   set L_PRTTP = LP_PRTTP; 
  set L_PRTCD = LP_PRTCD; 
  set L_GRPCD = L_PRTCD;  
 ...
 set L_DOCNO = LP_DOCRF; 
  ...
  set L_TRNFL = '0';  
    ...
    if L_SEQNO > 0 then 
       ...
       if L_SEQNO =0 then          call spldata.rwkPLTRN_SRL(LP_CMPCD, L_PRTTP, L_PRTCD,L_DOCDT, L_DOCTP, L_DOCNO,L_SEQNO);         set L_PLTRN_PUSH = '0'; 
        if L_SEQNO > 0 then 
            set L_PLTRN_PUSH = '1';  
       end if;         
--insert into spldata.temp_chk values ('Received Seq.No. : '||char(L_SEQNO));
         ...
         if L_BALFL = 'CR'  then 
            set L_BALVL1 = isnull(L_BALVL,0) + isnull(L_DOCVL,0);             set L_BALFL1 = 'CR';         else             set L_BALVL1 = isnull(L_BALVL,0) - isnull(L_DOCVL,0); 
           set L_BALFL1 = 'DB'; 
         end if;          
-- After deducting credit amount, if the balance goes negative,  Then it is a Credit Balancde 
         ...
         insert into spldata.mr_pltrn (PL_CMPCD,PL_PRTTP,PL_PRTCD,PL_DOCTP,PL_DOCNO,PL_SEQNO,PL_DOCDT,PL_DOCVL,PL_BALVL,PL_BALFL,PL_ADJVL,PL_ACCRF,PL_ACCDT,PL_MKTTP,PL_GRPCD,PL_SBSCD,PL_TRNFL,PL_STSFL,PL_LUSBY,PL_LUPDT) values  (LP_CMPCD,L_PRTTP,L_PRTCD,L_DOCTP,L_DOCNO,L_SEQNO,L_DOCDT,L_DOCVL,L_BALVL1,L_BALFL1,L_ADJVL,L_ACCRF,L_ACCDT,LP_MKTTP,L_GRPCD,L_SBSCD,L_TRNFL,L_STSFL,L_LUSBY,L_LUPDT);       update spldata.co_ptmst set pt_ytdvl = L_BALVL1, pt_ytdfl = L_BALFL1 where pt_prttp = L_PRTTP and PT_PRTCD = L_PRTCD; 
...
call spldata.updPLTRN_OTH('01','01','0B','C','G0077','80900328','02/06/2008',7700.00);
commit;
call spldata.updPLTRN_OTH('01','01','0Z','C','D0107','90900050','05/16/2008',30000.00);
commit;
;
;
