...
begin
  ...
  while END_TABLE = 0 DO 
     if L_INOCD = '0' then
        set L_WRKDT = date(L_PNCTM);
        if time(L_PNCTM) < '05:00' then
           set L_WRKDT = date(days(L_PNCTM)-1);
        end if; 
        select count(*) into L_RECCT from spldata.HR_SWTRN where SWT_CMPCD = LP_CMPCD and SWT_EMPNO = L_EMPNO and SWT_INCTM = L_PNCTM;
        select ifnull(max(ifnull(SWT_SRLNO,'0')),'0')+1 into L_SRLNO from spldata.HR_SWTRN where SWT_CMPCD = LP_CMPCD and SWT_EMPNO = L_EMPNO and SWT_WRKDT = L_WRKDT;
...
commit;
call spldata.updSWTRN_SLT('02');
commit;

select * from spldata.ho_sltrn where slt_cmpcd='02' and date(slt_pnctm)='01/24/2009';
select * from spldata.hr_sltrn where slt_cmpcd='02' and date(slt_pnctm)='01/24/2009';
...
commit;
   ...
   insert into spldata.hr_sltrn (SLT_EMPNO,SLT_PNCTM,SLT_INOCD,SLT_STSFL,SLT_CMPCD) select distinct SLT_EMPNO,SLT_PNCTM,SLT_INOCD,SLT_STSFL,SLT_CMPCD from spldata.ho_sltrn where ifnull(slt_empno,'') <> 'XXXX' and slt_cmpcd='02';
   commit;

select SLT_EMPNO,SLT_PNCTM,SLT_INOCD,SLT_STSFL,SLT_CMPCD from spldata.ho_sltrn where ifnull(slt_empno,'') <> 'XXXX' and slt_cmpcd='02';
