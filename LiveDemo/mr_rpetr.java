...
{ 										/** JTextField to accept from Date.*/
	...
	mr_rpamd()
	{
	    ...
	    try
		{
			...
			add(rdbDORTP = new JRadioButton("Delivery Order",true),1,3,1,1,pnlDOCTP,'L');
	...
	void exePRINT()
	{
		...
		try
		{
		    ...
		    if(M_rdbHTML.isSelected())
		        strFILNM = cl_dat.M_strREPSTR_pbst +"mr_rpamd.html";
		    else if(M_rdbTEXT.isSelected())
		        strFILNM = cl_dat.M_strREPSTR_pbst + "mr_rpamd.doc";				
			getDATA();				
			if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPPRN_pbst))
			{
				if(intRECCT >0)
...
//			M_strSQLQRY+= " FROM MR_DOTRN A,SPLDATA.CO_PTMST,CO_CDTRN C,CO_CDTRN D," ;
...
//			}
			...
			M_strSQLQRY+= " Integer(A.DOT_AMDNO)-Integer(B.DOT_AMDNO)=1  AND A.DOT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND A.DOT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND A.DOT_DORQT!= B.DOT_DORQT ";
			...
			M_strSQLQRY+= " A.DOT_DORDT != A.DOT_LUPDT AND A.DOT_SBSCD=INT_SBSCD AND A.DOT_CMPCD=INT_CMPCD AND A.DOT_INDNO=INT_INDNO AND ";
			...
			M_strSQLQRY+= " E.DOT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND IFNULL(E.DOT_AMDNO,'00') !='00' AND E.DOT_AMDDT BETWEEN '"+L_strFMDAT+"' AND '"+L_strTODAT+"'";
...
//			}
			...
			M_strSQLQRY+= " (ifnull(a.int_indqt,0)-ifnull(a.int_fcmqt,0)) DOCQT,A.INT_CFWQT CFWQT,B.INT_INDQT PREDQ,IN_LUSBY LUSBY FROM VW_INTRN A,";
			...
			M_strSQLQRY+= " Integer(A.INT_AMDNO)-Integer(B.INT_AMDNO)=1  AND A.INT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND B.INT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND (ifnull(a.int_indqt,0)-ifnull(a.int_fcmqt,0))!= (ifnull(b.int_indqt,0)-ifnull(b.int_fcmqt,0)) ";
			M_strSQLQRY+= " WHERE PT_PRTTP='C'  AND PT_PRTCD=IN_BYRCD AND D.CMT_CGMTP='SYS' AND  D.CMT_CGSTP='MR00SAL'";
			M_strSQLQRY+= " AND D.CMT_CODCD=IN_SALTP AND IN_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND IFNULL(IN_AMDNO,'00') !='00' AND A.IN_AMDDT BETWEEN '"+L_strFMDAT+"'";
			...
			M_strSQLQRY+= " E.INT_PRDDS PRDDS,(ifnull(e.int_indqt,0)-ifnull(e.int_fcmqt,0)) DOCQT,E.INT_CFWQT CFWQT,0  PREDQ,IN_LUSBY LUSBY FROM ";
			...
			M_strSQLQRY+= " H.CMT_CGSTP='MR00SAL' AND H.CMT_CODCD=IN_SALTP AND IN_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND IFNULL(IN_AMDNO,'00') !='00' AND ";
