...
class mr_rpdoh extends cl_rbase
{
	...
	public String strTRPCD;
	public String strTRPNM;
	public String strTREML;
	public String strDSEML;
	public String strDSRNM;
	public String strEMLPR;
	...
	private String strAMDDT;
	private String strPRDRF ="";
	private String strFILRF ="";
	...
	int intTXFLD_TOT = 6;
	String[] arrTXFLD = new String[] {"EXC","EDC","EHC","VAT","SCH","TOT","CST","STX"};
    ...
    private int intAE_TX_VATVL = 6;
    private int intAE_TX_VATFL = 7;
    private int intAE_TX_SCHVL = 8;
    private int intAE_TX_SCHFL = 9;
    private int intAE_TX_TOTVL = 10;
    private int intAE_TX_TOTFL = 11;
    private int intAE_TX_CSTVL = 12;
    private int intAE_TX_CSTFL = 13;
    private int intAE_TX_STXVL = 14;
    private int intAE_TX_STXFL = 15;

    private String strTSELF_fn ="S0029";		
	mr_rpdoh()
	{
		super(2);
			...
			if(L_KE.getKeyCode() == L_KE.VK_F1)
			{
				...
				else if(L_KE.getSource().equals(txtFMDOR))
				{
					...
					M_strSQLQRY = "SELECT DISTINCT DOT_DORNO,DOT_DORDT,PT_PRTNM,DOT_AMDNO,"
						...
						+"DOT_MKTTP = '"+txtPRDTP.getText()+"' and dot_cmpcd = '"+cl_dat.M_strCMPCD_pbst+"' and DOT_CMPCD = IN_CMPCD and DOT_MKTTP = IN_MKTTP AND DOT_INDNO = IN_INDNO AND IN_BYRCD = PT_PRTCD "
						+"AND PT_PRTTP = 'C' AND IN_STSFL IN ('A','U','1','2','3','4','X') and DOT_SBSCD1 in "+M_strSBSLS;
				...
				//		+"AND PT_PRTTP = 'C' AND IN_STSFL IN 'A','U','1','2','3','X') and IN_SBSCD1 in "+M_strSBSLS+" and DOTSBSCD1 inn "+M_strSBSLS;
				//	if(txtFMDOR.getText().trim().length() > 0)
	...
	void exePRINT()
	{
		try
		{
			...
			M_strSQLQRY  = "SELECT IN_BYRCD,IN_CNSCD,IN_INDNO,IN_INDDT,IN_AMDDT,IN_PORNO,"
				+"IN_PORDT,IN_SALTP,IN_DTPCD,IN_DSRCD,IN_DSRTP,IN_CPTVL,IN_AMDNO,IN_CFTAG,IN_DELAD,IN_FILRF,DOT_MKTTP,DOT_DORNO,"
				...
				+"DOT_AUTBY,INT_SRLNO,DOT_PRDCD,DOT_PKGTP,INT_BASRT,INT_EXCRT,INT_PRDRF,DOD_DSPDT,DOD_DORQT FROM VW_INTRN,"
				+"MR_DOTRN LEFT OUTER JOIN MR_DODEL ON DOT_DORNO = DOD_DORNO AND DOT_MKTTP = "
				+"DOD_MKTTP AND DOT_PRDCD = DOD_PRDCD WHERE DOT_cmpcd = '"+cl_dat.M_strCMPCD_pbst+"' and DOT_MKTTP = '"+txtPRDTP.getText().trim()+"' "
			    + (cl_dat.M_strUSRCT_pbst.equals("02") ? " and  IN_DSRTP='"+cl_dat.M_strUSRCD_pbst.substring(0,1)+"' and IN_DSRCD='"+cl_dat.M_strUSRCD_pbst.substring(1,6)+"'" : "")
				+"AND DOT_CMPCD = INT_CMPCD and DOT_MKTTP = INT_MKTTP AND DOT_INDNO = INT_INDNO AND DOT_PRDCD = INT_PRDCD and DOT_PKGTP = INT_PKGTP and DOT_SBSCD1 in "+M_strSBSLS
			...
			if(M_rstRSSET == null || !M_rstRSSET.next())
			{
				...
				while(!flgEOFFL)
				{
					...
					strDELAD = nvlSTRVL(M_rstRSSET.getString("IN_DELAD"),"");
					...
					strAUTDT = (M_rstRSSET.getTimestamp("DOT_AUTDT") == null) ? "" : M_fmtLCDTM.format(M_rstRSSET.getTimestamp("DOT_AUTDT"));
          	...
          	if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPSCN_pbst) || cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPPRN_pbst))
				...
				else if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPEML_pbst) || cl_dat.M_cmbOPTN_pbst.getSelectedItem().equals(cl_dat.M_OPPRN_pbst) || flgOUTPRT==true)
				{
				...
				//{
					if(strAUTBY.length() >0)
					{
						// E-mail to customer for EX-WORKS delivery
						int L_intOPTN = 1;
        				cl_eml ocl_eml = new cl_eml();
						if((strTRPCD.equals(strTSELF_fn))&& (strEMLPR.length()>0))  
						{
							    L_intOPTN = JOptionPane.showConfirmDialog( this,"E-mail D.O. to "+strEMLPR+" ? \n ("+strBYRNM+")","Message",JOptionPane.OK_CANCEL_OPTION);
        						if(L_intOPTN == 0)
        						{
								...
								ocl_eml.sendfile("EXT",strEMLPR,cl_dat.M_strREPSTR_pbst+"mr_rpdoh.html","Delivery Order ","Delivery Order");
        							ocl_eml.setORADR();                              
        							setMSG("Report has been e-mailed to Buyer",'N');																

        						}
						}
						if(strTREML.length() >0)  
						{
        						L_intOPTN = JOptionPane.showConfirmDialog( this,"E-mail D.O. to Transporter "+strTREML+" ? \n ("+strTRPNM+")","Message",JOptionPane.OK_CANCEL_OPTION);
        						if(L_intOPTN == 0)
        						{
        							//cl_eml ocl_eml = new cl_eml();
        							ocl_eml.setFRADR("cms@spl.co.in"); 
								...
								ocl_eml.sendfile("EXT",strTREML,cl_dat.M_strREPSTR_pbst+"mr_rpdoh.html","Delivery Order ","Delivery Order");
        							ocl_eml.setORADR();                              
        							setMSG("Report has been e-mailed to Transporter",'N');	
								}
						}
						
						if(strDSEML.length() >0)  
						{
        						L_intOPTN = JOptionPane.showConfirmDialog( this,"E-mail D.O. to Distributor "+strDSEML+" ? \n ("+strDSRNM+")","Message",JOptionPane.OK_CANCEL_OPTION);
								if(L_intOPTN == 0)
        							{
        								//cl_eml ocl_eml = new cl_eml();
        								ocl_eml.setFRADR("cms@spl.co.in"); 
									//System.out.println("DSR");
        								ocl_eml.sendfile("EXT",strDSEML,cl_dat.M_strREPSTR_pbst+"mr_rpdoh.html","Delivery Order ","Delivery Order");
        								ocl_eml.setORADR();                              
        								setMSG("Report has been e-mailed to Distributor",'N');	
        							}
						}
															
					}		
				//}
					// Fax Part commented on 20/02/2007
 ...
 */
	private void exeFTRPRN()
	{
		try
		{
			...
			M_strSQLQRY = "SELECT RM_REMDS FROM MR_RMMST WHERE RM_cmpcd = '"+cl_dat.M_strCMPCD_pbst+"' and RM_DOCNO ='"+strDORNO_OLD+"' "
			...
			/////
			dosREPORT.writeBytes("<TR>");
			dosREPORT.writeBytes("<TD colSpan=7> <P style=\"LINE-HEIGHT: 0.9\" align=left> Validity of this D.O. is till month end from the date of issue </TD>");
			dosREPORT.writeBytes("</TR>");
			...
			dosREPORT.writeBytes("<TR>");
			dosREPORT.writeBytes("<TD colSpan=4> <P style=\"LINE-HEIGHT: 0.9\" align=left>TIN No. :<STRONG>"+strTINNO+"</STRONG>&nbsp;&nbsp;&nbsp; "+(strVATNO.trim().length()>0 ? "VAT No. : <STRONG>"+strVATNO+"</STRONG>" : ""));
...
//			dosREPORT.writeBytes("<P style=\"LINE-HEIGHT: 0.9\" align=center> &nbsp </P>");
			dosREPORT.writeBytes("<P style=\"LINE-HEIGHT: 0.9\" align=center>"+strAUTBY+"</P>");
			dosREPORT.writeBytes("<P style=\"LINE-HEIGHT: 0.9\" align=center>"+" "+cl_dat.M_txtCLKDT_pbst.getText()+" "+cl_dat.M_txtCLKTM_pbst.getText()+" Hrs"+"</P>");
 ...
 */					
	private void exeGRDPRN(ResultSet LP_RSSET)
	{
		try
		{
			...
			while(!flgEOFFL)
			{
				...
				dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.9\">"+hstPKGTP.get(nvlSTRVL(LP_RSSET.getString("DOT_PKGTP"),"0")).toString()+"</TD>");
			...
			//	strPKGDS = hstPKGTP.get(nvlSTRVL(M_rstRSSET.getString("dot_pkgtp"),"")).toString();
				strPRDRF = nvlSTRVL(LP_RSSET.getString("INT_PRDRF"),"");
	...
	private void exeHDRPRN(ResultSet LP_RSSET)
	{
		try
		{
			...
			if(cl_dat.M_strCMPCD_pbst.equals("01"))
			{
				dosREPORT.writeBytes("<P align=center style = \"LINE-HEIGHT: 0.1\"><FONT size=2>Solitaire Corp.Park, Bldg.11, 5th flr. G.Hargovind Marg Chakala Andheri(E) Mumbai 93</FONT> </P>");
				dosREPORT.writeBytes("<P align=center style = \"LINE-HEIGHT: 0.1\"><FONT size=2>Tel: (91-22)6709 1900  Fax:(91-22)6709 1925/26/4005 5681 Web:www.supremepetrochem.com</FONT></A></P>");
			}
			else if(cl_dat.M_strCMPCD_pbst.equals("11"))
			{
				dosREPORT.writeBytes("<P align=center style = \"LINE-HEIGHT: 0.1\"><FONT size=2>39-I, North Parade Road,Chakarapani Colony, St.Thomus Mount,Chennai - 600 016</FONT> </P>");
				dosREPORT.writeBytes("<P align=center style = \"LINE-HEIGHT: 0.1\"><FONT size=2>Tel: 044 - 2231 3660 / 3659</FONT></A></P>");
			}
		//	dosREPORT.writeBytes("<P align=center style = \"LINE-HEIGHT: 0.8\"><STRONG><FONT size=4>Delivery Order</FONT> </STRONG></P></TD>");
			if(!nvlSTRVL(LP_RSSET.getString("DOT_AMDNO"),"00").equals("00"))
			{
				dosREPORT.writeBytes("<P align=center style = \"LINE-HEIGHT: 0.8\"><STRONG><FONT size=4>Delivery Order Amendment</FONT> </STRONG></P></TD>");
				M_intLINNO += 1;
			}
			else
				dosREPORT.writeBytes("<P align=center style = \"LINE-HEIGHT: 0.8\"><STRONG><FONT size=4>Delivery Order</FONT> </STRONG></P></TD>");
			...
			M_strSQLQRY = "SELECT PT_PRTNM,PT_ADR01,PT_ADR02,PT_ADR03,PT_ADR04,"
				...
				+"PT_CLLDS,PT_ITPNO,PT_TSTFL,PT_TINNO,PT_VATNO,PT_EMLPR,PT_EMLRF,PT_SPLRF FROM CO_PTMST WHERE PT_PRTCD ='"
			...
			{	
				...
				strSPLRF = nvlSTRVL(L_rstRSSET.getString("PT_SPLRF"),"");
				...
				strVATNO = nvlSTRVL(L_rstRSSET.getString("PT_VATNO"),"");
				strEMLPR = nvlSTRVL(L_rstRSSET.getString("PT_EMLPR"),"");
			...
			if(LP_RSSET.getString("IN_BYRCD").equals(LP_RSSET.getString("IN_CNSCD")))
			{
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\"><FONT size=2><STRONG>"+"Same as Buyer"+"</STRONG></p>");
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\">&nbsp</p>");
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\">&nbsp</p>");
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\">&nbsp</p>");
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\">&nbsp</p>");
			...
			else
			{
				...
				+LP_RSSET.getString("IN_CNSCD")+"' AND PT_PRTTP = 'C'";
				L_rstRSSET1 = cl_dat.exeSQLQRY2(M_strSQLQRY);
				if(L_rstRSSET1 !=null)
					L_rstRSSET1.next();	
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\"><FONT size=1><STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_PRTNM"),"")+" ("+LP_RSSET.getString("IN_CNSCD")+")"+"</STRONG></p>");
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\"><FONT size=1.2><STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_ADR01"),"")+"</STRONG></p>");
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\"><FONT size=1.2><STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_ADR02"),"")+"</STRONG></p>");
				dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\"><FONT size=1.2><STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_ADR03"),"")+"</STRONG></p>");
				dosREPORT.writeBytes("<p align=left ><FONT size=1.2><STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_ADR04"),"")+" "+nvlSTRVL(L_rstRSSET1.getString("PT_PINCD"),"")+"</STRONG></p>");
		...
		//		dosREPORT.writeBytes("<p align=left style= \"LINE-HEIGHT: 0.1\">&nbsp</p>");
			...
			if(LP_RSSET.getString("IN_BYRCD").equals(LP_RSSET.getString("IN_CNSCD")))
			{
				dosREPORT.writeBytes("<TD  align=left style= \"LINE-HEIGHT: 0.8\">C.S.T. No : ");
				dosREPORT.writeBytes("<p  align=left style= \"LINE-HEIGHT: 0.8\">S.T. No : </p>");
				dosREPORT.writeBytes("<p  align=left style= \"LINE-HEIGHT: 0.8\">ECC No : </p>");
				dosREPORT.writeBytes("</TD>");
				dosREPORT.writeBytes("</TR>");
			...
			{
				dosREPORT.writeBytes("<TD  align=left style= \"LINE-HEIGHT: 0.8\">C.S.T. No : <STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_CSTNO"),"")+"</STRONG>");
				dosREPORT.writeBytes("<p  align=left style= \"LINE-HEIGHT: 0.8\">S.T. No : <STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_STXNO"),"")+"</STRONG></p>");
				dosREPORT.writeBytes("<p  align=left style= \"LINE-HEIGHT: 0.8\">ECC No : <STRONG>"+nvlSTRVL(L_rstRSSET1.getString("PT_ECCNO"),"")+"</STRONG></p>");
				dosREPORT.writeBytes("</TD>");
				dosREPORT.writeBytes("</TR>");
			}
			...
			dosREPORT.writeBytes("<TR>");
			strAMDDT = (LP_RSSET.getDate("DOT_AMDDT") == null) ? "" : M_fmtLCDAT.format(M_rstRSSET.getDate("DOT_AMDDT"));
			if(nvlSTRVL(LP_RSSET.getString("DOT_AMDNO"),"00").equals("00"))
				dosREPORT.writeBytes("<TD align=left style= \"LINE-HEIGHT: 0.8\">D.O.No : <STRONG>"+nvlSTRVL(LP_RSSET.getString("DOT_DORNO"),"")+"</STRONG>");
			else
				dosREPORT.writeBytes("<TD align=left style= \"LINE-HEIGHT: 0.8\">D.O.No : <STRONG>"+nvlSTRVL(LP_RSSET.getString("DOT_DORNO"),"")+"/"+nvlSTRVL(LP_RSSET.getString("DOT_AMDNO"),"00")+"</STRONG>");

			if(LP_RSSET.getDate("DOT_DORDT") != null)
			{
				if(strAMDDT.length() > 0)
					dosREPORT.writeBytes("<P align=left style= \"LINE-HEIGHT: 0.8\">D.O.Dt : <STRONG>"+M_fmtLCDAT.format(LP_RSSET.getDate("DOT_DORDT"))+"</STRONG>  Amd. Dt. "+strAMDDT+"</P></TD>");
				else
			...
			/*if(!nvlSTRVL(LP_RSSET.getString("DOT_AMDNO"),"00").equals("00"))
			{
				dosREPORT.writeBytes("<P align=left style= \"LINE-HEIGHT: 0.8\">Amd.Dt : "+strAMDDT+"</P></TD>");
				M_intLINNO += 1;
			}*/
			dosREPORT.writeBytes("</TR>");
			//System.out.println("HDRPRN  0006");
			strAMDDT = (LP_RSSET.getDate("IN_AMDDT") == null) ? "" : M_fmtLCDAT.format(M_rstRSSET.getDate("IN_AMDDT"));				
			dosREPORT.writeBytes("<TR>");
			...
			{
				if(!nvlSTRVL(LP_RSSET.getString("DOT_AMDNO"),"00").equals("00"))
					dosREPORT.writeBytes("<P align=left style= \"LINE-HEIGHT: 0.8\">Indent Dt : <STRONG>"+M_fmtLCDAT.format(LP_RSSET.getDate("IN_INDDT"))+"</STRONG>   Amd. Dt. "+strAMDDT+"</P></TD>");
				else
			...
			if(L_rstRSSET.next())
			{
				...
				//strMKTTP = nvlSTRVL(LP_RSSET.getString("IN_MKTTP"),"");
			}
			else
			...
			M_strSQLQRY = "SELECT CMT_CODDS FROM CO_CDTRN WHERE CMT_CGMTP = 'SYS'"
				+" AND CMT_CGSTP = 'MR"+nvlSTRVL(txtPRDTP.getText(),"01")+"MOT' AND CMT_CODCD = '"+nvlSTRVL(LP_RSSET.getString("dot_tmocd"),"")+"'";
			...
			M_strSQLQRY = "SELECT CMT_CODDS FROM CO_CDTRN WHERE CMT_CGMTP = 'SYS'"
				+" AND CMT_CGSTP = 'MRXXDTP' AND CMT_CODCD = '"+nvlSTRVL(LP_RSSET.getString("in_dtpcd"),"")+"'";
			L_rstRSSET = cl_dat.exeSQLQRY3(M_strSQLQRY);
			if(L_rstRSSET.next())
			{
				dosREPORT.writeBytes("<P align=left style= \"LINE-HEIGHT: 0.8\">Del.Type : <STRONG>"+nvlSTRVL(L_rstRSSET.getString("CMT_CODDS"),"")+"</STRONG></P></TD>");
				//dosREPORT.writeBytes("<TD align=left style= \"LINE-HEIGHT: 0.8\">Del.Type : "+L_rstRSSET.getString("CMT_CODDS"));
			}
		...
		//	dosREPORT.writeBytes("<P align=left style= \"LINE-HEIGHT: 0.8\">Vehicle No : <STRONG>"+nvlSTRVL(LP_RSSET.getString("DOT_LRYNO"),"")+"</STRONG></P></TD>");
			...
			strDELAD =	nvlSTRVL(LP_RSSET.getString("in_delad"),"");
			//System.out.println(strDELAD);
			
			if(strSPLRF.length() >0)
			{	
				dosREPORT.writeBytes("<TD colSpan=7><font aling = right>SPL Ref. : <STRONG><FONT size=2>"+cl_dat.M_strCMPNM_pbst+"("+strSPLRF+")</STRONG></FONT></TD>");
				///dosREPORT.writeBytes("<p align=left ><FONT size=1.2>SPL Ref. : <STRONG>"+cl_dat.M_strCMPNM_pbst+"("+nvlSTRVL(L_rstRSSET.getString("PT_SPLRF"),"")+")</STRONG></p>");
				M_intLINNO += 1;
			}
			if(strDELAD.trim().length() >0)
			{
				dosREPORT.writeBytes("<TR>");
				dosREPORT.writeBytes("<TD colSpan=7><font aling = right>Del. Address : <STRONG><FONT size=2>"+strDELAD+"</STRONG></FONT></TD>");
				//dosREPORT.writeBytes("<TD> - </TD>");
				dosREPORT.writeBytes("</TR>");
				M_intLINNO += 1;
				/*if(strDELAD.length() >100)
					dosREPORT.writeBytes("<TD><font aling = right>Del. Address : <STRONG><FONT size=2>"+strDELAD.substring(100)+"</STRONG></FONT></TD>");
				else
					dosREPORT.writeBytes("<TD><font aling = right>Del. Address : <STRONG><FONT size=2>"+strDELAD+"</STRONG></FONT></TD>");
				if(strDELAD.length() >100)*/
			}
			dosREPORT.writeBytes("<TR>");
			M_strSQLQRY = "SELECT PT_PRTNM,PT_EMLPR FROM CO_PTMST WHERE PT_PRTTP = '"+LP_RSSET.getString("IN_DSRTP")+"' AND "
				+"PT_PRTCD = '"+LP_RSSET.getString("IN_DSRCD")+"'";
			L_rstRSSET = cl_dat.exeSQLQRY3(M_strSQLQRY);
			if(L_rstRSSET.next())
			{
				strDSRNM = L_rstRSSET.getString("PT_PRTNM");
				strDSEML = nvlSTRVL(L_rstRSSET.getString("PT_EMLPR"),"");
				System.out.println(strDSRNM +" "+strDSEML);
				dosREPORT.writeBytes("<TD><font aling = right>Distributor : <STRONG><FONT size=2>"+L_rstRSSET.getString("PT_PRTNM")+"</STRONG></FONT></TD>");
			}
			else
			...
			dosREPORT.writeBytes("<TR>");
			L_TMPSTR = cl_dat.getPRMCOD("CMT_CODDS","MST","COXXMKT",txtPRDTP.getText().trim());
		//		if(strMKTTP.equals("01") && strPRDCD1.substring(0,3).equals("512"))
		//			L_TMPSTR = "Expandable Polystyrene";
			dosREPORT.writeBytes("<TD><font aling = right>Product : <STRONG>"+L_TMPSTR+"</STRONG></TD>");
		//	dosREPORT.writeBytes("<TD><font aling = right>Product : <STRONG>Polystyrene</STRONG></TD>");
			strTREML = "";
			M_strSQLQRY = "SELECT PT_PRTCD,PT_PRTNM, PT_FAXNO,PT_EMLPR,PT_EMLRF FROM CO_PTMST WHERE PT_PRTTP = 'T' AND "
				+"PT_PRTCD = '"+LP_RSSET.getString("DOT_TRPCD")+"'";
			L_rstRSSET = cl_dat.exeSQLQRY3(M_strSQLQRY);
			if(L_rstRSSET.next())
			{
				strFAXNO = nvlSTRVL(L_rstRSSET.getString("PT_FAXNO"),"");
				strTRPCD = nvlSTRVL(L_rstRSSET.getString("PT_PRTCD"),"");
				strTRPNM = nvlSTRVL(L_rstRSSET.getString("PT_PRTNM"),"");
				//strTREML = nvlSTRVL(L_rstRSSET.getString("PT_EMLMR"),"");
				strTREML = nvlSTRVL(L_rstRSSET.getString("PT_EMLRF"),"");
			//	System.out.println("strEMAIL 1 : "+strTREML);
				...
				dosREPORT.writeBytes("</TR>");
				dosREPORT.writeBytes("<TR>");
				dosREPORT.writeBytes("<TD>Adv.Lic.File No. : "+nvlSTRVL(LP_RSSET.getString("IN_FILRF"),"")+"</TD>");
			...
			dosREPORT.writeBytes("<TR>");
			String L_strSUBTMP = LP_RSSET.getString("DOT_PRDCD").substring(0,2);
			System.out.println(L_TMPSTR+">>"+L_strSUBTMP);
			dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>"+(L_strSUBTMP.equals("SX") ? "Product":"Grade/Prd. Ref.")+"</STRONG></P></TD>");
			dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>Qty.</STRONG></P><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>"+(L_strSUBTMP.equals("SX") ? "Sq.Mtr.":"MT")+"</STRONG></P></TD>");
			dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>Pkg.</STRONG></P><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>"+(L_strSUBTMP.equals("SX") ? "Size":"Type")+"</STRONG></P></TD>");
			dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>No Of</STRONG> </P><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>"+(L_strSUBTMP.equals("SX") ? "Pcs":"Pkgs.")+"</STRONG></P></TD>");
			dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>Basic Price</STRONG> </P><P align=center style= \"LINE-HEIGHT: 0.8\"><STRONG>"+(L_strSUBTMP.equals("SX") ? "Rs / Sq.Mtr.":"Rs / MT")+"</STRONG>  </P></TD>");
	...
	private void chkTAXPRN(String LP_PRDTP, String LP_INDNO)
	{
		try
		{
			...
			if(intTXLIN>1)
			{
				//System.out.println("strPRDRF "+strPRDRF);
				//System.out.println("intTXLIN "+intTXLIN);

				dosREPORT.writeBytes("<TR>");
				if((intTXLIN==3)&&(strPRDRF.length() >0))
					dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.9\">"+strPRDRF +"</TD>");
				else
					dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.9\">&nbsp</TD>");
				dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.9\">&nbsp</TD>");
				dosREPORT.writeBytes("<TD><P align=center style= \"LINE-HEIGHT: 0.9\">&nbsp</TD>");
