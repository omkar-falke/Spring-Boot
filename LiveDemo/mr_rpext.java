...
<B>Conditions Give in Query:</b>
			 ...
			 3) IFNULL(IN_AMDNO,'00') !='00' AND A.INT_LUPDT BETWEEN '"+L_strFMDAT+"' AND '"+L_strTODAT+"' 
 ...
 */
public class mr_rpext extends cl_rbase
{ 										/** JTextField to accept from Date.*/
	//private JTextField txtFMDAT;		/** JTextField to accept to Date.*/
	//private JTextField M_txtTODAT;		/** JTextField to accept Main Product Category.*/
	...
	mr_rpext()
	{
        ...
        try
		{
			...
			//add(M_txtTODAT = new TxtDate(),5,5,1,1,this,'L');
			...
			add(rdbINDTP = new JRadioButton("Indent",true),1,2,1,1,this,'L');
			add(rdbDORTP = new JRadioButton("Delivery Order",true),1,3,1,1,this,'L');
	...
	*/
	void setENBL(boolean L_flgSTAT)
	{
		...
		M_txtTODAT.setEnabled(L_flgSTAT);		
		M_txtFMDAT.setEnabled(L_flgSTAT);		
		if (L_flgSTAT==true )
		{
			if (((M_txtTODAT.getText().trim()).length()== 0) ||((M_txtFMDAT.getText().trim()).length()==0))
			{					 
				try
				{
	...
	*/
	boolean vldDATA()
	{
		try
		{	
			if(M_txtFMDAT.getText().trim().toString().length() == 0)			
			{
				setMSG("Please Enter valid From-Date, To Specify Date Range ..",'E');
				M_txtFMDAT.requestFocus();
				return false;
			}
			else if(M_txtTODAT.getText().trim().toString().length() == 0)			
			{	
				setMSG("Please Enter valid To-Date To Specify Date Range ..",'E');
				M_txtTODAT.requestFocus();
				return false;
			}					
			else if (M_fmtLCDAT.parse(M_txtTODAT.getText().trim()).compareTo(M_fmtLCDAT.parse(cl_dat.M_strLOGDT_pbst))>0)			
			{			    
				setMSG("Please Enter valid To-Date, To Specify Date Range .. ",'E');				
				try
				{
			...
			else if (M_fmtLCDAT.parse(M_txtFMDAT.getText().trim()).compareTo(M_fmtLCDAT.parse(M_txtTODAT.getText().trim()))>0)			
			{			    
	...
	*/
	...
	{ 	    		
	    ...
	    {	
			...
			L_strFMDAT = M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText().trim()));
			L_strTODAT = M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText().trim()));
...
//			M_strSQLQRY+= "	A.DOT_AMDDT AMDDT, SUBSTR(A.DOT_SBSCD,1,2) ZONCD,SUBSTR(A.DOT_SBSCD,3,2)  SALTP,D.CMT_CODDS SALDS, " ;
...
//			M_strSQLQRY+= " AND  IFNULL(A.DOT_AMDNO,'00') !='00' AND A.DOT_LUPDT BETWEEN  '"+L_strFMDAT+"'"; 
//			M_strSQLQRY+= " AND '"+L_strTODAT+"' AND SUBSTR(A.DOT_SBSCD,3,2)  IN ('01','03','12') " ;
//			M_strSQLQRY+= "	AND SUBSTR(A.DOT_SBSCD,1,2)=C.CMT_CODCD AND C.CMT_CGMTP='SYS'  AND C.CMT_CGSTP='MR00ZON' " ;
//			M_strSQLQRY+= " AND D.CMT_CGMTP='SYS' AND D.CMT_CGSTP='MR00SAL' AND D.CMT_CODCD=SUBSTR(A.DOT_SBSCD,3,2)" ;
//			M_strSQLQRY+= " AND A.DOT_SBSCD=IN_SBSCD AND IN_INDNO=A.DOT_INDNO and int_indno=A.dot_indno AND INT_PRDCD=A.DOT_PRDCD " ;
//			M_strSQLQRY+= "ORDER BY C.CMT_CHP02,substr(A.DOT_SBSCD,3,2),INT_PRDGR,A.DOT_DORNO ";
//			}
			{
			M_strSQLQRY = " SELECT  distinct C.CMT_CHP02 RGNCD,A.DOT_DORNO DOCNO,A.DOT_AMDNO AMDNO,A.DOT_DORDT DOCDT,";
			M_strSQLQRY+= " A.DOT_AMDDT AMDDT, SUBSTR(A.DOT_SBSCD,1,2) ZONCD,SUBSTR(A.DOT_SBSCD,3,2) SALTP, ";
			...
			M_strSQLQRY+= " A.DOT_SBSCD=B.DOT_SBSCD AND A.DOT_DORNO=B.DOT_DORNO  AND A.DOT_PRDCD=B.DOT_PRDCD AND ";
			M_strSQLQRY+= " Integer(A.DOT_AMDNO)-Integer(B.DOT_AMDNO)=1  AND A.DOT_DORQT!= B.DOT_DORQT ";
			M_strSQLQRY+= " WHERE PT_PRTTP='C'  AND PT_PRTCD=IN_BYRCD AND D.CMT_CGMTP='SYS' AND  D.CMT_CGSTP='MR00SAL' ";
			M_strSQLQRY+= " AND D.CMT_CODCD=SUBSTR(A.DOT_SBSCD,3,2) AND IFNULL(A.DOT_AMDNO,'00') !='00' AND A.DOT_AMDDT ";
			M_strSQLQRY+= " BETWEEN '"+L_strFMDAT+"' AND '"+L_strTODAT+"'  AND SUBSTR(A.DOT_SBSCD,3,2) IN ('01','03','12') AND ";
			M_strSQLQRY+= " C.CMT_CGMTP='SYS' AND  C.CMT_CGSTP='MR00ZON' AND SUBSTR(A.DOT_SBSCD,1,2)=C.CMT_CODCD AND ";
			M_strSQLQRY+= " A.DOT_DORDT != A.DOT_LUPDT AND A.DOT_SBSCD=INT_SBSCD AND A.DOT_INDNO=INT_INDNO AND ";
			M_strSQLQRY+= " A.DOT_PRDCD=INT_PRDCD UNION ALL  SELECT  G.CMT_CHP02 RGNCD,E.DOT_DORNO DOCNO,";
			M_strSQLQRY+= " E.DOT_AMDNO AMDNO,E.DOT_DORDT DOCDT,E.DOT_AMDDT AMDDT,  SUBSTR(E.DOT_SBSCD,1,2) ZONCD,";
			M_strSQLQRY+= " SUBSTR(E.DOT_SBSCD,3,2) SALTP,H.CMT_CODDS SALDS,PT_PRTNM BUYER,E.DOT_PRDDS PRDDS,";
			...
			M_strSQLQRY+= " E.DOT_SBSCD||E.DOT_DORNO||E.DOT_PRDCD NOT IN(SELECT DOT_SBSCD||DOT_DORNO||DOT_PRDCD FROM ";
			M_strSQLQRY+= " MR_DOTAM) and  PT_PRTTP='C'  AND PT_PRTCD=IN_BYRCD AND H.CMT_CGMTP='SYS' AND ";
			M_strSQLQRY+= " H.CMT_CGSTP='MR00SAL' AND H.CMT_CODCD=SUBSTR(E.DOT_SBSCD,3,2) AND ";
			M_strSQLQRY+= " IFNULL(E.DOT_AMDNO,'00') !='00' AND E.DOT_AMDDT BETWEEN '"+L_strFMDAT+"' AND '"+L_strTODAT+"'";
			M_strSQLQRY+= " AND SUBSTR(E.DOT_SBSCD,3,2) IN ('01','03','12') AND G.CMT_CGMTP='SYS' AND G.CMT_CGSTP='MR00ZON' ";
			M_strSQLQRY+= " AND SUBSTR(E.DOT_SBSCD,1,2)=G.CMT_CODCD AND  E.DOT_DORDT != E.DOT_LUPDT ";
...
//			M_strSQLQRY+= " D.CMT_CGSTP='MR00SAL' AND D.CMT_CODCD=IN_SALTP AND IFNULL(IN_AMDNO,'00') !='00' AND A.IN_AMDDT BETWEEN '"+L_strFMDAT+"'";
...
//			}
			...
			M_strSQLQRY+= " A.INT_INDQT DOCQT,A.INT_CFWQT CFWQT,B.INT_INDQT PREDQ,IN_LUSBY LUSBY FROM VW_INTRN A,";
			M_strSQLQRY+= " CO_PTMST,CO_CDTRN C,CO_CDTRN D  INNER JOIN MR_INTAM B ON ";
			M_strSQLQRY+= " A.INT_SBSCD=B.INT_SBSCD AND A.INT_INDNO=B.INT_INDNO  AND A.INT_PRDCD=B.INT_PRDCD AND ";
			M_strSQLQRY+= " Integer(A.INT_AMDNO)-Integer(B.INT_AMDNO)=1  AND A.INT_INDQT!= B.INT_INDQT ";
			M_strSQLQRY+= " WHERE PT_PRTTP='C'  AND PT_PRTCD=IN_BYRCD AND D.CMT_CGMTP='SYS' AND  D.CMT_CGSTP='MR00SAL'";
			M_strSQLQRY+= " AND D.CMT_CODCD=IN_SALTP AND IFNULL(IN_AMDNO,'00') !='00' AND A.IN_AMDDT BETWEEN '"+L_strFMDAT+"'";
			...
			M_strSQLQRY+= " E.INT_PRDDS PRDDS,E.INT_INDQT DOCQT,E.INT_CFWQT CFWQT,0  PREDQ,IN_LUSBY LUSBY FROM ";
			M_strSQLQRY+= " VW_INTRN E,CO_PTMST,CO_CDTRN G,CO_CDTRN H WHERE ";
			M_strSQLQRY+= " E.INT_SBSCD||E.INT_INDNO||E.INT_PRDCD NOT IN(SELECT INT_SBSCD||INT_INDNO||INT_PRDCD FROM ";
			M_strSQLQRY+= " MR_INTAM) and  PT_PRTTP='C'  AND PT_PRTCD=IN_BYRCD AND H.CMT_CGMTP='SYS' AND ";
			M_strSQLQRY+= " H.CMT_CGSTP='MR00SAL' AND H.CMT_CODCD=IN_SALTP AND IFNULL(IN_AMDNO,'00') !='00' AND ";
	...
	*/	
	...
	{  
	    try
	    {
			...
			dosREPORT.writeBytes(padSTRING('R',"Exclusive Trasaction Report between " +M_txtFMDAT.getText().trim()+ " and " + M_txtTODAT.getText().trim(),120));			
