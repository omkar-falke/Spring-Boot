//Tentitive Despatch query / report
import java.awt.*;
...
class mr_qrtds extends cl_rbase implements MouseListener 
{
	...
	private String strRESSTR = cl_dat.M_strREPSTR_pbst+"mr_qrtds.doc"; 
	...
	private String strORDFLD1 = "", strORDFLD2 = "";
	...
	mr_qrtds()
	{
		...
		try
		{
			...
			arrRPTHDR[intTB1_DORNO] = "D.O.No.";
			arrRPTHDR[intTB1_DORDT] = "D.O.Date";
			arrRPTHDR[intTB1_BYRNM] = "Buyer";
			arrRPTHDR[intTB1_TRPNM] = "Transporter";
			arrRPTHDR[intTB1_PRDDS] = "Grade";
			arrRPTHDR[intTB1_PKGDS] = "Pkg.Type";
			arrRPTHDR[intTB1_DSPQT] = "Qty.";
			arrRPTHDR[intTB1_DSPDT] = "Desp.Date";
	...
	public void actionPerformed(ActionEvent L_AE)
	{
		try
		{
			...
			if(M_objSOURC == btnRPTPRINT)
				{
					strRESSTR = cl_dat.M_strREPSTR_pbst+"mr_qrtds.doc"; 
...
private void exePRINT1()
{
	...
	private void dspRPTDTL()
	{
		try
		{
			...
			setORDBY();
			M_strSQLQRY = "select dot_dorno,dot_dordt,in_byrcd,dot_trpcd,dod_dorno,dod_dspdt,dod_prdcd,dod_pkgtp,isnull(dod_dorqt,0)-isnull(dod_ladqt,0) dod_dspqt from mr_dodel,mr_inmst,mr_dotrn  where "+strWHRSTR+" and in_cmpcd=dot_cmpcd and in_mkttp=dot_mkttp and in_indno=dot_indno and dot_cmpcd=dod_cmpcd and dot_mkttp=dod_mkttp and dot_dorno=dod_dorno and dot_prdcd=dod_prdcd and dot_pkgtp=dod_pkgtp "+strORDBY;
			...
			while (true)
			{
				...
				tblRPTRPT.setValueAt(getPRMST(L_strPRDCD,"PR_PRDDS"),i,intTB1_PRDDS);
				tblRPTRPT.setValueAt(getCDTRN("SYSFGXXPKG"+L_strPKGTP,"CMT_SHRDS",hstCDTRN),i,intTB1_PKGDS);
				tblRPTRPT.setValueAt(setNumberFormat(dblORDQT,3),i,intTB1_DSPQT);
				tblRPTRPT.setValueAt(L_strDSPDT,i,intTB1_DSPDT);
				i += 1;
				
				if(!rstRSSET.next())
					break;
			}
			i = chkTOTPRN(i, rstRSSET, true);
		}
	 ...
	 */
	private int chkTOTPRN(int LP_LINCT, ResultSet LP_RSSET, boolean LP_ENDFL)
	{
		try
		{
				...
				if(!(strORDVAL1+strORDVAL2).equals(strORDVAL1_OLD+strORDVAL2_OLD))
				{
					...
					if(intORDCT>1)
					{
						//tblRPTRPT.setValueAt("      Total:",LP_LINCT,intTB1_PKGDS);
						tblRPTRPT.setValueAt(setNumberFormat(dblORDTOT,3),LP_LINCT,intTB1_DSPQT);
						LP_LINCT += 1;
					}
					LP_LINCT += 1;
					dblORDTOT = 0.000; intORDCT = 0;
					if(LP_ENDFL)
					{
						tblRPTRPT.setValueAt("  Grand Total:",LP_LINCT,intTB1_PKGDS);
						tblRPTRPT.setValueAt(setNumberFormat(dblGRNTOT,3),LP_LINCT,intTB1_DSPQT);
					}
				}
		}
		catch (Exception L_EX) {setMSG(L_EX,"chkTOTPRN");}
		return LP_LINCT;
	}
	...
	private void setORDBY()
	{
		try
		{
			...
			if(cmbRPTSQ.getSelectedItem().toString().equals(strDFTSQ))
				{strORDBY = " order by dod_dspdt desc,dod_prdcd asc"; strORDFLD1 = "dod_dspdt"; strORDFLD2 = "dod_prdcd";}
			else if(cmbRPTSQ.getSelectedItem().toString().equals(strPRDSQ))
				{strORDBY = " order by  dod_prdcd,dod_pkgtp "; strORDFLD1 = "dod_prdcd"; strORDFLD2 = "dod_pkgtp";}
			else if(cmbRPTSQ.getSelectedItem().toString().equals(strBYRSQ))
				{strORDBY = " order by  in_byrcd "; strORDFLD1 = "in_byrcd";  strORDFLD2 = "";}
			else if(cmbRPTSQ.getSelectedItem().toString().equals(strPKGSQ))
				{strORDBY = " order by  dod_pkgtp "; strORDFLD1 = "dod_pkgtp";  strORDFLD2 = "";}
			else if(cmbRPTSQ.getSelectedItem().toString().equals(strTRPSQ))
				{strORDBY = " order by  dot_trpcd "; strORDFLD1 = "dot_trpcd";  strORDFLD2 = "";}
			else if(cmbRPTSQ.getSelectedItem().toString().equals(strDDTSQ))
				{strORDBY = " order by  dod_dspdt,dot_trpcd "; strORDFLD1 = "dod_dspdt";  strORDFLD2 = "dot_trpcd";}
		}
		...
		try
		{
			...
			if (cmbRPTWS.getSelectedIndex()>0)
			{
				if(cmbRPTWS.getSelectedItem().toString().equals(strPRDWS))
					...
					{strWHRSTR += " and SUBSTRING(dod_prdcd,1,4) = '"+txtSPCCT.getText()+"'";strSHDRDS = getCDTRN("MSTCOXXPGR"+txtSPCCT.getText(),"CMT_CODDS",hstCDTRN); strSHDRNM = "Prod Category:";}
				else if(cmbRPTWS.getSelectedItem().toString().equals(strSALWS))
					{strWHRSTR += " and dod_dorno in (select dot_dorno from mr_dotrn where DOT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND dot_indno in (select in_indno from mr_inmst where IN_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND in_saltp = '"+txtSPCCT.getText()+"'))";strSHDRDS = getCDTRN("SYSMR00SAL"+txtSPCCT.getText(),"CMT_CODDS",hstCDTRN); strSHDRNM = "Sale Type :";}
	...
	*/	
	private void crtRPTHDR()
	{
		try
		{
			...
			intLINECT = 0;
			O_DOUT.writeBytes(padSTRING('R',cl_dat.M_strCMPNM_pbst,25)+padSTRING('C',"Tentitive Despatch Details "+strSHDRNM+" "+strSHDRDS+" from "+M_txtFMDAT.getText()+ " to "+ M_txtTODAT.getText(),intRPTWD-50)+padSTRING('L',"Page "+intPAGENO,25)+"\n\n"); intLINECT +=2;
			intRUNCL = 0;
	...
	private void crtRPTRPT()
	{
		try
		{
			...
			{
				if(tblRPTRPT.getValueAt(i,intTB1_PKGDS).toString().equals(""))
					if(tblRPTRPT.getValueAt(i+1,intTB1_PKGDS).toString().equals(""))
						if(tblRPTRPT.getValueAt(i+2,intTB1_PKGDS).toString().equals(""))
							if(tblRPTRPT.getValueAt(i+3,intTB1_PKGDS).toString().equals(""))
								break;
				else if(Double.parseDouble(tblRPTRPT.getValueAt(i,intTB1_DSPQT).toString())==0)
						if(Double.parseDouble(tblRPTRPT.getValueAt(i+1,intTB1_DSPQT).toString())==0)
							if(Double.parseDouble(tblRPTRPT.getValueAt(i+2,intTB1_DSPQT).toString())==0)
								if(Double.parseDouble(tblRPTRPT.getValueAt(i+3,intTB1_DSPQT).toString())==0)
									break;
				intRUNCL = 0;
				for(k=1;k<j;k++)
				{
	...
	private String setWHRSTR(String LP_STRDT, String LP_ENDDT)
	{
		...
		try
		{
			...
			L_strRETSTR = "  DOD_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND dod_dspdt between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(LP_STRDT))+"'  and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(LP_ENDDT))+"' and isnull(dod_dorqt,0)-isnull(dod_ladqt,0)>0  and dod_stsfl<>'X' and  dod_mkttp='"+cmbMKTTP.getSelectedItem().toString().substring(0,2)+"'";
	 ...
	 private void crtPTMST()
    {
        ...
        try
        {
            ...
            L_strSQLQRY = "select PT_PRTTP,PT_PRTCD,PT_PRTNM,PT_ZONCD,PT_SHRNM,PT_ADR01,PT_ADR02,PT_ADR03 "
				...
				//+ " union all select PT_PRTTP,PT_PRTCD,PT_PRTNM,PT_ZONCD,PT_SHRNM,PT_ADR01,PT_ADR02,PT_ADR03 "
				//+" from co_ptmst where pt_prttp = 'C' and pt_prtcd in (select distinct in_cnscd from mr_inmst where in_indno in (select dot_indno from mr_dotrn where dot_dorno in (select dod_dorno from mr_dodel where  "+strWHRSTR+")))"
				//+ " union all select PT_PRTTP,PT_PRTCD,PT_PRTNM,PT_ZONCD,PT_SHRNM,PT_ADR01,PT_ADR02,PT_ADR03 "
				//+" from co_ptmst where pt_prttp = 'D' and pt_prtcd in  (select distinct in_dsrcd from mr_inmst where in_indno in (select dot_indno from mr_dotrn where dot_dorno in (select dod_dorno from mr_dodel where  "+strWHRSTR+")))"
