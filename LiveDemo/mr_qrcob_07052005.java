...
class mr_qrcob extends cl_rbase
{
	...
	private Hashtable hstDLYTOT,hstMONTOT,hstCRFTOT;/** Report order set by user*/
	...
	private final int	intMKTTP_fn=1,
						...
						intPRDCD_fn=13,
						intCNSNM_fn=14,
						intDTPCD_fn=15;/** Display total zone-wise*/
	...
	private Vector vtrQRDAT;/**for sort order list items*/
	private Vector vtrODRBY;/**for sort order*/
	...
	void exePRINT()
	{
		...
		try
		{
			...
			{//FIRING QUERY TO DB DURING FIRST FORMATTING
				M_strSQLQRY="select IN_MKTTP ,IN_INDNO,a.CMT_CODDS,IN_BYRCD,c.PT_PRTNM CMT_BYRNM,IN_DSRCD,d.PT_SHRNM,INT_PRDDS,INT_INDQT,b.cmt_codds,INT_BASRT,INT_CDCVL/1000 INT_CDCVL,INT_DDCVL/1000 INT_DDCVL,INT_TDCVL/1000 INT_TDCVL, IN_APTVL ,e.PT_PRTNM PT_CNSNM,IN_CPTVL,IN_DTPCD,INT_PRDCD from MR_INMST,MR_INTRN,CO_CDTRN a,CO_CDTRN b,cO_PTMST c,CO_PTMST d, CO_PTMST e where IN_SBSCD in ("+L_strSBSCD+") and IN_MKTTP=INT_MKTTP and IN_INDNO=INT_INDNO and IN_BKGDT <= '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and IN_BKGDT >= '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"'"
					+" and IN_SALTP=a.CMT_CODCD and a.CMT_CGSTP='MR00SAL'"
					+" and IN_ZONCD=b.CMT_CODCD and b.CMT_CGSTP='MR00ZON'"
					+" and c.PT_PRTTP='C' and IN_BYRCD=c.PT_PRTCD"
					+" and d.PT_PRTTP='D' and IN_DSRCD=d.PT_PRTCD"
					+" and e.PT_PRTTP='C' and IN_CNSCD=e.PT_PRTCD"
					+" and in_stsfl<>'X' and int_stsfl<>'X' and IN_SBSCD in "+M_strSBSLS+" and INT_SBSCD in "+M_strSBSLS+" and IN_MKTTP in "+strMKTTP_LST;
				M_rstRSSET=cl_dat.exeSQLQRY(M_strSQLQRY);
			...
			//Digits are attached b4 zone to set order in desired manner. (i.e. region wise)
					...
					L_staTEMP[12]=L_strCPTVL+"  "+L_strAPTVL+" ";
			//		L_staTEMP[12]=nvlSTRVL(M_rstRSSET.getString("IN_CPTVL")," ")+"/"+nvlSTRVL(M_rstRSSET.getString("IN_APTVL")," ")+" ";
					L_staTEMP[13]=nvlSTRVL(M_rstRSSET.getString("PT_CNSNM")," ");
					L_staTEMP[14]=nvlSTRVL(M_rstRSSET.getString("IN_DTPCD")," ");
					L_staTEMP[15]=nvlSTRVL(M_rstRSSET.getString("INT_PRDCD")," ");
					vtrQRDAT.addElement(L_staTEMP);//Vector to hold total, unsorted data
				}
				M_rstRSSET.close();
			}
			Vector L_vtrORDR1=new Vector(10,5);//Vector to hold filtered data to be sorted
				...
				if(chkSPCOL(L_staTEMP))//Validates filteration rules applied by user
				{
					if(L_intORDR1!=-1)
					{
						if(rdbDAILY.isSelected())
							L_vtrORDR1.addElement(L_staTEMP[L_intORDR1-1]+"|"+L_staTEMP[L_intORDR2-1]+"|"+L_staTEMP[L_intORDR3-1]+"|"+L_staTEMP[intBYRCD_fn-1]+"|"+L_staTEMP[L_intORDR4-1]+"|"+L_staTEMP[intINDNO_fn-1]+"|"+L_staTEMP[intINDQT_fn-1]+"|"+L_staTEMP[intBASRT_fn]+"|"+L_staTEMP[intCDCVL_fn]+"|"+L_staTEMP[intDDCVL_fn]+"|"+L_staTEMP[intTDCVL_fn]+"|"+L_staTEMP[intAPTVL_fn]+"|"+L_staTEMP[intPRDCD_fn]+"|"+L_staTEMP[intCNSNM_fn]+"|"+L_staTEMP[intDTPCD_fn]);
						else
							L_vtrORDR1.addElement(L_staTEMP[L_intORDR1-1]+"|"+L_staTEMP[L_intORDR2-1]+"|"+L_staTEMP[L_intORDR3-1]+"|"+L_staTEMP[L_intORDR4-1]+"|"+L_staTEMP[intBYRCD_fn-1]+"|"+L_staTEMP[intINDNO_fn-1]+"|"+L_staTEMP[intINDQT_fn-1]+"|"+L_staTEMP[intBASRT_fn]+"|"+L_staTEMP[intCDCVL_fn]+"|"+L_staTEMP[intDDCVL_fn]+"|"+L_staTEMP[intTDCVL_fn]+"|"+L_staTEMP[intAPTVL_fn]+"|"+L_staTEMP[intPRDCD_fn]+"|"+L_staTEMP[intCNSNM_fn]+"|"+L_staTEMP[intDTPCD_fn]);
					}
				}
			}
	...
	private void exePRNRPT()
	{
		try
		{
			...
			if(M_rdbTEXT.isSelected())
				filREPORT=new File("c:\\reports\\mr_qrcob.doc");
			else
			{
				filREPORT=new File("c:\\reports\\mr_qrcob.html");
			}
			...
			for(int i=0;i<tblREPORT.getRowCount();i++)
			{
				if(!L_strCNSNM.equals(""))
				{
					if(L_strCNSNM.length()>15)
						L_strCNSNM=L_strCNSNM.substring(0,14);
				...
				{//DISPLAY COL - 1 VALUE IN SEPARATE LINE
					...
					{//CHECK IF IT IS NOT TOTALISING ROW
						if(!(tblREPORT.getValueAt(i,1).toString().substring(0,5).equalsIgnoreCase("TOTAL")||tblREPORT.getValueAt(i,1).toString().substring(0,5).equalsIgnoreCase("GRAND")))//||tblREPORT.getValueAt(i,1).toString().length()<5)
						{
				...
				for(int j=1;j<tblREPORT.getColumnCount()&&L_flgPRNLN;j++)
				{
					if(j==1)
					{
						...
						{//DISPLAY COL - 1 ONLY IF TOTAL IS TO BE PRINTED
							if(!(tblREPORT.getValueAt(i,1).toString().substring(0,5).equalsIgnoreCase("TOTAL")||tblREPORT.getValueAt(i,1).toString().substring(0,5).equalsIgnoreCase("GRAND")))
								continue;
						}
						else
							continue;
...
*/								int L_intPADLN=0;
							...
							else
							{
								...
								{
									L_strTEMP="   "+L_strTEMP.substring(0,L_strTEMP.length()-3);
									L_strSEPRTR="\n";
								}
								M_intLINNO++;
							}
						...
						else 
						{
							...
							if(L_strTEMP.length()>19)
								L_strTEMP=L_strTEMP.substring(0,18);
							try
							{
								if(L_strTEMP.equals("  -"))
...
//						M_intLINNO++;
			...
			if(rdbDAILY.isSelected())
			{
				hstDLYTOT=new Hashtable(20,0.2f);//FOR DAILY TOTAL VALUES
				getDAILY(hstDLYTOT,obaRPDAT);
				//RETRIEVING MONTHLY TOTALS
				M_strSQLQRY="select IN_MKTTP ,IN_INDNO,a.CMT_CODDS,IN_BYRCD,c.PT_PRTNM CMT_BYRNM,IN_DSRCD,d.PT_PRTNM,INT_PRDDS,INT_INDQT,b.cmt_codds,INT_BASRT,INT_CDCVL/1000 INT_CDCVL,INT_DDCVL/1000 INT_DDCVL,INT_TDCVL/1000 INT_TDCVL, IN_APTVL, e.PT_PRTNM PT_CNSNM, INT_PRDCD from MR_INMST,MR_INTRN,CO_CDTRN a,CO_CDTRN b,cO_PTMST c,CO_PTMST d,CO_PTMST e where IN_MKTTP=INT_MKTTP and IN_INDNO=INT_INDNO and IN_BKGDT <= '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and IN_BKGDT >= '"+M_fmtDBDAT.format(M_fmtLCDAT.parse("01"+M_txtFMDAT.getText().substring(2)))+"'"
					+" and IN_SALTP=a.CMT_CODCD and a.CMT_CGSTP='MR00SAL'"
					+" and IN_ZONCD=b.CMT_CODCD and b.CMT_CGSTP='MR00ZON'"
					+" and c.PT_PRTTP='C' and IN_BYRCD=c.PT_PRTCD"
					+" and d.PT_PRTTP='D' and IN_DSRCD=d.PT_PRTCD"
					+" and e.PT_PRTTP='C' and IN_CNSCD=e.PT_PRTCD"
					+" and in_stsfl<>'X' and int_stsfl<>'X' and IN_SBSCD in "+M_strSBSLS+" and INT_SBSCD in "+M_strSBSLS;
				ResultSet L_rstRSSET=cl_dat.exeSQLQRY1(M_strSQLQRY);
				Vector vtrQRDAT=new Vector(100,10);
				...
				while(L_rstRSSET.next())
				{
					...
					L_staTEMP[13]=nvlSTRVL(L_rstRSSET.getString("INT_PRDCD")," ");
					L_staTEMP[14]=nvlSTRVL(L_rstRSSET.getString("PT_CNSNM")," ");
					if (chkSPCOL(L_staTEMP))
						vtrQRDAT.addElement(L_staTEMP[L_intORDR1-1]+"|"+L_staTEMP[L_intORDR2-1]+"|"+L_staTEMP[L_intORDR3-1]+"|"+L_staTEMP[intBYRCD_fn-1]+"|"+L_staTEMP[L_intORDR4-1]+"|"+L_staTEMP[intINDNO_fn-1]+"|"+L_staTEMP[intINDQT_fn-1]+"|"+L_staTEMP[intBASRT_fn]+"|"+L_staTEMP[intCDCVL_fn]+"|"+L_staTEMP[intDDCVL_fn]+"|"+L_staTEMP[intTDCVL_fn]+"|"+L_staTEMP[intAPTVL_fn]+"|"+L_staTEMP[intPRDCD_fn]);
				}
				hstMONTOT=new Hashtable(20,0.2f);
				...
				M_strSQLQRY="select int_mkttp,int_indno,a.cmt_codds,in_byrcd,c.pt_prtnm,in_dsrcd,d.pt_prtnm,int_prdds,int_cfwqt,b.cmt_codds,int_basrt,int_cdcvl,int_ddcvl,int_tdcvl,in_aptvl,int_prdcd from mr_intrn,mr_inmst,co_cdtrn a,co_cdtrn b,co_ptmst c,co_ptmst d    where "
					+"  int_indno=in_indno and int_cfwqt>0 and  IN_SBSCD in "+M_strSBSLS+" and INT_SBSCD in "+M_strSBSLS+" and a.cmt_cgstp='MR00SAL' and a.cmt_codcd=in_saltp "
					+"  and b.cmt_cgstp='MR00ZON' and b.cmt_codcd=in_zoncd and c.pt_prttp='C' and c.pt_prtcd=in_byrcd and d.pt_prttp='D' and d.pt_prtcd=in_dsrcd ";
			...
			System.out.println(M_strSQLQRY);
				...
				L_rstRSSET.close();
				strHEADNG="";
				hstCRFTOT=new Hashtable(20,0.2f);
	...
	//NONACOUNTABLE
			...
			L_doutREPORT.flush();
			L_doutREPORT.close();
			doPRINT("c:\\reports\\mr_qrcob.doc");
		}catch(Exception e)
...
//		prnFMTCHR(L_doutREPORT,M_strBOLD);
	...
	private void getDAILY(Hashtable P_hstRPDAT,Object[] P_obaRPDAT)
	{
		try
		{
			...
			//System.out.println("004");
				...
				for(int i=0;i<L_staDATA.length;i++)
				{
					...
					if(L_staDATA[i][0].substring(1).equalsIgnoreCase("Export"))
					{
						L_dblTOTAL2+=L_dblODRQT;
						if(L_staDATA[i][intPRDCD_fn-1].substring(0,4).equalsIgnoreCase("5111"))
							L_dblXPTGP+=L_dblODRQT;
						else if(L_staDATA[i][intPRDCD_fn-1].substring(0,4).equalsIgnoreCase("5112"))
							L_dblXPTHI+=L_dblODRQT;
						else if(L_staDATA[i][intPRDCD_fn-1].substring(0,2).equalsIgnoreCase("51"))
							L_dblXPTOT+=L_dblODRQT;
						else if(L_staDATA[i][intPRDCD_fn-1].substring(0,2).equalsIgnoreCase("52"))
							L_dblXPTSP+=L_dblODRQT;
						else if(L_staDATA[i][intPRDCD_fn-1].substring(0,2).equalsIgnoreCase("53"))
							L_dblXPTWP+=L_dblODRQT;
						else if(L_staDATA[i][intPRDCD_fn-1].substring(0,2).equalsIgnoreCase("54"))
					...
					else if(L_staDATA[i][1].toString().equalsIgnoreCase("Captive Consumption")||L_staDATA[i][0].toString().equalsIgnoreCase("Free Test Sample"))
					{
						...
						if(L_staDATA[i][0].substring(1).equalsIgnoreCase("North"))
						{
							L_dblNORTH+=L_dblODRQT;
							L_dblTOTNESNAC+=L_dblODRQT;
						   if(L_staDATA[i][intPRDCD_fn-1].substring(0,2).equalsIgnoreCase("52"))
							{
						   ...
						   else if(L_staDATA[i][intPRDCD_fn-1].substring(0,4).equalsIgnoreCase("5111"))
							{
						...
						else if(L_staDATA[i][0].substring(1).equalsIgnoreCase("West"))
						{
						...
						else if(L_staDATA[i][0].substring(1).equalsIgnoreCase("South"))
						{
						...
						else if(L_staDATA[i][0].substring(1).equalsIgnoreCase("EAST"))
						{
							L_dblTOTNESNAC+=L_dblODRQT;
						...
						else if(L_staDATA[i][0].substring(1).equalsIgnoreCase("CENTRAL"))
						{
							L_dblTOTWCNAC+=L_dblODRQT;
