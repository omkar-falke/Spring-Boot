...
while END_TABLE = 0 DO 
   ...
   if L_CPTVL > 0 then
    ...
    update spldata.MR_INTRN set INT_INDVL = L_IND_VAL, INT_CCLVL = L_CCL_VAL, INT_COMVL = L_COM_VAL,INT_CRDVL = L_CRD_VAL, INT_RSNVL = (L_IND_VAL-(L_CCL_VAL+L_COM_VAL+L_CRD_VAL))  where INT_MKTTP = L_MKTTP and INT_INDNO = L_INDNO and INT_PRDCD =   L_PRDCD and INT_PKGTP = L_PKGTP; 
...
call spldata.setRLSN_IND('01','12/01/2004', '12/25/2004');
commit;
...
select * from spldata.mr_intrn where int_lupdt=current_date;


select * from spldata.mm_gptrn where gp_lupdt=current_date;
select * from spldata.mm_gptrn where gp_mgpno in ('85200040','85200041');

select * from spldata.co_ctmst where upper(ct_matds) like '%INK CARTRIDGE%' 
