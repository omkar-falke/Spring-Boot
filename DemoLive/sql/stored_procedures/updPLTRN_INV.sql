...
--        Party Groutemp Code (Picked up from Party Master (CO_PTMST) for the specfied Buyer.
...
drop procedure spldata.updPLTRN_INV;
commit;

create procedure spldata.updPLTRN_INV(IN LP_CMPCD char(2),IN LP_MKTTP char(2), IN LP_INVNO char(8))  language sql  modifies sql data 
P1:
begin
  ...
  declare C_INV cursor for  select  distinct IVT_BYRCD, IVT_CNSCD,IVT_SALTP,IVT_INVNO, date(IVT_INVDT) IVT_INVDT,IVT_PMDDT, IVT_SBSCD, isnull(ivt_invvl,0)  ivt_invvl from spldata.mr_ivtrn   where   IVT_MKTTP = LP_MKTTP and IVT_INVNO=LP_INVNO and IVT_INVQT > 0 and IVT_STSFL <> 'X' and length(IVT_INVNO)=8 and IVT_CMPCD=LP_CMPCD;

  declare continue handler for not found 
       set END_TABLE = 1; 

  open C_INV ;
  fetch C_INV into  L_BYRCD, L_CNSCD, L_SALTP,L_DOCNO, L_DOCDT,L_DUEDT, L_SBSCD, L_DOCVL;
  set L_PRTTP = 'C';
  ...
  select isnull(PL_SEQNO,0) PL_SEQNO into L_SEQNO from spldata.MR_PLTRN where PL_CMPCD = LP_CMPCD and PL_PRTTP = L_PRTTP and PL_PRTCD = L_PRTCD and PL_DOCTP = '21' and PL_DOCNO = LP_INVNO;
  ...
  if L_SEQNO > 0 then 
      ...
      --insert into spldata.temp_chk values (LP_CMPCD + '  ' + L_PRTTP + '  ' + L_PRTCD + '  ' + L_DOCTP + '  ' +  LP_INVNO + '  ' + char(L_SEQNO));
  ...
  if L_SEQNO =0 then 
    select max(isnull(PL_SEQNO,0))  into L_SEQNO from spldata.MR_PLTRN where PL_CMPCD = LP_CMPCD and PL_PRTTP = L_PRTTP and PL_PRTCD = L_PRTCD;
     ...
     if L_SEQNO > 0 then
         ...
         select  isnull(PT_YOPVL,0), isnull(PT_YOPFL,'DB')  into  L_BALVL, L_BALFL from spldata.CO_PTMST where PT_PRTTP = L_PRTTP and PT_PRTCD = L_PRTCD;
     end if;
   ...
   insert into spldata.mr_pltrn (PL_CMPCD,PL_PRTTP,PL_PRTCD,PL_CNSCD,PL_DOCTP,PL_DOCNO,PL_SEQNO,PL_DOCDT,PL_DUEDT,PL_DOCVL,PL_BALVL,PL_BALFL,PL_ADJVL,PL_ACCRF,PL_ACCDT,PL_MKTTP,PL_GRPCD,PL_SBSCD,PL_TRNFL,PL_STSFL,PL_LUSBY,PL_LUPDT) values  (LP_CMPCD,L_PRTTP,L_PRTCD,L_CNSCD1,L_DOCTP,L_DOCNO,L_SEQNO+1,L_DOCDT,L_DUEDT,L_DOCVL,L_BALVL1,L_BALFL1,L_ADJVL,L_ACCRF,L_ACCDT,LP_MKTTP,L_GRPCD,L_SBSCD,L_TRNFL,L_STSFL,L_LUSBY,L_LUPDT);
...
call spldata.updPLTRN_INV('11','01','00000170');
commit;
;

select * from spldata.mr_IVtrn where IVT_INVno = '90004463';
commit;
select * from spldata.mr_pltrn where pl_docno='90007536';
