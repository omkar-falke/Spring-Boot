...
drop procedure SPLDATA.rwkPLTRN_TRN;
commit;

create procedure SPLDATA.rwkPLTRN_TRN(IN LP_CMPCD char(2), IN LP_PRTTP char(2), IN LP_PRTCD char(5), IN LP_DOCTP char(2), IN LP_DOCNO char(8), IN LP_SEQNO  decimal(10))  language sql  modifies sql data
P1:
begin
  ...
  declare C_PLD cursor for  select  PL_SEQNO, PL_DOCTP, PL_DOCVL from  SPLDATA.MR_PLTRN  where   PL_CMPCD = LP_CMPCD and PL_PRTTP = LP_PRTTP and PL_PRTCD = LP_PRTCD and PL_SEQNO >= LP_SEQNO order by PL_SEQNO;
...
if LP_SEQNO>1 then
         select isnull(PL_BALVL,0) PL_BALVL, isnull(PL_BALFL,'CR') PL_BALFL  into L_TRNVL_OLD, L_TRNFL_OLD  from SPLDATA.MR_PLTRN  where PL_PRTTP = LP_PRTTP and PL_PRTCD = LP_PRTCD and PL_SEQNO = (LP_SEQNO-1);
    else
...
call SPLDATA.rwkPLTRN_TRN('01',  'C',  'K0131',  '21',  '70004187',  4);
commit;
