...
class fg_rpsts extends cl_rbase
{			
	...
	private boolean flgXPSFL = false;
	int j=0;
	int i=0;
	
	
	String LM_PRDTP,strPRSGR,LM_SUBPD,strCCSVL,strRESNO;
    String strPRDCD,strPKGTP,strPRMGR,LM_CLSQT,strSALRT,strSTSFL;
	...
	{ 
		try
		{
			...
			M_strSQLQRY += " AND CMT_CGSTP = 'FGXXLGR' ORDER BY CMT_CODCD";
	...
	public void exePRINT()
	{
		...
		try
		{
			...
			if(M_rdbHTML.isSelected())
		        strFILNM = cl_dat.M_strREPSTR_pbst +"fg_rpsts.html";
		    else if(M_rdbTEXT.isSelected())
		        strFILNM = cl_dat.M_strREPSTR_pbst + "fg_rpsts.doc";
			getDATA();
	...
	private void getDATA()
	{
		...
		try
		{
            ...
            String L_strDBGRP = " SUBSTRING(ST_PRDCD,1,2),SUBSTRING(ST_PRDCD,1,4),PR_STSFL,ST_PKGTP,ST_PRDCD,PR_PRDDS,CMT_CCSVL,ST_STSFL,ST_RESNO";
            String L_strDBORD = " LM_PRDTP,LM_SUBPD,PR_STSFL,ST_PKGTP,ST_PRDCD,CMT_CCSVL,ST_STSFL,ST_RESNO";
		//	String L_GRPSTR = getGRPSTR("SP",L_DTBSTR,L_DBSSTR);
				...
				while((strPRMGR).equals(strPRMGR1) && !LM_EOF)
				{
					...
					while((strPRMGR+strPRSGR).equals(strPRMGR1+strPRSGR1) && !LM_EOF)
					{
						...
						//	prnGRPHDR("PK",12);
							...
							strSTSFL = strSTSFL2;
							strSTSFL1 = strSTSFL;
                            while((strPRMGR+strPRSGR+strSTSFL+strPKGTP).equals(strPRMGR1+strPRSGR1+strSTSFL1+strPKGTP1) && !LM_EOF)
							{
								...
								strSTSFL = strSTSFL2;
								strSTSFL1 =strSTSFL;
                                while((strPRMGR+strPRSGR+strSTSFL+strPKGTP+strPRDCD).equals(strPRMGR1+strPRSGR1+strSTSFL1+strPKGTP1+strPRDCD1) && !LM_EOF)
								{
									...
									if(hstRESQT.containsKey(strPRDCD))
									{
									   ...
									   strRESPRQT = setNumberFormat((Double.parseDouble(hstRESQT.get(strPRDCD).toString()) > dblDSTQT ? dblDSTQT : Double.parseDouble(hstRESQT.get(strPRDCD).toString())),3);
									}
								...
								strQLHPRQT = "0";
								if(hstQLHQT.containsKey(strPRMGR1+strPRSGR1+strSTSFL1+strPKGTP1+strPRDCD1))
								   strQLHPRQT = hstQLHQT.get(strPRMGR1+strPRSGR1+strSTSFL1+strPKGTP1+strPRDCD1).toString();
								if (Double.parseDouble(strQLHPRQT) > 0)
								{
									strQLHPKQT = setNumberFormat(Double.parseDouble(strQLHPKQT)+Double.parseDouble(strQLHPRQT),3);
									strQLHSGQT = setNumberFormat(Double.parseDouble(strQLHSGQT)+Double.parseDouble(strQLHPRQT),3);
									strQLHMGQT = setNumberFormat(Double.parseDouble(strQLHMGQT)+Double.parseDouble(strQLHPRQT),3);
									strQLHSTQT = setNumberFormat(Double.parseDouble(strQLHSTQT)+Double.parseDouble(strQLHPRQT),3);
									strQLHGTQT = setNumberFormat(Double.parseDouble(strQLHGTQT)+Double.parseDouble(strQLHPRQT),3);
								}
								if (Double.parseDouble(strRESPRQT) > 0)
								{
									strRESPKQT = setNumberFormat(Double.parseDouble(strRESPKQT)+Double.parseDouble(strRESPRQT),3);
									strRESSGQT = setNumberFormat(Double.parseDouble(strRESSGQT)+Double.parseDouble(strRESPRQT),3);
									strRESMGQT = setNumberFormat(Double.parseDouble(strRESMGQT)+Double.parseDouble(strRESPRQT),3);
									strRESSTQT = setNumberFormat(Double.parseDouble(strRESSTQT)+Double.parseDouble(strRESPRQT),3);
									strRESGTQT = setNumberFormat(Double.parseDouble(strRESGTQT)+Double.parseDouble(strRESPRQT),3);
								}
								prnGRPTOT("PR",strSLRPRQT,strTPRQT,strRESPRQT,strQLHPRQT,"N");
								intGRPTOT("PR");
							}
							prnGRPTOT("PK",strSLRPKQT,strTPKQT,strRESPKQT,strQLHPKQT,"B");
							intGRPTOT("PK");
						}
						prnGRPTOT("ST",strSLRSTQT,strTSTQT,strRESSTQT,strQLHSTQT,"B");
						intGRPTOT("ST");
					}
					prnGRPTOT("SG",strSLRSGQT,strTSGQT,strRESSGQT,strQLHSGQT,"B");
					intGRPTOT("SG");
				}
				prnGRPTOT("MG",strSLRMGQT,strTMGQT,strRESMGQT,strQLHMGQT,"B");
				intGRPTOT("MG");
			}
			if(M_rstRSSET != null)
				M_rstRSSET.close();
			...
			dosREPORT.writeBytes("\n");
			cl_dat.M_intLINNO_pbst +=1;
			prnGRPTOT("GT",strSLRGTQT,strTGTQT,strRESGTQT,strQLHGTQT,"B");
			dosREPORT.writeBytes("\n");
			cl_dat.M_intLINNO_pbst +=1;
	...
	private void prnGRPTOT(String LP_GRPCT,String LP_SLRXXQT,String LP_TOTXXQT,String LP_RESXXQT,String LP_QLHXXQT,String LP_BLDFL)
	{
		try
		{
			/*if((cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPPRN_pbst)) &&(M_rdbTEXT.isSelected()))
			...
			if (LP_GRPCT.equals("PR"))
			{
				...
				dosREPORT.writeBytes(padSTRING('R',strPRDDS1,18));
			...
			else if (LP_GRPCT.equals("PK"))
			{
                ...
                dosREPORT.writeBytes(padSTRING('R',"",10));
                dosREPORT.writeBytes(padSTRING('R',"TOTAL " + hstPKGTP.get(strPKGTP1).toString(),20));
				for(int l=intFIRST;l<=intLAST;l++)
				{
					L_strPRT="0"+l;
				//	System.out.println( "L_strPRT= "+L_strPRT);
			...
			//	L_GRPDS = cl_dat.getPRMCOD("cmt_codds","STS","MRXXSTS",strSTSFL1);
                dosREPORT.writeBytes(padSTRING('R',"",8));
                dosREPORT.writeBytes(padSTRING('R',"TOTAL " +hstRCTTP.get(strSTSFL1).toString(),22));
				for(int l=intFIRST;l<=intLAST;l++)
				{
					L_strPRT="0"+l;
				//	System.out.println( "L_strPRT= "+L_strPRT);
			...
			//	L_GRPDS = cl_cust.ocl_cust.getSBPRD(strPRSGR1);
				dosREPORT.writeBytes(padSTRING('R',"",6));
				dosREPORT.writeBytes(padSTRING('R',"TOTAL " + hstSBGRP.get(strPRSGR1).toString(),24));
				for(int l=intFIRST;l<=intLAST;l++)
				{
					L_strPRT="0"+l;
				//	System.out.println( "L_strPRT= "+L_strPRT);
			...
			else if (LP_GRPCT.equals("MG"))
			{
				...
				dosREPORT.writeBytes(padSTRING('R',"",2));
				dosREPORT.writeBytes(padSTRING('R',"TOTAL " + hstMNGRP.get(strPRMGR1).toString(),28));
				for(int l=intFIRST;l<=intLAST;l++)
				{
					L_strPRT="0"+l;
					//System.out.println( "L_strPRT= "+L_strPRT);
			...
			else if (LP_GRPCT.equals("GT"))
			{
				...
				cl_dat.M_intLINNO_pbst+=1;
				dosREPORT.writeBytes(padSTRING('R',"",2));
				dosREPORT.writeBytes(padSTRING('R',"GRAND TOTAL",28));
				for(int l=intFIRST;l<=intLAST;l++)
				{
					L_strPRT="0"+l;
				//	System.out.println( "L_strPRT= "+L_strPRT);
	...
	*/	
	...
	{  
	    try
	    {
			...
			dosREPORT.writeBytes(padSTRING('L',"Tot.Qty.",12));
