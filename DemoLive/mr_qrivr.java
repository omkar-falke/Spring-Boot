...
class mr_qrgrr extends cl_rbase implements MouseListener 
{
	...
	private String strRESSTR = cl_dat.M_strREPSTR_pbst+"mr_qrivr.doc"; 
	...
	private boolean flgIVT_RLSN;	// Flag for Invoice Based Realisation
	...
	public void actionPerformed(ActionEvent L_AE)
	{
		try
		{
			...
			if(M_objSOURC == btnPRINT_CNS)
				{
					strRESSTR = cl_dat.M_strREPSTR_pbst+"mr_csivr.doc"; 
			...
			if(M_objSOURC == btnPRINT_GRD)
				{
					strRESSTR = cl_dat.M_strREPSTR_pbst+"mr_grivr.doc"; 
					this.getParent().getParent().setCursor(cl_dat.M_curWTSTS_pbst);	this.setCursor(cl_dat.M_curDFSTS_pbst);
					crtGRDRPT();
					exePRINT1();
				}
			if(M_objSOURC == btnPRINT_IND)
				{
					strRESSTR = cl_dat.M_strREPSTR_pbst+"mr_inivr.doc"; 
					this.getParent().getParent().setCursor(cl_dat.M_curWTSTS_pbst);	this.setCursor(cl_dat.M_curDFSTS_pbst);
					crtINDRPT();
					exePRINT1();
				}
	...
	private boolean chkCPMST_EXIST()
	{
		try
		{
			...
			String L_strWHRSTR =  "CP_MKTTP = '" +cmbMKTTP.getSelectedItem().toString().substring(0,2)+"' and "
	...
	private void exeSAVE_CPR1(String LP_PRDCD, String LP_CPRVL)
	{
		try
		{
			...
			strWHRSTR =  "CP_MKTTP = '" +cmbMKTTP.getSelectedItem().toString().substring(0,2)+"' and "
...
private void exePRINT1()
{
		...
		if(L_KE.getKeyCode() == L_KE.VK_F1)
			{
				...
				if(cmbRPTWS.getSelectedItem().toString().equals(strGRDWS))
					{M_strHLPFLD = "txtGRD"; cl_hlp("Select PR_PRDCD, PR_PRDDS from co_prmst where substr(pr_prdcd,1,2) in ('51','52','53','54') and pr_prdcd in (select distinct "+(flgIVT_RLSN ? " ivt_prdcd from MR_IVTRN " : " int_prdcd from vw_intrn")+" where "+setWHRSTR(M_txtFMDAT.getText(),M_txtTODAT.getText())+") order by PR_PRDCD" ,2,1,new String[] {"Code","Description"},2,"CT");}
				...
				if(cmbRPTWS.getSelectedItem().toString().equals(strPRDWS))
					{M_strHLPFLD = "txtPRD"; cl_hlp("Select substr(CMT_CODCD,1,4) CMT_CODCD, CMT_CODDS from CO_CDTRN where cmt_cgmtp='MST' and cmt_cgstp='COXXPGR' and substr(cmt_codcd,1,4) in (select distinct "+(flgIVT_RLSN ? " substr(ivt_prdcd,1,4) int_prdcd from MR_IVTRN " : " substr(int_prdcd,1,4) int_prdcd from vw_intrn")+" where "+setWHRSTR(M_txtFMDAT.getText(),M_txtTODAT.getText())+")  order by CMT_CODCD" ,2,1,new String[] {"Code","Description"},2,"CT");}
	...
	private void dspCNSDTL()
	{
		try
		{
			...
			if (flgIVT_RLSN)
				M_strSQLQRY = "select ivt_cnscd in_cnscd,sum(ifnull(ivt_invqt,0)) int_indqt from mr_ivtrn where "+strWHRSTR+" group by in_cnscd ";
			else
				M_strSQLQRY = "select in_cnscd,sum(ifnull(int_indqt,0)) int_indqt from vw_intrn where "+strWHRSTR+" group by in_cnscd ";
			...
			if (flgIVT_RLSN)
				M_strSQLQRY = "select ivt_cnscd in_cnscd,ifnull(ivt_invqt,0) int_indqt, ifnull(ivt_cprvl,0) int_cprvl,int(round((ifnull(ivt_rsnvl,0)-ifnull(ivt_crdvl,0))/ifnull(ivt_invqt,1),0)) int_rsnvl from mr_ivtrn where "+strWHRSTR+" order by in_cnscd";
			else
				M_strSQLQRY = "select in_cnscd,ifnull(int_indqt,0) int_indqt, ifnull(int_cprvl,0) int_cprvl,int(round((ifnull(int_rsnvl,0)-ifnull(int_crdvl,0))/ifnull(int_indqt,1),0)) int_rsnvl from vw_intrn where "+strWHRSTR+" order by in_cnscd";
			rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			if(!rstRSSET.next() || rstRSSET==null)
				return;
	...
	private void dspGRDDTL()
	{
		try
		{
			...
			ResultSet rstRSSET = null;
			if (flgIVT_RLSN)
				M_strSQLQRY = "select ivt_prdcd,sum(ifnull(ivt_invqt,0)) int_indqt from mr_ivtrn where "+strWHRSTR+" group by int_prdcd ";
			else
				M_strSQLQRY = "select int_prdcd,sum(ifnull(int_indqt,0)) int_indqt from vw_intrn where "+strWHRSTR+" group by int_prdcd ";
			//System.out.println(M_strSQLQRY);
			rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			if(!rstRSSET.next() || rstRSSET==null)
				return;
			...
			if (flgIVT_RLSN)
				M_strSQLQRY = "select ivt_prdcd,ifnull(ivt_invqt,0) int_indqt,int(round((ifnull(ivt_rsnvl,0)-ifnull(ivt_crdvl,0))/ifnull(ivt_invqt,1),0)) int_rsnvl, ifnull(ivt_cprvl,0) int_cprvl from mr_ivtrn where "+strWHRSTR+" order by int_prdcd";
			else
				M_strSQLQRY = "select int_prdcd,ifnull(int_indqt,0) int_indqt,int(round((ifnull(int_rsnvl,0)-ifnull(int_crdvl,0))/ifnull(int_indqt,1),0)) int_rsnvl, ifnull(int_cprvl,0) int_cprvl from vw_intrn where "+strWHRSTR+" order by int_prdcd";
	...
	private void dspINDDTL()
	{
		try
		{
			...
			if(flgIVT_RLSN)
				M_strSQLQRY = "select ivt_prdcd int_prdcd,ivt_prdds int_prdds,ivt_invno int_indno,ifnull(ivt_invqt,0) int_indqt,int(round((ifnull(ivt_rsnvl,0)-ifnull(ivt_crdvl,0))/ifnull(ivt_invqt,1),0)) int_rsnvl, ifnull(ivt_cprvl,0) int_cprvl, int(round(ifnull(ivt_invvl,0)/ifnull(ivt_invqt,1),0)) int_indvl, int(round(ifnull(ivt_cclvl,0)/ifnull(ivt_invqt,1),0)) int_cclvl,  int(round(ifnull(ivt_comvl,0)/ifnull(ivt_invqt,1),0)) int_comvl,  int(round(ifnull(ivt_crdvl,0)/ifnull(ivt_invqt,1),0)) int_crdvl, ivt_cnscd in_cnscd from mr_ivtrn where "+strWHRSTR+"  order by int_prdcd";
			else
				M_strSQLQRY = "select int_prdcd,int_prdds,int_indno,ifnull(int_indqt,0) int_indqt,int(round((ifnull(int_rsnvl,0)-ifnull(int_crdvl,0))/ifnull(int_indqt,1),0)) int_rsnvl, ifnull(int_cprvl,0) int_cprvl, int(round(ifnull(int_indvl,0)/ifnull(int_indqt,1),0)) int_indvl, int(round(ifnull(int_cclvl,0)/ifnull(int_indqt,1),0)) int_cclvl,  int(round(ifnull(int_comvl,0)/ifnull(int_indqt,1),0)) int_comvl,  int(round(ifnull(int_crdvl,0)/ifnull(int_indqt,1),0)) int_crdvl, in_cnscd from vw_intrn where "+strWHRSTR+"  order by int_prdcd";
			rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			if(!rstRSSET.next() || rstRSSET==null)
				return;
			int i =0;
	...
	private void setDETAIL_CPR()
	{
		try
		{
			cstCPRVL.setString(1,cmbMKTTP.getSelectedItem().toString().substring(0,2));
			cstCPRVL.setString(2,M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText())));
			cstCPRVL.setString(3,M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText())));
	...
	private void crtCNSHDR()
	{
		try
		{
			...
			intLINECT = 0;
			O_DOUT.writeBytes(padSTRING('R',"Supreme Petrochem Ltd",25)+padSTRING('C',"Consigneewise Realisation Details  from "+M_txtFMDAT.getText()+ " to "+ M_txtTODAT.getText(),intCNSWD-50)+padSTRING('L',"Page "+intPAGENO,25)+"\n\n"); intLINECT +=2;
			setOPTHDR();						
			O_DOUT.writeBytes("\n"); ;
			intRUNCL = 0;
	...
	private void crtGRDHDR()
	{
		try
		{
			...
			intLINECT = 0;
			O_DOUT.writeBytes(padSTRING('R',"Supreme Petrochem Ltd",25)+padSTRING('C',"Gradewise Realisation Detail from "+M_txtFMDAT.getText()+ " to "+ M_txtTODAT.getText(),intGRDWD-50)+padSTRING('L',"Page "+intPAGENO,25)+"\n\n"); intLINECT +=2;
			setOPTHDR();						
			intRUNCL = 0;
	...
	private String setWHRSTR(String LP_STRDT, String LP_ENDDT)
	{
		...
		try
		{
			L_strRETSTR = (flgIVT_RLSN ? " ivt_bkgdt " : " in_bkgdt ")+" between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(LP_STRDT))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(LP_ENDDT))+"' and int_stsfl<>'X' and ifnull(int_indqt,0)>0  and int_mkttp='"+cmbMKTTP.getSelectedItem().toString().substring(0,2)+"'"
				+ " and "+(flgIVT_RLSN ? " ivt_sbscd " : " in_sbscd ")+" in "+M_strSBSLS;
		}
