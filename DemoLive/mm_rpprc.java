...
class mm_rpprc extends cl_rbase
{
	...
	private Hashtable<String,String> hstMGPDS;/** Process to display report*/
	...
	mm_rpprc()
	{
		...
		try
		{
			...
			hstMGPDS=new Hashtable<String,String>(50,0.75f);
	...
	private boolean exeRPLGR()
	{
		...
		try
		{
			if(rdbYEAR.isSelected())
			{
				...
				L_rstRSSET=cl_dat.exeSQLQRY0("Select YR_MATCD MATCD, YR_MATDS MATDS, YR_YOSQT OPSQT, YR_YOSVL OPSVL, YR_YCSQT CLSQT, YR_YCSVL CLSVL from MM_YRPRC  where  yr_cmpcd = '"+cl_dat.M_strCMPCD_pbst+"' and "+(chbGRPAL.isSelected() ? "" : "  SUBSTRING(yr_MATCD,1,2) not in('68','69','85','86') and ")+" YR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND YR_STRTP='"+M_strSBSCD.substring(2,4)+"' order by YR_MATCD");
				M_rstRSSET=cl_dat.exeSQLQRY0("Select PS_DOCNO DOCNO, PS_DOCDT DOCDT, PS_DOCTP DOCTP, PS_MATCD MATCD, PS_DOCQT DOCQT, PS_DOCVL DOCVL from MM_PSLYR where  ps_cmpcd = '"+cl_dat.M_strCMPCD_pbst+"' and "+(chbGRPAL.isSelected() ? "" : " SUBSTRING(Ps_MATCD,1,2) not in('68','69','85','86') and ")+" PS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND  PS_STRTP='"+M_strSBSCD.substring(2,4)+"' order by PS_MATCD,PS_DOCTP");
			...
			//TRANSFER DATA FROM LIVE TABLES TO TEMP. TABLE
				...
				cl_dat.exeSQLUPD("Insert into "+L_strTBLNM+" (Select GR_STRTP,GR_MATCD,'1',GR_GRNNO,GR_BATNO,gr_ACPQT,gr_BILVL,GR_ACPDT from MM_GRMST where GR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND GR_ACPDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"'  and isnull(GR_STSFL,'')='2')","");
				setMSG("Collecting MIN data ..",'N');
				cl_dat.exeSQLUPD("Insert into "+L_strTBLNM+" (Select IS_STRTP,IS_MATCD,'6',IS_ISSNO,IS_TAGNO,IS_ISSQT,IS_ISSVL,CONVERT(varchar,IS_AUTDT,101) from MM_ISMST where IS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND CONVERT(varchar,IS_AUTDT,101) between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and isnull(IS_STSFL,'')='2')","");
				setMSG("Collecting MRN data ..",'N');
				cl_dat.exeSQLUPD("Insert into "+L_strTBLNM+" (Select MR_STRTP,MR_MATCD,'2',MR_MRNNO,MR_MRNNO,MR_RETQT,MR_MRNVL,MR_MRNDT from MM_MRMST where MR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND MR_MRNDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and isnull(MR_STSFL,'')='2')","");
				setMSG("Collecting SAN+ data ..",'N');
				cl_dat.exeSQLUPD("Insert into "+L_strTBLNM+" (Select SA_STRTP,SA_MATCD,'3',SA_SANNO,SA_SANNO,SA_SANQT,SA_SANVL,SA_SANDT  from MM_SAMST where SA_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND SA_SANDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and SA_SANQT>0 and isnull(SA_STSFL,'')<>'X')","");
                cl_dat.exeSQLUPD("Insert into "+L_strTBLNM+" (Select STN_TOSTR,STN_TOMAT,'4',STN_STNNO,STN_STNNO,STN_TRNQT,STN_TRNVL,STN_STNDT  from MM_STNTR where STN_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STN_STNDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and STN_STNTP IN('01','02') and isnull(STN_STSFL,'')= '2')","");
				cl_dat.exeSQLUPD("Insert into "+L_strTBLNM+" (Select SA_STRTP,SA_MATCD,'7',SA_SANNO,SA_SANNO,SA_SANQT,SA_SANVL,SA_SANDT  from MM_SAMST where SA_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND SA_SANDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and SA_SANQT<0 and isnull(SA_STSFL,'')<>'X')","");
				cl_dat.exeSQLUPD("Insert into "+L_strTBLNM+" (Select STN_FRSTR,STN_FRMAT,'8',STN_STNNO,STN_STNNO,(- STN_TRNQT) STN_TRNQT,(- STN_TRNVL) STN_TRNVL,STN_STNDT  from MM_STNTR where STN_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STN_STNDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and STN_STNTP IN('01','03') and isnull(STN_STSFL,'')='2')","");
				setMSG("Formatting data. Please wait ..",'N');
				cl_dat.exeDBCMT("trfDATA");
			//COLLECTING DATA IN RESULT SETS
				L_rstRSSET=cl_dat.exeSQLQRY0("Select STP_MATCD MATCD, STP_MATDS MATDS, STP_MOSQT OPSQT, STP_MOSVL OPSVL, STP_MCSQT CLSQT, STP_MCSVL CLSVL from MM_STPRC where STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STP_strtp='01' "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(STP_MATCD,1,2) not in('68','69','85','86') ")+" and STP_STRTP='"+M_strSBSCD.substring(2,4)+"' order by STP_MATCD");
				M_rstRSSET=cl_dat.exeSQLQRY0("Select TT_DOCNO DOCNO, TT_DOCDT DOCDT, TT_DOCTP DOCTP, TT_MATCD MATCD, TT_DOCQT DOCQT, TT_DOCVL DOCVL from "+L_strTBLNM+" where TT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND TT_strtp='01' "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(TT_MATCD,1,2) not in('68','69','85','86') ")+" and TT_STRTP='"+M_strSBSCD.substring(2,4)+"' order by TT_MATCD,TT_DOCTP");
			}
				if(M_rstRSSET==null)
				{
				...
				String[] L_staTRNTP=new String[]{"GRIN","MRN","+SAN","+STN"," ","MIN","SAN","STN"};//TO PRINT TRANSACTION TYPE WRT DOCTP
				String L_strBLANK="  ";
				prnHEADR();
				L_rstRSSET.next();
				...
				while(M_rstRSSET.next())
				{
					...
					{//DISPLAY CLOSING BALANCE FOR CURRENT AND OPENING BALANCE FOR NEXT MATCD
						if(!L_rstRSSET.next())
							break;
						setMSG(M_rstRSSET.getString("MATCD")+"---"+L_rstRSSET.getString("MATCD")+"---"+L_rstRSSET.getString("CLSQT"),'N');
						prnFMTCHR(dosREPORT,M_strCPI10);
	...
	private boolean exeRPGTS()
	{
		try
		{
			...
			if(rdbYEAR.isSelected())
			{
				M_strSQLQRY="Select substring(YR_MATCD,1,2) YR_GRPCD,CT_MATDS,sum(YR_YOSVL),sum(YR_CRCVL)+sum(YR_CSAVL)+sum(YR_CSTVL),sum(YR_CISVL)-sum(YR_CMRVL),sum(YR_ADJVL),sum(YR_YCSVL) from MM_YRPRC , CO_CTMST where  "
						+" YR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND SUBSTRING(CT_MATCD,1,2)=SUBSTRING(YR_MATCD,1,2) ";
					
			//  For imported items figure			
				if(chbIMPIT.isSelected())
					M_strSQLQRY +=" and SUBSTRING(YR_MATCD,10,1) IN('2','6') ";
						
				M_strSQLQRY += (chbGRPAL.isSelected()? "" : " and SUBSTRING(YR_MATCD,1,2) not in('68','69','85','86') " )
						+"and YR_STRTP ='01'"	
			...
			else
			{
				...
				M_strSQLQRY="Select substring(STP_MATCD,1,2),CT_MATDS,sum(isnull(STP_MOSVL,0)),sum(isnull(STP_MCRCV,0)-isnull(STP_MORCV,0)+isnull(STP_MCSAV,0)-isnull(STP_MOSAV,0)+isnull(STP_MCSTV,0)-isnull(STP_MOSTV,0)),sum(isnull(STP_MCISV,0)-isnull(STP_MOISV,0)-(isnull(STP_MCMRV,0)-isnull(STP_MOMRV,0))),0,sum(isnull(STP_MCSVL,0)) from MM_STPRC, CO_CTMST where STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND substring(CT_MATCD,1,2) = substring(STP_MATCD,1,2) and CT_CODTP='MG' "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(STP_MATCD,1,2) not in('68','69','85','86') ")+" and STP_STRTP='"+M_strSBSCD.substring(2,4)+"' group by substring(STP_MATCD,1,2),CT_MATDS order by substring(STP_MATCD,1,2)";
			}
...
//				L_dblTOT4+=M_rstRSSET.getDouble(6);
	...
	private boolean exeRPGTS1() // Method for displaying the groupwise trans summary category wise 
	{
		try
		{
			...
			Hashtable<String,String> hstMATDS = new Hashtable<String,String>();
			if(M_rstRSSET !=null)
				while(M_rstRSSET.next())
					hstMATDS.put(M_rstRSSET.getString("CT_GRPCD"),nvlSTRVL(M_rstRSSET.getString("CT_MATDS"),""));
			if(rdbYEAR.isSelected())
			{
				M_strSQLQRY="Select substring(YR_MATCD,1,2) YR_GRPCD,YR_CATFL,sum(YR_YOSVL),sum(YR_CRCVL)+sum(YR_CSAVL)+sum(YR_CSTVL),sum(YR_CISVL)-sum(YR_CMRVL),sum(YR_ADJVL),sum(YR_YCSVL) from MM_YRPRC where YR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND YR_STRTP ='01'";
					//	+" SUBSTRING(CT_MATCD,1,2)=SUBSTRING(YR_MATCD,1,2) ";
			...
			{
				//M_strSQLQRY="Select substring(STP_MATCD,1,2),CT_MATDS,sum(STP_MOSVL),sum(STP_MCRCV-STP_MORCV+STP_MCSAV-STP_MOSAV),sum(STP_MCISV-STP_MOISV-(STP_MCMRV-STP_MOMRV)),0,sum(STP_MCSVL) from MM_STPRC, CO_CTMST where substring(CT_MATCD,1,2) = substring(STP_MATCD,1,2) and CT_CODTP='MG' group by substring(STP_MATCD,1,2),CT_MATDS order by substring(STP_MATCD,1,2)";
				M_strSQLQRY="Select substring(STP_MATCD,1,2),STP_CATFL,sum(isnull(STP_MOSVL,0)),sum(isnull(STP_MCRCV,0)-isnull(STP_MORCV,0)+isnull(STP_MCSAV,0)-isnull(STP_MOSAV,0)+isnull(STP_MCSTV,0)-isnull(STP_MOSTV,0)),sum(isnull(STP_MCISV,0)-isnull(STP_MOISV,0)-(isnull(STP_MCMRV,0)-isnull(STP_MOMRV,0))),0,sum(isnull(STP_MCSVL,0)) from MM_STPRC where  STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STP_STRTP='"+M_strSBSCD.substring(2,4)+"'"+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(STP_MATCD,1,2) not in('68','69','85','86') ")+" group by substring(STP_MATCD,1,2),STP_CATFL order by substring(STP_MATCD,1,2),STP_CATFL";
			}
			M_rstRSSET=cl_dat.exeSQLQRY0(M_strSQLQRY);
			if(M_rstRSSET==null)
			{
...
//				L_dblTOT4+=M_rstRSSET.getDouble(6);
	...
	private boolean exeRPGTS2() // Method for displaying the groupwise trans summary category wise 
	{
		try
		{
			...
			M_rstRSSET = cl_dat.exeSQLQRY(M_strSQLQRY);
			Hashtable<String,String> hstMATDS = new Hashtable<String,String>();
			Hashtable<String,String> hstDPTDS = new Hashtable<String,String>();
			if(M_rstRSSET !=null)
				while(M_rstRSSET.next())
					hstMATDS.put(M_rstRSSET.getString("CT_GRPCD"),nvlSTRVL(M_rstRSSET.getString("CT_MATDS"),""));
			...
			if(rdbYEAR.isSelected())
			{
				M_strSQLQRY="Select YR_OWNBY,substring(YR_MATCD,1,2) YR_GRPCD,YR_CATFL,sum(YR_YOSVL),sum(YR_CRCVL)+sum(YR_CSAVL)+sum(YR_CSTVL),sum(YR_CISVL)-sum(YR_CMRVL),sum(YR_ADJVL),sum(YR_YCSVL) from MM_YRPRC where YR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND  YR_STRTP ='01'";
			...
			{
				M_strSQLQRY="Select STP_OWNBY,substring(STP_MATCD,1,2),STP_CATFL,sum(isnull(STP_MOSVL,0)),sum(isnull(STP_MCRCV,0)-isnull(STP_MORCV,0)+isnull(STP_MCSAV,0)-isnull(STP_MOSAV,0)+isnull(STP_MCSTV,0)-isnull(STP_MOSTV,0)),sum(isnull(STP_MCISV,0)-isnull(STP_MOISV,0)-(isnull(STP_MCMRV,0)-isnull(STP_MOMRV,0))),0,sum(isnull(STP_MCSVL,0)) from MM_STPRC where STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STP_STRTP='"+M_strSBSCD.substring(2,4)+"'"+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(STP_MATCD,1,2) not in('68','69','85','86') ")+" group by STP_OWNBY,substring(STP_MATCD,1,2),STP_CATFL order by STP_OWNBY,substring(STP_MATCD,1,2),STP_CATFL";
			}
			M_rstRSSET=cl_dat.exeSQLQRY0(M_strSQLQRY);
			if(M_rstRSSET==null)
			{
	...
	private boolean exeRPGTD()
	{
		try
		{
			...
			if(rdbYEAR.isSelected())
			{
				M_strSQLQRY = "SELECT YR_STRTP,SUBSTRING(YR_MATCD,1,2)MATCD,CT_MATDS,SUM(isnull(YR_YOSVL,0)) YOSVL,"
					+"SUM(isnull(YR_CRCVL,0)) CRCVL ,SUM(isnull(YR_CMRVL,0)) CMRVL ,"
					+"SUM(isnull(YR_CSAVL,0)) CSAVL ,SUM(isnull(YR_ADJVL,0)) ADJVL ,"
					+"SUM(isnull(YR_CISVL,0)) CISVL ,SUM(isnull(YR_YCSVL,0)) YCSVL, SUM(isnull(YR_CSTVL,0)) CSTVL "
					+" FROM MM_YRPRC,CO_CTMST WHERE CT_GRPCD = SUBSTRING(YR_MATCD,1,2) "
					+(chbGRPAL.isSelected() ? "" : " and YR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND SUBSTRING(YR_MATCD,1,2) not in('68','69','85','86') " )
					+" AND CT_CODTP = 'MG' "
					+"AND YR_STRTP = '"+M_strSBSCD.substring(2,4)+"' "
					+"GROUP BY YR_STRTP,SUBSTRING(YR_MATCD,1,2),CT_MATDS ORDER BY SUBSTRING(YR_MATCD,1,2) ";
			}
			else
			{
				M_strSQLQRY="Select STP_STRTP,substring(STP_MATCD,1,2) MATCD,CT_MATDS,sum(isnull(STP_MOSVL,0)) YOSVL,"
							+" sum(isnull(STP_MCRCV,0)-isnull(STP_MORCV,0)) CRCVL,sum(isnull(STP_MCMRV,0)-isnull(STP_MOMRV,0)) CMRVL,"
							+" sum(isnull(STP_MCSAV,0)-isnull(STP_MOSAV,0)) CSAVL,0 ADJVL,"
							+" sum(isnull(STP_MCISV,0)-isnull(STP_MOISV,0)) CISVL,sum(isnull(STP_MCSVL,0)) YCSVL,sum(isnull(STP_MCSTV,0)-isnull(STP_MOSTV,0)) CSTVL from MM_STPRC, CO_CTMST where STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND substring(CT_MATCD,1,2) = substring(STP_MATCD,1,2) and CT_CODTP='MG'  "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(STP_MATCD,1,2) not in('68','69','85','86') ")+" and STP_STRTP='"+M_strSBSCD.substring(2,4)+"' group by substring(STP_MATCD,1,2),CT_MATDS,STP_STRTP order by substring(STP_MATCD,1,2)";
			}
			M_rstRSSET=cl_dat.exeSQLQRY0(M_strSQLQRY);
			if(M_rstRSSET==null)
			{
...
//						prnFMTCHR(dosREPORT,M_strEJT);
					...
					dosREPORT.writeBytes(padSTRING('L',setNumberFormat(M_rstRSSET.getDouble("CSTVL"),0),22));
					...
					fltCSTVL = fltCSTVL + M_rstRSSET.getDouble("CSTVL");
				...
				dosREPORT.writeBytes(padSTRING('L',setNumberFormat(fltCSTVL,0),22));
	 ...
	 */
	private boolean exeRPSVS() // CHANGED FOR STN
	{
		try
		{
			...
			if(rdbYEAR.isSelected())
			{
				M_strSQLQRY="Select YR_MATCD MATCD,CT_MATDS MATDS,YR_UOMCD UOMCD,YR_YOSVL YOSVL,YR_YOSQT YOSQT,YR_CRCQT+YR_CSAQT+YR_CSTQT CRCQT,YR_CRCVL+YR_CSAVL+YR_CSTVL CRCVL,YR_CISVL-YR_CMRVL CISVL,YR_CISQT-YR_CMRQT CISQT,YR_YCSVL YCSVL, YR_YCSQT YCSQT, YR_ADJVL ADJVL from MM_YRPRC , CO_CTMST where YR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND  YR_STRTP='"+M_strSBSCD.substring(2,4)+"' and CT_MATCD=YR_MATCD "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(YR_MATCD,1,2) not in('68','69','85','86') ")+" and CT_CODTP='CD'  order by yr_matcd ";//and yr_ycsqt<>st_stkqt
			}
			else
			{
				M_strSQLQRY="Select STP_MATCD MATCD,STP_MATDS MATDS,STP_UOMCD UOMCD,STP_MOSVL YOSVL,STP_MOSQT YOSQT,(STP_MCRCQ-STP_MORCQ)+(STP_MCSAQ-STP_MOSAQ)+(STP_MCSTQ-STP_MOSTQ) CRCQT,(STP_MCRCV-STP_MORCV)+(STP_MCSAV-STP_MOSAV)+(STP_MCSTV-STP_MOSTV) CRCVL,(STP_MCISV-STP_MOISV)-(STP_MCMRV-STP_MOMRV) CISVL,(STP_MCISQ-STP_MOISQ)-(STP_MCMRQ-STP_MOMRQ) CISQT,STP_MCSVL YCSVL, STP_MCSQT YCSQT, 0 ADJVL from MM_STPRC  where STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STP_STRTP='"+M_strSBSCD.substring(2,4)+"' "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(STP_MATCD,1,2) not in('68','69','85','86') ")+" order by STP_matcd ";//and yr_ycsqt<>st_stkqt
			}
			M_rstRSSET=cl_dat.exeSQLQRY0(M_strSQLQRY);
			if(M_rstRSSET==null)
			{
...
*/
	private boolean exeRPSVS1()
	{
		try
		{
			...
			if(rdbYEAR.isSelected())
			{
				M_strSQLQRY="Select YR_MATCD MATCD,CT_MATDS MATDS,YR_UOMCD UOMCD,YR_YCSVL YCSVL, YR_YCSQT YCSQT,(stp_ycsvl/stp_ycsqt) YCLRT  from MM_YRPRC , CO_CTMST where YR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND YR_STRTP='"+M_strSBSCD.substring(2,4)+"' and CT_MATCD=YR_MATCD "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(YR_MATCD,1,2) not in('68','69','85','86') ")+" and CT_CODTP='CD'  order by yr_matcd ";//and yr_ycsqt<>st_stkqt
			}
			else
			{
				M_strSQLQRY="Select STP_MATCD MATCD,STP_MATDS MATDS,STP_UOMCD UOMCD,STP_MCSVL YCSVL, STP_MCSQT YCSQT,(stp_mcsvl/stp_mcsqt) YCLRT from MM_STPRC  where STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STP_STRTP='"+M_strSBSCD.substring(2,4)+"' "+(chbGRPAL.isSelected() ? "" : " and SUBSTRING(STP_MATCD,1,2) not in('68','69','85','86') ")+"and isnull(stp_mcsqt,0) > 0  order by STP_matcd ";//and yr_ycsqt<>st_stkqt
			}
			M_rstRSSET=cl_dat.exeSQLQRY0(M_strSQLQRY);
			if(M_rstRSSET==null)
			{
	...
	private void prnHEADR() throws Exception
	{
		...
		prnFMTCHR(dosREPORT,M_strCPI10);
		prnFMTCHR(dosREPORT,M_strBOLD);
		dosREPORT.writeBytes(padSTRING('R',cl_dat.M_strCMPNM_pbst,50)+padSTRING('L',"Date : "+cl_dat.M_txtCLKDT_pbst.getText(),38));
		crtNWLIN();
		...
		if(cmbRPTOP.getITMCD().equals("0"))
		{
			dosREPORT.writeBytes(padSTRING('R',"Trn. Date",15));
		...
		else if(cmbRPTOP.getITMCD().equals("2"))
		{
			...
			dosREPORT.writeBytes("\n");
			dosREPORT.writeBytes("                  Receipt Value             SAN Value           STN Value ");
			crtNWLIN();
			crtHRLIN(106,"-");
			crtNWLIN();
		}
...
private void prnDIFFHD() throws Exception
	{
		...
		dosDIFF.writeBytes(padSTRING('R',cl_dat.M_strCMPNM_pbst,50)+padSTRING('L',"Date : "+cl_dat.M_txtCLKDT_pbst.getText(),38));
		dosDIFF.writeBytes("\n");
	...
	private boolean crtYRDBF()
	{
		try
		{
			...
			M_strSQLQRY ="SELECT PS_STRTP,PS_DOCNO,PS_DOCDT,PS_MATCD,PS_DOCQT,PS_DOCRT,PS_DOCVL,PS_MODVL,"+
				        "GR_VENCD,GR_VENNM,ST_MATDS,isnull(GR_VATBL,GR_VATVL) L_VATVL FROM MM_PSLYR,MM_GRMST,MM_STMST WHERE PS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND PS_DOCTP ='1' "+
					//	" AND PS_DOCDT BETWEEN '07/01/2003' AND '06/30/2004' AND ST_STRTP = GR_STRTP AND ST_MATCD = GR_MATCD AND ST_STRTP ='01'"+
						 " AND PS_DOCDT BETWEEN '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' AND ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND  ST_STRTP = GR_STRTP AND ST_MATCD = GR_MATCD AND ST_STRTP ='01'"+
						" AND PS_CMPCD = GR_CMPCD and  GR_CMPCD = ST_CMPCD and PS_STRTP = GR_STRTP AND PS_DOCNO = GR_GRNNO AND PS_MATCD = GR_MATCD AND PS_DOCRF = GR_BATNO AND PS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND PS_STRTP ='01' and SUBSTRING(ps_matcd,1,2) not in('68','69','85','86') ";
			...
			if (M_rstRSSET != null){
				...
				while(M_rstRSSET.next())
				{
					...
					M_strSQLQRY += nvlSTRVL(M_rstRSSET.getString("l_vatvl"),"0")+")";
			...
			M_strSQLQRY ="SELECT GR_STRTP,GR_GRNNO,GR_ACPDT,GR_MATCD,GR_ACPQT,po_porRT,GR_PORVL,GR_MODVL,"+
				        "GR_VENCD,GR_VENNM,ST_MATDS FROM MM_GRMST,mm_pomst,MM_STMST WHERE GR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND gr_acpqt > 0 "+
					//	" AND gr_ACPDT BETWEEN '07/01/2003' AND '06/30/2004' AND gr_strtp = po_strtp and gr_porno = po_porno and gr_matcd = po_matcd and ST_STRTP = GR_STRTP AND ST_MATCD = GR_MATCD AND ST_STRTP ='01'"+
						" AND gr_ACPDT BETWEEN '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' AND GR_CMPCD=PO_CMPCD and gr_strtp = po_strtp and gr_porno = po_porno and gr_matcd = po_matcd and ST_CMPCD=GR_CMPCD and ST_STRTP = GR_STRTP AND ST_MATCD = GR_MATCD AND ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_STRTP ='01'"+
						 " AND SUBSTRING(gr_matcd,1,2) not in('68','69','85','86') and isnull(gr_bilvl,0) =0";
		...
		M_strSQLQRY ="SELECT PS_STRTP,PS_DOCNO,PS_DOCDT,PS_MATCD,PS_DOCQT,PS_DOCRT,PS_DOCVL,PS_MODVL,"+
				        "ST_MATDS FROM MM_PSLYR,MM_STMST WHERE PS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND PS_DOCTP in('3','7') "+
					//	" AND PS_DOCDT BETWEEN '07/01/2003' AND '06/30/2004' AND ST_STRTP ='01'"+
						...
						" AND PS_CMPCD = ST_CMPCD and PS_STRTP = st_STRTP AND pS_MATCD = st_MATCD AND PS_STRTP ='01' and SUBSTRING(ps_matcd,1,2) not in('68','69','85','86') ";
	...
	private boolean crtMNDBF()
	{
		try
		{
			...
			M_strSQLQRY ="SELECT GR_STRTP,GR_GRNNO,GR_ACPDT,GR_MATCD,GR_ACPQT,GR_GRNRT,GR_BILVL,GR_MODVL,"+
				        "GR_VENCD,GR_VENNM,ST_MATDS,isnull(GR_VATBL,GR_VATVL)L_VATVL FROM MM_GRMST,MM_STMST WHERE "+
						" GR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND GR_ACPDT BETWEEN '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' AND ST_CMPCD=GR_CMPCD and ST_STRTP = GR_STRTP AND ST_MATCD = GR_MATCD AND ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_STRTP ='01'"+
					//	" and SUBSTRING(GR_matcd,1,2) not in('68','69','85','86') AND GR_ACPDT IS NOT NULL";
			...
			M_strSQLQRY ="SELECT GR_STRTP,GR_GRNNO,GR_ACPDT,GR_MATCD,GR_ACPQT,GR_GRNRT,GR_BILVL,GR_MODVL,"+
				        "GR_VENCD,GR_VENNM,ST_MATDS,isnull(GR_VATBL,GR_VATVL) L_VATVL FROM MM_GRMST,MM_STMST WHERE "+
						" GR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND GR_ACPDT BETWEEN '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' AND ST_CMPCD=GR_CMPCD and ST_STRTP = GR_STRTP AND ST_MATCD = GR_MATCD "+
						" and SUBSTRING(GR_matcd,1,2) in('68','69') AND GR_ACPDT IS NOT NULL";
			...
			i=0;
			j=0;
			M_strSQLQRY ="SELECT IS_STRTP,IS_ISSNO,CONVERT(varchar,IS_AUTDT,101)L_ISSDT,IS_MATCD,ST_MATDS,ST_UOMCD,IS_ISSQT FROM MM_ISMST,MM_STMST WHERE "+
						" IS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND CONVERT(varchar,IS_AUTDT,101) BETWEEN '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' AND ST_CMPCD=IS_CMPCD and ST_STRTP = IS_STRTP AND ST_MATCD = IS_MATCD AND ST_STRTP ='01'"+
					//	" and SUBSTRING(IS_matcd,1,2) not in('68','69','85','86') AND IS_AUTDT IS NOT NULL";
						 	"  AND IS_AUTDT IS NOT NULL";
			setMSG("Genearating dbf psl_is01",'N');
			System.out.println("psl_is01 : "+M_strSQLQRY);
			M_rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			cl_dat.M_flgLCUPD_pbst = true;
			if (M_rstRSSET != null)
			{
			...
			j=0;
			M_strSQLQRY ="SELECT IS_STRTP,IS_ISSNO,CONVERT(varchar,IS_AUTDT,101)L_ISSDT,IS_MATCD,ST_MATDS,ST_UOMCD,IS_ISSQT FROM MM_ISMST,MM_STMST WHERE "+
						" IS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND CONVERT(varchar,IS_AUTDT,101) BETWEEN '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' AND ST_CMPCD=IS_CMPCD and ST_STRTP = IS_STRTP AND ST_MATCD = IS_MATCD "+
						" and SUBSTRING(IS_matcd,1,2) in('68','69') AND IS_AUTDT IS NOT NULL";
			setMSG("Genearating dbf psl_is07",'N');
			System.out.println("psl_is07 : "+M_strSQLQRY);
			M_rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			cl_dat.M_flgLCUPD_pbst = true;
			if (M_rstRSSET != null)
			{
...
////    TANKFARM DBF's
			...
			M_strSQLQRY =	" SELECT IS_STRTP,IS_ISSTP,IS_ISSNO,IS_ISSDT,IS_MATCD,CT_MATDS,IS_ISSQT "
						+" FROM MM_ISMST,CO_CTMST where CT_MATCD = IS_MATCD AND IS_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND IS_STRTP ='04'"
			...
			System.out.println("mm_issdt : "+M_strSQLQRY);
			M_rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			cl_dat.M_flgLCUPD_pbst = true;
			if (M_rstRSSET != null)
			{
			...
			M_strSQLQRY =  " SELECT GR_STRTP,GR_GRNTP,GR_GRNNO,GR_GRNDT,GR_MATCD,CT_MATDS,"
						   ...
						   +" AND GR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND GR_STRTP ='04' and GR_GRNDT BETWEEN '"
...
private boolean crtTFDBF() // For B/E/ wise advance licence receipt
{
	try
	{
		...
		M_strSQLQRY = "SELECT GR_GRNTP,GR_BOENO,BE_BOEDT,BE_BOEQT,GR_GRNNO,GR_GRNDT,GR_MATCD,'',"
					 ...
					 +" FROM MM_GRMST,MM_BETRN WHERE GR_CMPCD=BE_CMPCD and GR_BOENO = BE_BOENO AND BE_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND BE_MATTP ='03' "
			         +" AND GR_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND GR_GRNDT BETWEEN '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText())) + "' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' ORDER BY GR_BOENO,GR_MATCD";
		/*M_strSQLQRY ="SELECT GR_GRNTP,GR_BOENO,BE_BOEDT,BE_BOEQT,GR_GRNNO,GR_GRNDT,GR_MATCD,"
