...
txtFMSUB    SUBSTRING(ST_MATCD,1,6)    MM_STMST        VARCHAR(10)   Material Code
txtTOSUB    SUBSTRING(ST_MATCD,1,6)    MM_STMST        VARCHAR(10)   Material Code
...
1)For Materials below Minimum Level Report, Data is taken from MM_STMST,MM_POMST 
  and MM_INMST for condiations :-
     ...
     4) AND isnull(PO_PORQT,0)-isnull(PO_ACPQT,0)-isnull(PO_FRCQT,0) >0 
     5) AND isnull(PO_STSFL ,'') <>'X'
     6) LEFT OUTER JOIN MM_INMST ON 
     7) ST_STRTP = IN_STRTP 
     8) AND ST_MATCD = IN_MATCD 
     9) AND isnull(IN_INDQT,0)-isnull(IN_ORDQT,0)-isnull(IN_FCCQT,0) >0 
    10) AND isnull(IN_STSFL,'') <>'X'"
    11) WHERE ST_STKFL = 'Y' 
    12) AND isnull(ST_STKQT,0) < isnull(ST_MINLV,0) 
    13) AND ST_MMSBS = Specified Subsystem Code
    14) AND SUBSTRING(ST_MATCD,1,2) between given range of Material Code
...
5)For Weekly Stock Status Of Packing Materials Report, Data is taken from MM_STMST for condiations :-
    1) SUBSTRING(ST_MATCD,1,6) in ('690520','691010') 
    2) AND isnull(ST_STKQT,0) > 0 
    3) AND ST_STSFL <> 'X' 
...
6) For Materials Below Re-order Level Report, Data is taken from MM_STMST for condiations :-
    ...
    2) AND  ST_MATCD = PO_MATCD 
    3) AND isnull(PO_PORQT,0)- isnull(PO_ACPQT,0)-isnull(PO_FRCQT,0) >0 
    4) AND isnull(PO_STSFL ,'') <>'X'
    5) LEFT OUTER JOIN MM_INMST ON ST_STRTP = IN_STRTP 
    6) AND ST_MATCD = IN_MATCD 
    7) AND isnull(IN_INDQT,0)-isnull(IN_ORDQT,0)-isnull(IN_FCCQT,0) >0 
    8) AND isnull(IN_STSFL,'') <>'X'
    9) WHERE ST_STKFL = 'Y' 
   10) AND isnull(ST_STKQT,0) < isnull(ST_RORLV,0) 
   11) AND ST_MMSBS = Specified SubSystem Code 
   12) AND SUBSTRING(ST_MATCD,1,2) between th given range of Material Group Code
   13) AND  ST_STSFL <> 'X'
...
7)For Location wise Inventory Report, Data is taken from MM_STMST for condiations :-
    ...
    3) AND isnull(ST_STSFL,'') <> 'X' 
    4) AND isnull(ST_STKQT,0) <> 0
...
{									/** JPanel to display Main Group title.*/
	...
	private Hashtable<String,String> hstLOCPR;
	mm_rpstk()
	{
		super(2);
		try
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		if(L_KE.getKeyCode() == L_KE.VK_F1)
		{
			...
			{	
				...
				M_strSQLQRY ="SELECT distinct ST_LOCCD FROM MM_STMST Where ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND isnull(ST_STSFL,'')<>'X'";
			...
			if(M_objSOURC == txtFMSUB)
			{
				...
				M_strSQLQRY ="SELECT SUBSTRING(CT_MATCD,1,6),CT_MATDS FROM CO_CTMST WHERE CT_CODTP ='SS' ";
		...
		try
		{
			...
			if(cmbRPTOP.getSelectedIndex() == 0)
			{
				...
				/* M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS,ST_UOMCD,ST_LOCCD,ST_MINLV,ST_MAXLV,ST_RORLV,ST_STODT,"
					+"ST_RORQT,ST_STKQT,ST_STKIP,ST_STKIN,ST_STKOR,ST_YTDIS,PO_PORNO,isnull(PO_PORQT,0)-isnull(PO_ACPQT,0) L_PORQT,PO_PORRT,"
					+"IN_INDNO,isnull(IN_INDQT,0)-isnull(IN_ORDQT,0) L_INDQT,ST_PPONO "
					+"FROM MM_STMST left OUTER JOIN MM_POMST ON "
					+"ST_STRTP = PO_STRTP AND  ST_MATCD = PO_MATCD "
					+" AND isnull(PO_PORQT,0) > isnull(PO_ACPQT,0) AND isnull(PO_STSFL ,'') <>'X' "
					+" LEFT OUTER JOIN MM_INMST ON ST_STRTP = IN_STRTP AND ST_MATCD = IN_MATCD "
					+" AND isnull(IN_INDQT,0)-isnull(IN_ORDQT,0) >0 AND isnull(IN_STSFL,'') <>'X'"
					+" WHERE ST_STKFL = 'Y' AND "
					+"isnull(ST_STKQT,0) < isnull(ST_MINLV,0) AND ST_MMSBS = '"+M_strSBSCD+"' AND "
				    +"SUBSTRING(ST_MATCD,1,2) BETWEEN '"+txtFMGRP.getText()+"' AND '"+txtTOGRP.getText()+"' AND  ST_STSFL <> 'X' "
					+"ORDER BY ST_MATCD"; */
				...
				M_strSQLQRY = "SELECT distinct ST_MATCD,ST_MATDS,ST_UOMCD,ST_LOCCD,ST_MINLV,ST_MAXLV,ST_RORLV,ST_STODT,"
					+"ST_RORQT,ST_STKQT,ST_STKIP,ST_STKIN,ST_STKOR,ST_YTDIS,PO_PORNO,isnull(PO_PORQT,0)-isnull(PO_ACPQT,0)-isnull(PO_FRCQT,0) L_PORQT,PO_PORRT,"
					+"IN_INDNO,isnull(IN_INDQT,0)-isnull(IN_ORDQT,0)-isnull(IN_FCCQT,0) L_INDQT,ST_PPONO "
					+"FROM MM_STMST left OUTER JOIN MM_POMST ON "
					+"ST_CMPCD = PO_CMPCD and ST_STRTP = PO_STRTP AND  ST_MATCD = PO_MATCD "
					+" AND PO_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND isnull(PO_PORQT,0)-isnull(PO_ACPQT,0)-isnull(PO_FRCQT,0) >0 AND isnull(PO_STSFL ,'') <>'X' "
					+" LEFT OUTER JOIN MM_INMST ON ST_CMPCD=IN_CMPCD and ST_STRTP = IN_STRTP AND ST_MATCD = IN_MATCD "
					+" AND IN_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND isnull(IN_INDQT,0)-isnull(IN_ORDQT,0)-isnull(IN_FCCQT,0) >0 AND isnull(IN_STSFL,'') <>'X' AND IN_INDTP ='01'"
					+" WHERE ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_STKFL = 'Y' AND "
					+"isnull(ST_STKQT,0) < isnull(ST_MINLV,0) AND ST_MMSBS = '"+M_strSBSCD+"' AND "
				    +"SUBSTRING(ST_MATCD,1,2) BETWEEN '"+txtFMGRP.getText()+"' AND '"+txtTOGRP.getText()+"' AND  ST_STSFL <> 'X' "
					+"ORDER BY ST_MATCD";
			}
			...
			if(cmbRPTOP.getSelectedIndex() == 1)
			{
				...
				M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS,ST_UOMCD,ST_STKIN,ST_STKOR,ST_STKIP,"
					...
					+"FROM MM_STMST left outer join MM_POMST ON ST_CMPCD=PO_CMPCD and ST_STRTP = PO_STRTP AND "
					+"ST_MATCD = PO_MATCD  AND PO_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND isnull(PO_PORQT,0)-isnull(PO_ACPQT,0)- isnull(PO_FRCQT,0) >0 AND "
					+"PO_STSFL NOT IN('C','X') LEFT OUTER JOIN MM_INMST ON ST_CMPCD=IN_CMPCD and ST_STRTP = IN_STRTP "
					+"AND ST_MATCD = IN_MATCD  AND isnull(IN_INDQT,0)-isnull(IN_ORDQT,0) - isnull(IN_FCCQT,0) >0 "
					+"AND IN_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND IN_STSFL <> 'X' AND IN_INDTP ='01' WHERE ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND isnull(ST_STKQT,0) = 0 AND ST_MMSBS = '"+M_strSBSCD+"' "
					+"AND  ST_STSFL <> 'X' AND SUBSTRING(ST_MATCD,1,6) BETWEEN '"+txtFMSUB.getText()+"' AND '"
					+txtTOSUB.getText()+"' ";
			...
			if(cmbRPTOP.getSelectedIndex() == 2)
			{
				...
				M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS,ST_UOMCD,ST_MINLV,ST_STKQT,ST_STKIN,"
					+"ST_STKOR,ST_YTDIS,ST_PPOVL FROM MM_STMST WHERE ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_STKFL = 'Y' AND "
					+"(isnull(ST_STKQT,0)+isnull(ST_STKIN,0)+isnull(ST_STKOR,0)) <= isnull(ST_MINLV,0) AND ST_MMSBS = '"+M_strSBSCD+"' AND ST_STRTP = '"+M_strSBSCD.substring(2,4)+"' AND "
					+" SUBSTRING(ST_MATCD,1,6) BETWEEN '"+txtFMSUB.getText()+"' AND '"+txtTOSUB.getText()+"' AND  isnull(ST_STSFL,'') <> 'X' "
					+"ORDER BY ST_MATCD ";
			}
			//Stock Statement
			if(cmbRPTOP.getSelectedIndex() == 3)
			{
				...
				M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS, ST_UOMCD, ST_STKQT,ST_LOCCD FROM MM_STMST "
					+"WHERE ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND isnull(ST_STKQT,0) > 0 AND "
					+"SUBSTRING(ST_MATCD,1,2) BETWEEN '"+txtFMGRP.getText()+"' AND '"+txtTOGRP.getText()+"' "
					+"AND ST_MMSBS = '"+M_strSBSCD+"' AND isnull(ST_STSFL,'') <> 'X' AND "
			...
			/*	M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS,ST_UOMCD,ST_LOCCD,ST_MINLV,ST_MAXLV,ST_RORLV,ST_STODT,"
					...
					+" AND isnull(IN_INDQT,0)-isnull(IN_ORDQT,0) >0 AND isnull(IN_STSFL,'') <>'X'"
					+" WHERE ST_STKFL = 'Y' AND "
					+"isnull(ST_STKQT,0) < isnull(ST_RORLV,0) AND ST_MMSBS = '"+M_strSBSCD+"' AND "
				    +"SUBSTRING(ST_MATCD,1,2) BETWEEN '"+txtFMGRP.getText()+"' AND '"+txtTOGRP.getText()+"' AND  ST_STSFL <> 'X' "
					+"ORDER BY ST_MATCD";*/
			...
			else if(cmbRPTOP.getSelectedIndex() == 6)
			{
				...
				hstLOCPR = new Hashtable<String,String>();
				M_strSQLQRY = "SELECT CMT_CODCD,CMT_CODDS FROM CO_CDTRN WHERE CMT_CGMTP ='SYS' AND CMT_CGSTP ='MMXXLCP' AND isnull(CMT_STSFL,'') <>'X'";
				...
				if(rdoSUMRY.isSelected())
				{
				     M_strSQLQRY = "SELECT CMT_CODCD,CMT_CODDS,COUNT(STP_MATCD)L_ITMCT,SUM(isnull(STP_YCSVL,0))L_ITMVL FROM CO_CDTRN,MM_STPRC,MM_STMST "+
				     " WHERE STP_CMPCD=ST_CMPCD and STP_STRTP = ST_STRTP AND STP_MATCD = ST_MATCD AND CMT_CGMTP = 'SYS' AND CMT_CGSTP ='MMXXLCP' AND CMT_CODCD = SUBSTRING(ST_LOCCD,1,2)"+ 
                     " AND STP_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND STP_STRTP ='"+M_strSBSCD.substring(0,2)+"' AND STP_YCSQT >0 GROUP BY CMT_CODCD,CMT_CODDS";
				}
				else
				{
				    M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS,ST_UOMCD,ST_LOCCD,ST_STKQT,STP_YCSVL "
					+"FROM MM_STMST,MM_STPRC WHERE ST_CMPCD=STP_CMPCD and ST_STRTP = STP_STRTP AND ST_MATCD = STP_MATCD AND ST_STRTP = '"+M_strSBSCD.substring(2,4)+"' AND "
					+"ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_LOCCD BETWEEN '"+txtFMLOC.getText()+"' AND '"+txtTOLOC.getText()+"' "
					+"AND  isnull(ST_STSFL,'') <> 'X' AND isnull(ST_STKQT,0) <> 0 ORDER BY ST_LOCCD,ST_MATCD";				
				}
			}
			...
			if(cmbRPTOP.getSelectedIndex() == 7)
			{
				...
				M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS,ST_UOMCD,ST_LOCCD,ST_ABCFL,ST_VEDFL,ST_HMLFL,"
					...
					+"FROM MM_STMST,CO_CTMST WHERE ST_MATCD = CT_MATCD and ct_codtp ='CD' AND ST_STRTP = '"+M_strSBSCD.substring(2,4)+"' AND isnull(ST_STSFL,'') <> 'X' AND "
					+"ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_MATCD BETWEEN '"+txtFMMAT.getText().trim()+"' AND '"+txtTOMAT.getText().trim()+"'";
			}
			///
			if(cmbRPTOP.getSelectedIndex() == 0)
				prnHEAD0();
				...
				while(M_rstRSSET.next())
				{
					...
					if(cmbRPTOP.getSelectedIndex() == 3)
					{
						...
						else 
						{
						 ...
						 dosREPORT.writeBytes(padSTRING('R',nvlSTRVL(M_rstRSSET.getString("ST_MATCD"),""),11));
						 dosREPORT.writeBytes(padSTRING('R',nvlSTRVL(M_rstRSSET.getString("ST_MATDS"),""),56));
	...
	private void prnHEAD3()
	{
		try
		{
			...
			{	
				...
				dosREPORT.writeBytes("     "+padSTRING('R',"Stock Statement As On Date "+cl_dat.M_strLOGDT_pbst,strDOTLN.length()-25));
				dosREPORT.writeBytes("Date    : "+cl_dat.M_strLOGDT_pbst+"\n");			
				dosREPORT.writeBytes("     "+padSTRING('R',"Stores Type : "+cl_dat.M_cmbSBSL2_pbst.getSelectedItem().toString(),strDOTLN.length()-25));
				dosREPORT.writeBytes("Page No : "+cl_dat.M_PAGENO+"\n");			
	...
	private void getTBLDT()
	{
		try
		{
			M_strSQLQRY = "SELECT ST_MATCD,ST_MATDS,ST_UOMCD,ST_STKQT,ST_STKIP "
				+"FROM MM_STMST WHERE ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND SUBSTRING(ST_MATCD,1,6) in ('690520','691010') AND isnull(ST_STKQT,0) > 0 AND  ST_STSFL <> 'X' AND "
			...
			try
			{
				if(input == txtFMGRP || input == txtTOGRP )
				{
					if(((JTextField)input).getText().length() == 0)
						return true;
					ResultSet L_rstTEMP = cl_dat.exeSQLQRY3("Select count(*) from MM_STMST where ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_MATCD LIKE '"+((JTextField)input).getText()+"%' AND ST_MMSBS = '"+M_strSBSCD+"'");
					if(L_rstTEMP != null)
					{
						if(L_rstTEMP.next())
						{
				...
				else if(input == txtFMSUB || input == txtTOSUB )
				{
					if(((JTextField)input).getText().length() == 0)
						return true;
					ResultSet L_rstTEMP = cl_dat.exeSQLQRY3("Select count(*) from MM_STMST where ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND SUBSTRING(ST_MATCD,1,6) ='"+((JTextField)input).getText()+"' AND ST_MMSBS = '"+M_strSBSCD+"'");
					if(L_rstTEMP != null)
					{
						if(L_rstTEMP.next())
						{
				...
				if(input == txtFMLOC || input == txtTOLOC)
				{
					if(((JTextField)input).getText().length() == 0)
						return true;
					ResultSet L_rstTEMP = cl_dat.exeSQLQRY0("Select count(*) from MM_STMST where ST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND ST_LOCCD = '"+((JTextField)input).getText()+"'");
					if(L_rstTEMP != null)
					{
						if(L_rstTEMP.next())
						{
