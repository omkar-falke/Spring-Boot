...
class mm_rpiss extends cl_rbase 
{
	...
	private Hashtable<String,String> hstDPTCD; /** Hashtable to Store Material Type & associated description.*/
	private Hashtable<String,String> hstMATTP; /** Hashtable to Store Excise Ctegory Code & associated description.*/
	private Hashtable<String,String> hstEXCCT;			/** FileOutputStream for generated report file handeling.*/
	...
	mm_rpiss()
	{
		...
		try
		{
			...
			hstEXCCT = new Hashtable<String,String>();
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		else if(L_KE.getKeyCode()==L_KE.VK_F1 && M_objSOURC == txtDPTSL)
		{
			...
			M_strSQLQRY = "SELECT CMT_CODCD,CMT_CODDS FROM CO_CDTRN "
					 +"WHERE CMT_CGMTP = 'SYS' AND CMT_CGSTP = 'COXXDPT' and isnull(cmt_stsfl,' ') <>'X'"+L_strADDQR+" ORDER BY CMT_CODDS";
			...
			if(rdbDPTAL.isSelected())
			{
				if(rdbISSNO.isSelected())
				{
					M_strSQLQRY = "SELECT IS_ISSDT,IS_ISSNO,IS_MATCD,IS_REQQT,IS_ISSQT,IS_DPTCD, "
						...
						+"IS_MATCD = CT_MATCD AND isnull(is_stsfl,' ') <>'X' and "
						+"CONVERT(varchar,IS_AUTDT,101) BETWEEN '"
					...
					if(chbOBSOL.isSelected())
						M_strSQLQRY +=" AND isnull(CT_STSFL,'') ='9' "; 
			...
			if(rdbDPTSP.isSelected())
			{
				...
				if(rdbISSNO.isSelected())
				{
					M_strSQLQRY = "SELECT IS_ISSDT,IS_ISSNO,IS_MATCD,IS_REQQT,IS_ISSQT,IS_DPTCD, "
						...
						+"isnull(IS_STSFL,' ') <>'X' and CONVERT(varchar,IS_AUTDT,101) BETWEEN '"
						+M_fmtDBDAT.format(M_fmtLCDAT.parse(txtFRDAT.getText()))+"' AND '"
						+M_fmtDBDAT.format(M_fmtLCDAT.parse(txtTODAT.getText()))+"' ";
					if(cmbEXCCT.getSelectedIndex() >0)
						M_strSQLQRY +=" AND IS_EXCCT ='"+strEXCCT +"'"; 
