...
class co_teau1 extends cl_pbase implements ChangeListener
{
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		else if(L_KE.getKeyCode() == L_KE.VK_F1)
		{
			if(M_objSOURC == txtLOTNO)
			{
        	    ...
        	    M_strSQLQRY = "select LT_LOTNO,LT_RCLNO,LT_PSTDT,PR_PRDDS,CMT_SHRDS,LT_CLSFL from PR_LTMST,CO_PRMST,co_cdtrn where LT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and LT_PRDTP = '";
    		...
    		else if(L_KE.getSource().equals(txtRCLNO))
			{
				cl_dat.M_flgHELPFL_pbst = true;
			    M_strSQLQRY = " ";
			    M_strSQLQRY = "select LT_RCLNO,PR_PRDDS from PR_LTMST,CO_PRMST where LT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and LT_PRDCD = PR_PRDCD AND LT_LOTNO = ";
			...
			else if(M_objSOURC == txtPTFCT)
			{
        		...
        		M_strSQLQRY += " and CMT_CGSTP = 'FGXXCAT' and SUBSTRING(CMT_CODCD,1,4)='PTF_' and SUBSTRING(CMT_CODCD,5,2) = '01'";//and cmt_modls like '%"+cl_dat.M_strUSRTP_pbst+"%'";
...
private boolean getLOTDET()
{
	...
	try
	{
	    strMSG ="";
         ...
         +" LT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and LT_PRDCD = PR_PRDCD and LT_PRDCD = QP_PRDCD and QP_QCATP = "+ "'"+cmbPRDTP.getSelectedItem().toString().substring(0,2) + "'"
...
private void getALLREC()
{
    try
    {
	   ...
	   M_strSQLQRY +=" from PR_LTMST,CO_QPMST where LT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and LT_CMPCD = QP_CMPCD and LT_PRDTP = '";
...
//	M_strSQLQRY += " AND RM_STSFL <> 'X'";
	...
	private class co_teau1TBLVR extends TableInputVerifier
	{
...
private boolean getACLST(String LP_PRDTP,String LP_LOTNO,String LP_RCLNO)
{
    try
    {
        ...
        M_strSQLQRY = "select QP_QPRCD,QP_NPFVL,QP_NPTVL,QP_CMPFL,QP_TXTVL,A.* from  "
                      +" PR_LTMST,CO_QPMST,QC_PSMST A where LT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and LT_TPRCD = QP_PRDCD AND LT_LOTNO = PS_LOTNO AND LT_RCLNO = PS_RCLNO "
...
private boolean updRMK()
{
	...
	try
	{
		...
		M_strSQLQRY += " RM_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and RM_QCATP = '"+cmbPRDTP.getSelectedItem().toString().substring(0,2)+"' AND RM_TSTTP  ='"+strCLSTP_fn+"' and RM_TSTNO ='"+txtLOTNO.getText().trim()+"' ";
	   ...
	   // if(( L_RECCNT > 0)&&(!LM_CLSFL.equals("9")))
		...
		else if((L_RECCNT == 0)&&(!txtREMDS.getText().trim().equals("")))
		{
		     ...
		     M_strSQLQRY +="'"+cl_dat.M_strCMPCD_pbst+"','"+cmbPRDTP.getSelectedItem().toString().substring(0,2).trim()+"',";
...
private void dspLOTDT()
{
	...
	try
	{
		...
		M_strSQLQRY = "Select ST_MNLCD,ST_STKQT,ST_RESNO from fg_STMST where st_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and st_STKQT >0 and ST_RCLNO = '" +strLSTRCL +"'";
...
public void getQPDTL(String P_strPRDCD)
{
	 try
     {
          ...
          if(LM_CMPFL !=null)
    		  LM_CMPFL.removeAllElements();
          M_strSQLQRY = "Select QP_QPRCD,QP_QPRDS,QP_UOMDS,QP_NPFVL,QP_NPTVL,QP_CMPFL,QP_ORDBY from CO_QPMST where QP_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and QP_QCATP = '" + cmbRCPRD.getSelectedItem().toString().substring(0,2).trim() + "' and QP_TSTTP = '"+strCMPTP_fn+"'";
...
private void exeLOTRCL()
{
	try
	{
    	...
    	strRCLDOC = cl_dat.getPRMCOD("CMT_CCSVL","D"+cl_dat.M_strCMPCD_pbst,"QCXXGCH",L_strCODCD);
    	...
    	M_strSQLQRY +=" WHERE CMT_CGMTP = 'D"+cl_dat.M_strCMPCD_pbst+"' and CMT_CGSTP ='QCXXGCH' and CMT_CODCD ='"+L_strCODCD.trim()+"'";
    	cl_dat.exeSQLUPD(M_strSQLQRY,"setLCLUPD");
    	...
    	M_strSQLQRY += " From PR_LTMST where LT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and LT_RCLNO = '"+strLSTRCL +"'";
    ...
    if(flgTSTCP)
    {
		...
		M_strSQLQRY +=" SELECT ";
		M_strSQLQRY +="'"+cl_dat.M_strCMPCD_pbst+"','"+cmbRCPRD.getSelectedItem().toString().substring(0,2)+"',";
    	...
    	if(cl_dat.M_flgLCUPD_pbst)
    	{
            ...
            M_strSQLQRY += "LT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and LT_RCLNO = '"+strLSTRCL.trim() +"'";
            M_strSQLQRY += " AND LT_LOTNO = '"+txtRCLOT.getText().trim() +"'";
...
private void genRCEML()
{
	...
	fosREPORT.close();
	cl_eml ocl_eml = new cl_eml();
	M_strSQLQRY="Select CMT_CODDS from CO_CDTRN where CMT_CGMTP='EML' and CMT_CGSTP='QCXXGCH'";
	M_rstRSSET = cl_dat.exeSQLQRY(M_strSQLQRY);
	if(M_rstRSSET !=null)
	while(M_rstRSSET.next())	
	{
...
private void getPTFREC()
{
	try
	{
		...
		if(strPTFCT.equals("01"))
		{
			...
			M_strSQLQRY += "sum(ST_UCLQT) L_UCLQT from FG_STMST,PR_LTMST where ST_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and ST_PRDTP=LT_PRDTP and ST_CMPCD = LT_CMPCD and ST_LOTNO=LT_LOTNO";
		...
		{
			M_strSQLQRY = "Select ST_WRHTP,ST_PRDTP,ST_LOTNO,ST_RCLNO,ST_TPRCD,LT_PRDCD,ST_PKGTP,LT_CLSFL,LT_REMDS,";
			M_strSQLQRY += "sum(ST_UCLQT) L_UCLQT from FG_STMST,PR_LTMST where ST_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and ST_PRDTP=LT_PRDTP";
			M_strSQLQRY += " and ST_CMPCD = LT_CMPCD and ST_LOTNO=LT_LOTNO and ST_RCLNO=LT_RCLNO and ST_RCLNO > '00' and ST_UCLQT > 0";
...
private void genPTFREC()
{
	try
	{
		...
		if(strPTFCHP.trim().equals(strDGRTP))
		{
			...
			M_strSQLQRY += " and CMT_CODCD = '"+strPTFCT+"'";
			cl_dat.exeSQLUPD(M_strSQLQRY,"setLCLUPD");   
		...
		{			
			...
			{	
				...
				{	
    				...
    				M_strSQLQRY += "ST_LUSBY = '"+cl_dat.M_strUSRCD_pbst+"',";
    				M_strSQLQRY += "ST_LUPDT = '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_strLOGDT_pbst))+"'";
    				M_strSQLQRY += " where ST_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and ST_WRHTP = '"+L_strWRHTP+"'";
    				...
    				M_strSQLQRY += "'"+cl_dat.M_strCMPCD_pbst+"','"+strPTFNO+"',";
