...
//				tblSCTDL.cmpEDITR[i].addKeyListener(this);
			...
			M_strSQLQRY="Select CMT_CODCD,CMT_CODDS from CO_CDTRN"
						+" where CMT_CGMTP='MST' and CMT_CGSTP='COXXPRT' "+(cl_dat.M_strUSRCT_pbst.equals("02") ? " and cmt_codcd = 'C'" : "");
		...
		try
		{
            if(intKEYVL==L_KE.VK_F1)
			{
				...
				else if(M_objSOURC==txtPRTCD)
				{
					...
					if(!(txtPRTCD.getText().length()==1))
						return;
					M_strSQLQRY = "SELECT PT_PRTNM,PT_PRTCD,PT_ADR01,PT_ADR02,PT_ADR03 from CO_PTMST where  PT_PRTTP='"+cmbPRTTP.getSelectedItem().toString().substring(0,1)+"' and SUBSTRING(PT_PRTCD,1,1)= '"+txtPRTCD.getText().substring(0,1)+"' and SUBSTRING(PT_PRTCD,2,3) not in ('777','888') and PT_STSFL<>'X'";
					if(cl_dat.M_strUSRCT_pbst.equals("02") && cl_dat.M_strUSRCD_pbst.substring(0,1).equals("D"))
						M_strSQLQRY +=   " and PT_DSRCD = '"+cl_dat.M_strUSRCD_pbst.substring(1,6)+"' " ;
					else if(cl_dat.M_strUSRCT_pbst.equals("02") && cl_dat.M_strUSRCD_pbst.substring(0,1).equals("G"))
						M_strSQLQRY +=   " and PT_CNSRF = '"+cl_dat.M_strUSRCD_pbst.substring(1,6)+"' " ;
            ...
            else if(intKEYVL==9 || intKEYVL==L_KE.VK_ENTER)
			{
				...
				else if(M_objSOURC==txtCNSRF)
				{
					...
					{
						M_strSQLQRY="Select PT_PRTNM From CO_PTMST Where PT_PRTCD='"+txtCNSRF.getText().trim().toUpperCase()+"'"
							+" And PT_PRTTP='G'";
						M_rstRSSET=cl_dat.exeSQLQRY(M_strSQLQRY);
						if(M_rstRSSET.next())
						{
				...
				{
					if(txtDSTCD.getText().trim().length()!=3)
					{
    					    setMSG("Invalid Destination..",'E');
    					    txtDSTCD.requestFocus();
    					    return;
					}
					...
					else
					{
						...
						if(L_SELOPT == 0)
						{
					    	...
					    	add(new JLabel("Dest. Code"),1,1,1,1,pnlDEST,'L');
		                	add(txtDESCD=new TxtLimit(5),2,1,1,1,pnlDEST,'L');
					    	add(new JLabel("Dest. Name"),1,2,1,2,pnlDEST,'L');
		                	add(txtDESTN=new TxtLimit(30),2,2,1,2,pnlDEST,'L');
		                	add(new JLabel("Lead Time"),1,4,1,1,pnlDEST,'L');
		                	add(txtLEDTM=new TxtNumLimit(2.0),2,4,1,1,pnlDEST,'L');
						    System.out.println("Add destination");
						    /*JDialog dialog = new JDialog();
						    dialog.getContentPane().add(pnlDEST);*/
						    pnlDEST.setSize(700,200);
					        pnlDEST.setPreferredSize(new Dimension(700,100));
        				...
        				//	System.out.println(L_intOPTN);
        					...
        					if(txtDESTN.getText().trim().length() ==0)
        					{
        				        setMSG("Destination Name can not be blank..",'E');
        				        txtDSTCD.setText("");
        				        return;
        					}
        					else if(txtDESCD.getText().trim().length() ==0)
        					{
        				        setMSG("Destination Code can not be blank..",'E');
        				        return;
        					}
        					else if(txtLEDTM.getText().trim().length() ==0)
        					{
        					...
        					if(M_rstRSSET.next())
        					{
        					    ...
        					    {
        					        setMSG("Destination"+L_strDSTNM+ " already exists ...",'E');
        				            return;
        					    }
        					}
        					...
        					if(M_rstRSSET.next())
        					{
        					    if(M_rstRSSET.getInt("L_CNT") >0)
        					    {
        					        setMSG("Code  "+L_strDSTCD+" exist, enter some other code..",'E');
        				            return;
        					    }
        					}
	...
	*/
	void vldCAPQT()
		...
		private boolean vldPRTCD()
		{
			...
			if(cl_dat.M_strUSRCT_pbst.equals("02") && cl_dat.M_strUSRCD_pbst.substring(0,1).equals("D"))
				strWHRSTR +=  " and PT_DSRCD = '"+cl_dat.M_strUSRCD_pbst.substring(1,6)+"' " ;
			else if(cl_dat.M_strUSRCT_pbst.equals("02") && cl_dat.M_strUSRCD_pbst.substring(0,1).equals("G"))
				strWHRSTR +=  " and PT_CNSRF = '"+cl_dat.M_strUSRCD_pbst.substring(1,6)+"' " ;
	...
	try
	{
		...
		if(!flgCHK_EXIST)
		{
			...
			+"PT_COWEB,PT_EMLPR,PT_EMLMR,PT_EMLAC,PT_DSRCD,PT_CNSRF,PT_GRPCD,"
