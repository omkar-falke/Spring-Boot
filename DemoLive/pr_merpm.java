...
*/
class pr_merpm extends cl_pbase implements ActionListener, FocusListener
{
	...
	private cl_JTable		tblPSDTL;/**DETAILS OF POLYSTYRENE TO BE ADDED TO THE BATCH*/
	private cl_JTable			tblSTFNL;/**RECIPES OF TRIALS CARRIED OUT*/
	private cl_JTable			tblRECIP;/**RECIPE TABLE*/
	private cl_JTable		tblFDRQT;
//				tblRCLQT;/**DETAILS OF RECYCLE MATERIAL AVAILABLE*/
	private cl_JTable			tblRFRCP;/**JTable for ref. recipies */
...
//CREATING TABLE FOR RECIPE
		...
		tblRECIP=crtTBLPNL1(ptemp,names,LM_RCPROW,1,1,1,1,wid,new int[]{0,10}) ;
		//tblRECIP=crtTBLPNL1(ptemp,names,LM_RCPROW,1,1,1,1,wid,new int[]{0,10});
		tblRECIP.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
		((cl_JTable)tblRECIP).clrTABLE();
		((cl_JTable)tblRECIP).setCellEditor(1,txtMATCD = new TxtLimit(10));
		TxtLimit t2=new TxtLimit(45);t2.setEnabled(false);
		((cl_JTable)tblRECIP).setCellEditor(2,t2);
		((cl_JTable)tblRECIP).setCellEditor(3,new TxtLimit(40));
		((cl_JTable)tblRECIP).setCellEditor(4,new TxtLimit(10));
		((cl_JTable)tblRECIP).setCellEditor(5,new TxtNumLimit(8.5));
		((cl_JTable)tblRECIP).setCellEditor(9,new TxtNumLimit(1.0));
		TxtNumLimit t1=new TxtNumLimit(9.6);t1.setEnabled(false);
		((cl_JTable)tblRECIP).setCellEditor(6,t1);
//ADDING KEY LISTENER TO EDITOR COMPONENTS OF JTBL		
		for (int i=0;i<((cl_JTable)tblRECIP).cmpEDITR.length;i++)
		{
			((JComponent)((cl_JTable)tblRECIP).cmpEDITR[i]).addKeyListener(this);
			((JComponent)((cl_JTable)tblRECIP).cmpEDITR[i]).addFocusListener(this);	
		}
		((cl_JTable)tblRECIP).setCellEditor(8,cmbUOMCD);
...
//ADDING ALL COMPONENTS
		...
		add(new JLabel("Batch No./Count"),4,3,1,1,this,'L');
		add(txtBATNO,4,4,1,0.5,this,'L');
		add(txtBATCT=new TxtNumLimit(2.0),4,4,1,0.5,this,'R');
		...
		tblPSDTL=crtTBLPNL1(ptemp,names,5,1,1,1,1,wid,new int[]{0}) ;
...
//CREATING TABLE FOR FEEDER QTY DETAILS
		...
		tblFDRQT=(cl_JTable)crtTBLPNL1(ptemp,names,4,1,1,1,1,wid,new int[]{0}) ;
...
//				tblRFRCP=null;
				...
				row=0;
				((cl_JTable)tblRECIP).clrTABLE();
				((cl_JTable)tblRECIP).setEnabled(true);
				((cl_JTable)tblRECIP).cmpEDITR[2].setEnabled(false);
				((cl_JTable)tblRECIP).cmpEDITR[6].setEnabled(false);
				((cl_JTable)tblRECIP).cmpEDITR[7].setEnabled(false);
				if(M_rstRSSET!=null)
				{
					double L_BSMQT=0.0;
					while(M_rstRSSET.next())
			...
			{//SET FINAL RECIPE FOR THE RUN
				...
				((cl_JTable)tblRECIP).setEnabled(false);
			...
			{
				//CHECKS WHETHER RUN START TIME HAS BEEN EDITED. IF YES, CONFIRMS AND THEN PROCEEDS FOR NEW RUN.
				if(!(txtRNSDT.getText()+txtRNSTM.getText()).equals(strRNSTM))
				{
					int option=JOptionPane.showConfirmDialog(null,"Do you want to schedule new Run ?","Confirm ..",JOptionPane.YES_NO_OPTION);
					requestFocus();
					if(option!=1)
					{
						//System.out.println("call to exeNWRUN1");
						exeNWRUN();//STARTING NEW RUN
					}
					txtPRAQT.requestFocus(); // added on 19/08/2005
				}
				else
				    txtPRAQT.requestFocus(); // added on 19/08/2005
			}
			...
			{//CHECKS WHETHER GRADE CODE HAS BEEN EDITED. IF YES, CONFIRMS AND THEN PROCEEDS FOR NEW RUN.
				if(!txtGRDCD.getText().equals(LM_GRDCD))
				{
					...
					if(option!=1)
					{
					    //System.out.println("call to exeNWRUN2");
						exeNWRUN();//STARTING NEW RUN
					}
					else txtRNSDT.requestFocus(); // added on 19/08/05
				}
			}
	...
	*///ADDING RECYCLE QTY TO FEEDER QTY
	...
	private void exeNWRUN()
	{
		...
		//txtRNSTM.setText("");txtRNSDT.setText(""); // changed on 19/08/05
		txtPRTQT.setText("");txtPSPWD.setText("");txtTRLCT.setText("1");
...
//GET OLD RUNS OF SAME GRADE, IRRESPECTIVE OF LINE NO.
		...
		//System.out.println("exeNEW RUN "+M_strSQLQRY);
        M_rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY );
		try
		{
			if(M_rstRSSET.next())
...
//GET LIST OF RAW MATERIALS USED IN PREVIOUS RUNS				
				...
				Vector<String> vtrMTLCD=new Vector<String>(10,2);
				if(L_RSSET!=null)
				{
					while(L_RSSET.next())
					{
                ...
                //System.out.println("vtrmtlcd "+vtrMTLCD.size());
...
//SET COL HDR TO RAW MATERIALS NAME USING VCTOR				
				...
				int h1=JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS;
				int v1 = JScrollPane.VERTICAL_SCROLLBAR_ALWAYS;
				tblRFRCP=((cl_JTable)crtTBLPNL1(pnlRECIP=new JPanel(null),names,10,1,1,4,4,wid,new int[]{0}));
...
//FIND OUT COL TO ADD THE VALUE					
				...
				if(!L_EOF)
				{
					...
					dlgRFRCP.toFront();
					//dlgRFRCP.show();	deprecated in 1.6
					dlgRFRCP.setVisible(true);
...
//GENERATING RUN NUMBER
	...
	//		{
				...
				{
					//System.out.println("2-2 +first run");
					txtRUNNO.setText(L_STRTMP+"00001");
					setMSG("First Run for the Grade ..",'N');
					txtTRLCT.setText("1");
				}
	...
	*/		((cl_JTable)tblRECIP).setEnabled(true);
			((cl_JTable)tblRECIP).cmpEDITR[0].setEnabled(false);
			((cl_JTable)tblRECIP).cmpEDITR[2].setEnabled(false);
			((cl_JTable)tblRECIP).cmpEDITR[6].setEnabled(false);
			((cl_JTable)tblRECIP).cmpEDITR[7].setEnabled(false);
...
//SET COL HDR TO RAW MATERIALS NAME USING VECTOR				
				...
				int h1=JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS;
				int v1 = JScrollPane.VERTICAL_SCROLLBAR_ALWAYS;
				tblRFRCP=((cl_JTable)crtTBLPNL1(pnlRECIP=new JPanel(null),names,15,1,1,8,4,wid,new int[]{0}));
				tblRFRCP.setCellEditor(0,chbRFRCP);
				tblRFRCP.addKeyListener(this);
				String L_STRTMP="";
				int L_RPPOS=-1,L_LINNO=0;
...
//FIND OUT COL TO ADD THE VALUE					
	...
	{
		//((cl_JTable)tblRECIP).dceEDITR[tblRECIP.getSelectedColumn()].stopCellEditing();
		if(tblRECIP.isEditing())
		    tblRECIP.getCellEditor().stopCellEditing(); // changed on 19/08/2005
...
//VALIDATING FEEDER NO.s
		...
		if(flag)
		{
			...
			else
			{
				...
				{
				    if(total != 0.0) // added on 19/08/05
...
//CALCULATING FEEDER WISE QTY.
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		{
			M_strSQLQRY="SELECT PR_PRDDS,PR_PRDCD FROM CO_PRMST ";
			if(txtGRDCD.getText().length() >0)
			    M_strSQLQRY+= " WHERE PR_PRDCD like '"+txtGRDCD.getText().trim()+"%'";
			M_strSQLQRY+= " ORDER BY PR_PRDDS";
			M_strHLPFLD="txtGRDCD";
			cl_hlp(M_strSQLQRY ,1,1,new String[] {"Grade Name","Grade Code"},2,"CT");
		}
		else if(M_objSOURC==((cl_JTable)tblRECIP).cmpEDITR[1]&&key==L_KE.VK_F1)
		{
			M_strSQLQRY="SELECT CT_MATCD,CT_MATDS FROM CO_CTMST ";
			if(txtMATCD.getText().length() >0)
			    M_strSQLQRY+= " where CT_GRPCD in('68','51') AND ct_matcd like '"+txtMATCD.getText()+"%'";
			else
			    M_strSQLQRY+=" where ct_grpcd='68' or ct_grpcd='51' ";
...
/*		else if(M_objSOURC==txtPSRCL&&key==L_KE.VK_F1)
		{
			...
			int h1=JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS;
			int v1 = JScrollPane.VERTICAL_SCROLLBAR_ALWAYS;
			tblRCLQT=(cl_JTable)crtTBLPNL(pnlRCLQT,names,LM_RCPROW,0,25,M_intCOLWD*4,150,wid,new int[]{0,9}) ;
...
//			tblRFRCP=null;
		...
		else if(M_objSOURC==txtGRDCD&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    txtRNSDT.requestFocus();
		}
		else if(M_objSOURC==txtRNSDT&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    txtRNSTM.requestFocus();
		}
		else if(M_objSOURC==txtRNSTM&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    txtPRAQT.requestFocus();
		}
		else if(M_objSOURC==txtBATSZ&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    txtBATCT.requestFocus();
		}
        else if(M_objSOURC==txtBATCT&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    txtLOTNO.requestFocus();
		}
		else if(M_objSOURC==txtLOTNO&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    cmbCMRTP.requestFocus();
		}
		else if(M_objSOURC==cmbCMRTP&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    txtCMRNO.requestFocus();
		}
		else if(M_objSOURC==txtCMRNO&&L_KE.getKeyCode()==L_KE.VK_ENTER)
		{
		    txtCMRRV.requestFocus();
		}
		}catch (Exception e)
		{System.out.println(e);}
	}
	...
	public void focusGained(FocusEvent L_FE)
	{
		...
		if(!cl_dat.M_txtSTAT_pbst.getForeground().equals(Color.red))
		{
			...
			else if(M_objSOURC==((cl_JTable)tblRECIP).cmpEDITR[1])
				setMSG("Enter Material Code, Press 'F1' for list ..",'N');
			else if(M_objSOURC==((cl_JTable)tblRECIP).cmpEDITR[5])
				setMSG("Enter PHR quantity 'ddd.ddddd' ..",'N');
			else if(M_objSOURC==((cl_JTable)tblRECIP).cmpEDITR[8])
	...
	public void exeHLPOK()
	{
		try
		{
			...
			else if(M_strHLPFLD.equals("cmpEDITR[1]"))
			{
				...
				super.exeHLPOK();
				StringTokenizer L_STRTKN=new StringTokenizer(cl_dat.M_strHELP_pbst,"|");
				((JTextField)((cl_JTable)tblRECIP).cmpEDITR[1]).setText(L_STRTKN.nextToken());
...
//VALIDATING FEEDER NO.s
	...
	private void exeENQRY() throws Exception
	{
		...
		if(M_rstRSSET.next())
		{
			...
			{
				btnSTFNL.setVisible(false);
				txtBTACT.setText(Integer.toString(Integer.parseInt(M_rstRSSET.getString("RN_BATCT"))));
				txtTRLCT.setText(Integer.toString(Integer.parseInt(M_rstRSSET.getString("RN_TRLCT"))));
				((cl_JTable)tblRECIP).setEnabled(false);
				((cl_JTable)tblRECIP).cmpEDITR[8].setEnabled(true);((cl_JTable)tblRECIP).cmpEDITR[9].setEnabled(true);
			}
			txtGRDCD.setText(M_rstRSSET.getString("RN_GRDCD"));
		...
		strRNSTM=txtRNSDT.getText()+txtRNSTM.getText();
			...
			if(((String)cl_dat.M_cmbOPTN_pbst.getSelectedItem()).equals(cl_dat.M_OPADD_pbst))
			{
				if(LM_GRDCD.equals(txtGRDCD.getText()))
				{
					M_strSQLQRY="UPDATE pr_RNMST SET "
						...
						+"RN_TOTPM="+(btnSTFNL.isVisible()==true ? "0" : (" RN_TOTPM +"+Double.toString((Double.parseDouble(txtBATSZ.getText())/1000.0)*Double.parseDouble(nvlSTRVL(txtBATCT.getText(),"1")))+" "))+" ,"//RN_TOTPM
						...
						+"RN_BATCT= "+nvlSTRVL(txtBTACT.getText(),"0")+"+"+nvlSTRVL(txtBATCT.getText(),"1")+","//RN_BATCT,
				...
				cl_dat.exeSQLUPD(M_strSQLQRY ,"setLCLUPD");
				for(int j=0;j<Integer.parseInt( nvlSTRVL(txtBATCT.getText(),"1"));j++)
				{
	...
	//INSERTING INTO pr_BTMST (BATCH MASTER)
						M_strSQLQRY="INSERT INTO pr_BTMST (BT_BATNO,BT_GRDCD,BT_RUNNO,BT_BATDT,BT_BATSZ,BT_TRLFL,BT_TRNFL,BT_STSFL,BT_LUSBY,BT_LUPDT) VALUES ("
							+""+txtBATNO.getText()+"+"+Integer.toString(j)+","//RP_BATNO,
	...
	//ADDING DATA TO BATCH TRANSACTION TABLE 							
							M_strSQLQRY="INSERT INTO pr_BTTRN (BTT_BATNO,BTT_GRDCD,BTT_RUNNO,BTT_MTLDS,BTT_MTLMF,BTT_MTLCD,BTT_MTLBT,BTT_MTLQT,BTT_BSMFL,BTT_TRNFL,BTT_STSFL,BTT_LUSBY,BTT_LUPDT) VALUES ("
								+""+nvlSTRVL(txtBATNO.getText(),"0")+"+"+Integer.toString(j)+","//BTT_BATNO
	...
	void setENBL(boolean L_STAT)
	{
		...
		tblRECIP.setEnabled(L_STAT);((cl_JTable)tblRECIP).cmpEDITR[6].setEnabled(false);btnPRINT.setEnabled(false);
		((cl_JTable)tblRECIP).cmpEDITR[7].setEnabled(false);cmbUOMCD.setEnabled(true);
		((cl_JTable)tblRECIP).cmpEDITR[2].setEnabled(false);((cl_JTable)tblRECIP).cmpEDITR[8].setEnabled(true);
		if(btnSTFNL.isVisible())
			txtBATCT.setEnabled(false);
		else
			txtBATCT.setEnabled(true);
	}
	void clrCOMP()
	{
		RCLQT=0.0;
...
//			outDTSTR.writeBytes(padSTRING('C',"RECIPE AUTHORISATION SHEET\n",80));
			outDTSTR.writeBytes("\n_________________________________________________________________________________________\n");
			outDTSTR.writeBytes("|Production Type : "+padSTRING('R',(btnSTFNL.isVisible()==true ? "Trial" : "Production"),25)+"Lot No. : "+padSTRING('L',txtLOTNO.getText(),10)+"   Grade : "+padSTRING('L',txtGRDDS.getText(),13)+"|\n");
			if(Integer.parseInt( nvlSTRVL(txtBATCT.getText(),"1"))>1)
				outDTSTR.writeBytes("|Batch No.       : "+padSTRING('R',(btnSTFNL.isVisible()==true ? txtTRLCT.getText() : txtBATNO.getText()+" - "+Integer.toString(Integer.parseInt(txtBATNO.getText())+Integer.parseInt(txtBATCT.getText()))),25)+"Date    : "+padSTRING('L',cl_dat.M_strLOGDT_pbst,10)+"   Line : "+padSTRING('L',txtRUNNO.getText().substring(0,2),14)+"|\n");
			else
				outDTSTR.writeBytes("|Batch No.       : "+padSTRING('R',(btnSTFNL.isVisible()==true ? txtTRLCT.getText() : txtBATNO.getText()),25)+"Date    : "+padSTRING('L',cl_dat.M_strLOGDT_pbst,10)+"   Line : "+padSTRING('L',txtRUNNO.getText().substring(0,2),14)+"|\n");
			outDTSTR.writeBytes("|Batch Size      : "+padSTRING('R',txtBATSZ.getText(),45)+"   Run :"+padSTRING('L',txtRUNNO.getText(),16)+"|\n");
			outDTSTR.writeBytes("|_______________________________________________________________________________________|\n");
...
/*			outDTSTR.writeBytes("OPERATING CONDITIONS                                                    |     | 10 |      | \n");
		...
		*/	outDTSTR.writeBytes("REMARKS :                                                              |     | 10 |     |  \n");
			...
			if(Integer.parseInt( nvlSTRVL(txtBATCT.getText(),"1"))>1)
			{
				outDTSTR.writeBytes("Batches Completed : ");
				for(int j=0;j<Integer.parseInt( nvlSTRVL(txtBATCT.getText(),"1"));j++)
					outDTSTR.writeBytes(Integer.toString(Integer.parseInt(nvlSTRVL(txtBATNO.getText(),"0"))+j)+"   ");
			}
