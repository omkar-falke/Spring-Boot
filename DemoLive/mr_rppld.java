...
Program Name  : Discrepancies Party Ledger
Program Desc. : This program take report for discrepancise party ledger 
...
class mr_rppld extends cl_rbase
{			
	...
	public void exePRINT()
	{
		...
		try
		{
			...
			if(M_rdbHTML.isSelected())
		        strFILNM = cl_dat.M_strREPSTR_pbst +"mr_rppld.html";
		    else if(M_rdbTEXT.isSelected())
		        strFILNM = cl_dat.M_strREPSTR_pbst + "mr_rppld.doc";
			seldata();
			fosREPORT.close();
			dosREPORT.close();
			if(intRECCT == 0)
	...
	private void seldata()
	{
		...
		if(M_rdbHTML.isSelected())
		{
		    dosREPORT.writeBytes("<HTML><HEAD><Title>Discrepancise Party Ledger </title> </HEAD> <BODY><P><PRE style =\" font-size : 10 pt \">");    
			dosREPORT.writeBytes("<STYLE TYPE=\"text/css\"> P.breakhere {page-break-before: always}</STYLE>"); 
		}
		int i=0;
		M_strSQLQRY="Select distinct PL_PRTTP,PL_PRTCD,PT_PRTNM from MR_PLTRN,CO_PTMST  where PL_PRTTP=PT_PRTTP AND PL_PRTCD=PT_PRTCD AND PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND PL_DOCDT ";
		M_strSQLQRY += " BETWEEN '"+ M_fmtDBDAT.format(M_fmtLCDAT.parse(L_strFMDAT))+"' AND '"+ M_fmtDBDAT.format(M_fmtLCDAT.parse(strTODAT))+"'  order by PL_PRTTP,PL_PRTCD ";
		...
		if(L_rstRSSET!=null)
		{
			...
			{
				L_strPRTTP=L_rstRSSET.getString("PL_PRTTP");
				L_strPRTCD=L_rstRSSET.getString("PL_PRTCD");
				strPRTNM=L_rstRSSET.getString("PT_PRTNM");
				System.out.println(i +"= "+L_strPRTTP +",  "+L_strPRTCD);
				getDATA(L_strPRTTP,L_strPRTCD);
				i++;
			}
	...
	private void getDATA(String P_strPRTTP,String P_strPRTCD)
	{
		try
		{
			boolean flgTEMP=true;
			boolean L_flgBRK = false;
			double L_dblBALVL =0;
			...
			M_strSQLQRY="Select PT_YOPVL PRTBL,PT_YOPFL BALFL,PT_YSTDT,PT_YTDVL CURBL,PT_YTDFL CURFL from CO_PTMST where PT_PRTTP ='"+P_strPRTTP+"'";
			M_strSQLQRY +=" AND PT_PRTCD='"+P_strPRTCD.toUpperCase()+"'"; 
			...
			if(M_rstRSSET!=null && M_rstRSSET.next())
			{
				...
				L_strCURBL=nvlSTRVL(M_rstRSSET.getString("CURBL"),"0.0");
				L_strCURFL=nvlSTRVL(M_rstRSSET.getString("CURFL"),"");
		...
		//	System.out.println("DATE 1 ="+L_datTMPDT);
			...
			if(flgTEMP)
			{
				M_strSQLQRY ="Select PL_SEQNO from MR_PLTRN where PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND PL_PRTTP='"+P_strPRTTP+"'";
				M_strSQLQRY +=" AND PL_PRTCD='"+P_strPRTCD.toUpperCase()+"' AND PL_DOCDT >='"+M_fmtDBDAT.format(M_fmtLCDAT.parse(L_strFMDAT))+ "' and isnull(PL_STSFL,'')<> 'X' order by PL_SEQNO"; 
				...
				{
					intSEQNO=intSEQNO-1;
					M_strSQLQRY="Select PL_BALVL PRTBL,PL_BALFL  BALFL from MR_PLTRN where PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND PL_PRTTP ='"+P_strPRTTP+"' AND PL_SEQNO ="+intSEQNO+"";
					M_strSQLQRY +=" AND PL_PRTCD='"+P_strPRTCD.toUpperCase()+"'  AND isnull(PL_STSFL,'')<> 'X'"; 
			...
			M_strSQLQRY +="where PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND PL_PRTTP ='"+P_strPRTTP+"'";
			M_strSQLQRY +=" AND PL_PRTCD='"+P_strPRTCD.toUpperCase()+"' ";
			M_strSQLQRY +=" AND PL_DOCDT BETWEEN '"+ M_fmtDBDAT.format(M_fmtLCDAT.parse(L_strFMDAT))+"' AND '"+ M_fmtDBDAT.format(M_fmtLCDAT.parse(strTODAT))+"'  ";
			...
			while(M_rstRSSET.next())
			{
				...
				if(L_strDOCTP.equals("2")||L_strDOCTP.equals("3"))
				{
				...
				{
					dosREPORT.writeBytes(padSTRING('R',P_strPRTTP,12));
					dosREPORT.writeBytes(padSTRING('R',strPRTNM+" ("+P_strPRTCD+" )",50));
					//dosREPORT.writeBytes(padSTRING('R',P_strPRTCD,12));
			...
			if(!L_flgBRK)
			{
			if(L_dblPRTBL!=Double.parseDouble(L_strCURBL))
			{
				dosREPORT.writeBytes(padSTRING('R',P_strPRTTP,12));
				dosREPORT.writeBytes(padSTRING('R',strPRTNM+" ("+P_strPRTCD+" )",50));
				//dosREPORT.writeBytes(padSTRING('R',P_strPRTCD,12));
	...
	*/
	void prnHEADER()
	{
		try
		{
			...
			dosREPORT.writeBytes(padSTRING('R',"Party Ledger",strDOTLN.length()-22));
			...
			dosREPORT.writeBytes("Party Type  Party Name & Code  ");
			dosREPORT.writeBytes("\n"+ strDOTLN +"\n");			
