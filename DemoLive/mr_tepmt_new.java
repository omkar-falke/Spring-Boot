...
public class mr_tepmt extends cl_pbase
{
	...
	public mr_tepmt()
	{
		...
		try
		{
			...
			+ " AND CMT_CGSTP = 'MRXXPTT' AND CMT_CODCD like '"+strYRDGT+"1%' AND ifnull(CMT_STSFL,'') <> 'X'";

			//	+ " AND CMT_CGSTP = 'MRXXPTT' AND CMT_CODCD like '"+cl_dat.M_strFNNYR1_pbst.substring(3)+"1%' AND ifnull(CMT_STSFL,'') <> 'X'";
			M_rstRSSET = cl_dat.exeSQLQRY(M_strSQLQRY);
			if(M_rstRSSET!=null)
			{
				while(M_rstRSSET.next())
			...
			add(new JLabel("Consignee"),7,2,1,1,pnlINVWS,'L');
			add(txtCNSCD = new TxtLimit(5),7,3,1,1,pnlINVWS,'L');
			
			add(txtCNSNM = new TxtLimit(40),7,4,1,4,pnlINVWS,'L');
			...
			txtDOCDT1.setText(cl_dat.M_strLOGDT_pbst);
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		if(L_KE.getKeyCode() == L_KE.VK_F1)
		{
			...
			if(M_objSOURC==txtPRTCD)
			{
				...
				{
					M_strSQLQRY="Select distinct PT_PRTCD,PT_PRTNM,PT_YTDVL,PT_YTDFL from CO_PTMST,MR_PLTRN  where  PT_PRTTP='C'  AND PL_PRTTP=PT_PRTTP AND PL_PRTCD=PT_PRTCD AND (ifnull(PL_DOCVL,0)-ifnull(PL_ADJVL,0))>0 ";
			...
			if(M_objSOURC==txtINVNO)
			{
				...
				if(rdbINVIC.isSelected())
				{
					M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(ifnull(PL_DOCVL,0) -ifnull(PL_ADJVL,0)) PT_AVLVL,IVT_CNSCD,IVT_DSRCD,IVT_BYRCD ,ifnull(PL_ADJVL,0) PL_ADJVL,PL_DOCTP,PL_CNSCD ";
					M_strSQLQRY +=" FROM MR_PLTRN,MR_IVTRN WHERE PL_DOCNO = IVT_INVNO AND PL_CMPCD='"+ strCMPCD +"' ";
					M_strSQLQRY += " AND PL_MKTTP='"+L_strMKTTP+"' AND (ifnull(PL_DOCVL,0)-ifnull(PL_ADJVL,0))>0 ";
				...
				if(rdbDEBIT.isSelected())
				{
					M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(ifnull(PL_DOCVL,0) -ifnull(PL_ADJVL,0)) PT_AVLVL,ifnull(PL_ADJVL,0),PL_DOCTP  ";
			...
			if(M_objSOURC==txtINVNO1)
			{
				...
				String L_strMKTTP=cmbMKTTP.getSelectedItem().toString();
				L_strMKTTP=L_strMKTTP.substring(0,2);
				M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(ifnull(PL_DOCVL,0) -ifnull(PL_ADJVL,0)) PT_AVLVL,PL_CNSCD,IVT_DSRCD,IVT_BYRCD ,ifnull(PL_ADJVL,0),PL_DOCTP ";
				M_strSQLQRY +=" FROM MR_PLTRN,MR_IVTRN WHERE PL_DOCNO = IVT_INVNO AND PL_DOCTP like '"+strDOCTP+"%'  AND (ifnull(PL_DOCVL,0) -ifnull(PL_ADJVL,0))>0";
				M_strSQLQRY +="	AND PL_CMPCD='"+ strCMPCD +"'";
	...
	void exeHLPOK()
	{
		...
		try
		{
			...
			if(M_strHLPFLD == "txtINVNO1")
			{
				...
				txtDSRNM.setText((String)hstPARTY.get("D"+txtDSRCD.getText().trim()));
			...
			else if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPADD_pbst))
			{
				...
				if(L_strDOCNO !=null)
				{
					...
					if(tbpMAIN.getSelectedIndex()==1)
					{
						...
						M_strSQLQRY="Insert into MR_PRTRN(PR_CMPCD,PR_PRTTP,PR_MKTTP,PR_PRTCD,PR_GRPCD,PR_DOCTP,PR_DOCNO,PR_DOCDT,PR_BNKCD,PR_CHQNO,PR_CHQDT,PR_MOPCD,PR_RCTVL,PR_SBSCD,PR_CNSCD,PR_TRNFL,PR_STSFL,PR_LUSBY,PR_LUPDT ) values ( ";
				...
				if(cl_dat.M_flgLCUPD_pbst)
				{
					...
					if(cl_dat.exeDBCMT("exeSAVE"))
					{
						...
						cstPLTRN_PMR.setString(1,strCMPCD);
	...
	private void insRCTDATA(int P_intROWNO)
	{
		try
		{
			...
			if(chkREVFL.isSelected() && cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPDEL_pbst) &&   txtCHQNO_REV.getText().length() > 2)
			{
				...
				M_strSQLQRY += " AND ifnull(PR_REFNO,'')  = '' ";
				cl_dat.exeSQLUPD(M_strSQLQRY,"setLCLUPD");
	...
	private void delPAYMT_NEW()
	{
	   try
	   {
		...
		if(!flgPAYADJ)// payment is not adjusted
		{
			...
			{
				M_strSQLQRY = "SELECT PL_SEQNO FROM MR_PLTRN WHERE pl_prtcd ='"+txtPRTCD.getText().trim() +"' AND PL_DOCNO ='"+txtDOCNO.getText().trim() +"'";
				M_rstRSSET = cl_dat.exeSQLQRY(M_strSQLQRY);
				if(M_rstRSSET !=null)
				{
					if(M_rstRSSET.next())
		...
		public boolean verify (JComponent input)
		{
			try
			{
				...
				if(input==txtINVNO1)
				{
					if(txtINVNO1.getText().trim().length()<8)
						return true;
					M_strSQLQRY ="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(ifnull(PL_DOCVL,0) -ifnull(PL_ADJVL,0)) PT_AVLVL,PL_CNSCD,IVT_DSRCD,IVT_BYRCD ,ifnull(PL_ADJVL,0),PL_DOCTP FROM MR_PLTRN,MR_IVTRN";
					M_strSQLQRY +=" WHERE PL_DOCNO = IVT_INVNO  AND PL_DOCTP like '"+strDOCTP+"%' AND  PL_CMPCD='"+ strCMPCD +"' AND (ifnull(PL_DOCVL,0)-ifnull(PL_ADJVL,0))>0 ";
					...
					if(M_rstRSSET!=null &&M_rstRSSET.next())
					{
						...
						if(hstPARTY.containsKey("D"+txtDSRCD.getText().trim()))
						{   
							txtDSRNM.setText((String)hstPARTY.get("D"+txtDSRCD.getText().trim()));
						}
			...
			try
			{
				...
				if(getSource()==tblRECAD)
				{
					if(P_intCOLID==TB2_DOCNO)
					{
						...
						if(rdbINVIC.isSelected())
						{
							M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(ifnull(PL_DOCVL,0)-ifnull(PL_ADJVL,0)) PT_AVLVL,IVT_CNSCD,IVT_DSRCD,IVT_BYRCD ,ifnull(PL_ADJVL,0) PL_ADJVL,PL_DOCTP FROM MR_PLTRN,MR_IVTRN WHERE PL_DOCNO = IVT_INVNO  AND PL_CMPCD='"+ strCMPCD +"' ";
						...
						{
							M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(ifnull(PL_DOCVL,0)-ifnull(PL_ADJVL,0)) PT_AVLVL,PL_ADJVL,PL_DOCTP FROM MR_PLTRN WHERE PL_CMPCD='"+ strCMPCD +"' ";
