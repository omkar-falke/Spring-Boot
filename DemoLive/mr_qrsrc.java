...
class mr_qrsrc extends cl_rbase implements MouseListener 
{
	private JTabbedPane tbpMAIN;
	private JPanel pnlDTLIND,pnlDTLINV;
	...
	private String strRESSTR = cl_dat.M_strREPSTR_pbst+"mr_qrsrc.doc"; 
	private JTextField txtSPCCT;

	private cl_JTable tblDTLIND,tblDTLINV;
	...
	private Vector vtrIVTRN_D1;			// Distributor list for Toppers
	private Hashtable hstIVTRN_D1;			// Distributor list with Tot.Qty for Toppers
	private Hashtable hstIVTRN_D2;			// Distributorwise details for toppers

	private Vector vtrIVTRN_G1;			// Grade list for Toppers
	private Hashtable hstIVTRN_G1;			// Grade list with Tot.Qty for Toppers
	private Hashtable hstIVTRN_G2;			// Grade details for toppers
	...
	mr_qrsrc()
	{
		...
		try
		{
			...
			pnlDTLIND = new JPanel(null);
			pnlDTLINV = new JPanel(null);
			txtSPCCT = new JTextField();
			
			hstCDTRN = new Hashtable();
			hstIVTRN_D1 = new Hashtable();
			hstIVTRN_D2 = new Hashtable();
			hstIVTRN_G1 = new Hashtable();
			hstIVTRN_G2 = new Hashtable();
			...
			vtrIVTRN_D1 = new Vector();
			vtrIVTRN_G1 = new Vector();
			vtrRPTWS = new Vector();
			vtrMKTTP = new Vector();
			...
			tblDTLIND = crtTBLPNL1(pnlDTLIND,arrDTTHDR,500,2,1,9.1,7.9,arrDTTHDR_WD,new int[]{0});  //,"Amount"
			tblDTLINV = crtTBLPNL1(pnlDTLIND,arrGTTHDR,500,2,1,9.1,7.9,arrGTTHDR_WD,new int[]{0});  //,"Amount"
			
			tbpMAIN.addTab("Toppers (Distr)",pnlDTLIND);
			tbpMAIN.addTab("Toppers (Grade)",pnlDTLINV);
			
			add(tbpMAIN,7,1,13,8,this,'L');
			tblDTLIND.addMouseListener(this);
			tblDTLINV.addMouseListener(this);
...
private void exePRINT1()
{
		...
		if(L_KE.getKeyCode() == L_KE.VK_F1)
			{
				...
				if(cmbRPTWS.getSelectedItem().toString().equals(strGRDWS))
					cl_hlp("Select PR_PRDCD, PR_PRDDS from CO_PRMST where substr(pr_prdcd,1,2) in ('51','52','53') and pr_prdcd in (select distinct ivt_prdcd from mr_ivtrn where "+setWHRSTR(M_txtFMDAT.getText(),M_txtTODAT.getText())+") order by PR_PRDCD" ,2,1,new String[] {"Code","Description"},2,"CT");
				...
				if(cmbRPTWS.getSelectedItem().toString().equals(strPRDWS))
					cl_hlp("Select substr(CMT_CODCD,1,4) CMT_CODCD, CMT_CODDS from CO_CDTRN where cmt_cgmtp='MST' and cmt_cgstp='COXXPGR' and substr(cmt_codcd,1,4) in (select distinct substr(ivt_prdcd,1,4) ivt_prdcd from mr_ivtrn where "+setWHRSTR(M_txtFMDAT.getText(),M_txtTODAT.getText())+")  order by CMT_CODCD" ,2,1,new String[] {"Code","Description"},2,"CT");
				if(cmbRPTWS.getSelectedItem().toString().equals(strLOTWS))
					{cl_hlp("Select distinct IST_LOTNO,IST_ISSDT from FG_ISTRN where IST_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND IST_ISSTP='10' and ist_issno in (select ivt_ladno from mr_ivtrn where "+setWHRSTR(M_txtFMDAT.getText(),M_txtTODAT.getText())+") order by ist_lotno",2,1,new String[] {"Lot No.","Desp.Date"},2,"CT");}
	...
	private void dspDTTDTL()
	{
		try
		{
			...
			M_strSQLQRY = "select ivt_dsrcd,ifnull(ivt_invqt,0) ivt_invqt,ifnull(ivt_invrt,0) ivt_invrt,ifnull(ivt_cclvl,0) ivt_cclvl,ifnull(ivt_comvl,0) ivt_comvl,ifnull(ivt_frtvl,0) ivt_frtvl,((ifnull(ivt_invrt,0)*ifnull(ivt_invqt,0))-ifnull(ivt_excvl,0)-ifnull(ivt_edcvl,0)-ifnull(ivt_ehcvl,0)-ifnull(ivt_cclvl,0)-ifnull(ivt_comvl,0)-ifnull(ivt_frtvl,0)) ivt_netvl,ivt_cptvl,ifnull(ivt_rsnvl,0) ivt_rsnvl from mr_ivtrn where "+strWHRSTR+" order by ivt_dsrcd";
			//System.out.println(M_strSQLQRY);
			rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			if(!rstRSSET.next() || rstRSSET==null)
				return;
			...
			for(i=0;i<vtrIVTRN_D1.size();i++)
			{
			    ...
			    tblDTLIND.setValueAt(getPTMST("D",L_strDSRCD,"PT_PRTNM"),i,intTB1_DSRNM);
				tblDTLIND.setValueAt(getIVTRN_D2(L_strDSRCD,"IVT_INVQT"),i,intTB1_INVQT);
				tblDTLIND.setValueAt(getIVTRN_D2(L_strDSRCD,"IVT_INVRT"),i,intTB1_INVRT);
				tblDTLIND.setValueAt(getIVTRN_D2(L_strDSRCD,"IVT_CCLVL"),i,intTB1_CCLVL);
				tblDTLIND.setValueAt(getIVTRN_D2(L_strDSRCD,"IVT_COMVL"),i,intTB1_COMVL);
				tblDTLIND.setValueAt(getIVTRN_D2(L_strDSRCD,"IVT_FRTVL"),i,intTB1_FRTVL);
				tblDTLIND.setValueAt(getIVTRN_D2(L_strDSRCD,"IVT_NETVL"),i,intTB1_NETVL);
				tblDTLIND.setValueAt(getIVTRN_D2(L_strDSRCD,"IVT_CPTVL"),i,intTB1_CPTVL);
			}
		}
	...
	private void dspGTTDTL()
	{
		try
		{
			...
			M_strSQLQRY = "select ivt_prdcd,ifnull(ivt_invqt,0) ivt_invqt,ifnull(ivt_invrt,0) ivt_invrt,ifnull(ivt_cclvl,0) ivt_cclvl,ifnull(ivt_comvl,0) ivt_comvl,ifnull(ivt_frtvl,0) ivt_frtvl,((ifnull(ivt_invrt,0)*ifnull(ivt_invqt,0))-ifnull(ivt_excvl,0)-ifnull(ivt_edcvl,0)-ifnull(ivt_ehcvl,0)-ifnull(ivt_cclvl,0)-ifnull(ivt_comvl,0)-ifnull(ivt_frtvl,0)) ivt_netvl,ivt_cptvl,ifnull(ivt_rsnvl,0) ivt_rsnvl from mr_ivtrn where "+strWHRSTR+" order by ivt_prdcd";
			//System.out.println(M_strSQLQRY);
			rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			if(!rstRSSET.next() || rstRSSET==null)
				return;
			...
			for(i=0;i<vtrIVTRN_G1.size();i++)
			{
			    ...
			    tblDTLINV.setValueAt(getPRMST(L_strPRDCD,"PR_PRDDS"),i,intTB2_PRDDS);
				tblDTLINV.setValueAt(getIVTRN_G2(L_strPRDCD,"IVT_INVQT"),i,intTB2_INVQT);
				tblDTLINV.setValueAt(getIVTRN_G2(L_strPRDCD,"IVT_INVRT"),i,intTB2_INVRT);
				tblDTLINV.setValueAt(getIVTRN_G2(L_strPRDCD,"IVT_CCLVL"),i,intTB2_CCLVL);
				tblDTLINV.setValueAt(getIVTRN_G2(L_strPRDCD,"IVT_COMVL"),i,intTB2_COMVL);
				tblDTLINV.setValueAt(getIVTRN_G2(L_strPRDCD,"IVT_FRTVL"),i,intTB2_FRTVL);
				tblDTLINV.setValueAt(getIVTRN_G2(L_strPRDCD,"IVT_NETVL"),i,intTB2_NETVL);
				tblDTLINV.setValueAt(getIVTRN_G2(L_strPRDCD,"IVT_CPTVL"),i,intTB2_CPTVL);
			}
		}
		...
		try
		{
			...
			if (cmbRPTWS.getSelectedIndex()>0)
			{
				if(cmbRPTWS.getSelectedItem().toString().equals(strRGNWS))
					...
					{strWHRSTR += " and substr(ivt_prdcd,1,4) = '"+txtSPCCT.getText()+"'";strSHDRDS = getCDTRN("SYSMR00PGR"+txtSPCCT.getText(),"CMT_CODDS",hstCDTRN); strSHDRNM = "Prod Category:";}
 ...
 */	
	private void crtGTTHDR()
	{
		try
		{
			...
			intLINECT = 0;
			O_DOUT.writeBytes(padSTRING('R',"Supreme Petrochem Ltd",25)+padSTRING('C',"Toppers (Grade) "+strSHDRNM+" "+strSHDRDS+" from "+M_txtFMDAT.getText()+ " to "+ M_txtTODAT.getText(),intGTTWD-50)+padSTRING('L',"Page "+intPAGENO,25)+"\n\n"); intLINECT +=2;
			intRUNCL = 0;
	...
	private void crtDTLIND()
	{
		try
		{
			...
			int i=0, j = tblDTLIND.getColumnCount(), k=0, intRUNCL = 0;

			crtDTTHDR();
			for(i=0;i<tblDTLIND.getRowCount();i++)
			{
				if(tblDTLIND.getValueAt(i,intTB1_INVQT).toString().equals(""))
					break;
				else if(Double.parseDouble(tblDTLIND.getValueAt(i,intTB1_INVQT).toString())==0)
					break;
				intRUNCL = 0;
				for(k=1;k<j;k++)
				{
					O_DOUT.writeBytes(padSTRING(arrDTTHDR_PAD[k],tblDTLIND.getValueAt(i,k).toString().trim(),arrDTTHDR_WD[k]/8)+" "); intRUNCL += 1;
					if(intRUNCL>intGTTCL)
					{
						O_DOUT.writeBytes("\n"+padSTRING('L'," ",arrDTTHDR_WD[1]/8)+" "); intRUNCL=0; intLINECT +=1;
	...
	private void crtDTLINV()
	{
		try
		{
			...
			int i=0, j = tblDTLINV.getColumnCount(), k=0, intRUNCL = 0;

			crtGTTHDR();
			for(i=0;i<tblDTLINV.getRowCount();i++)
			{
				if(tblDTLINV.getValueAt(i,intTB1_INVQT).toString().equals(""))
					break;
				else if(Double.parseDouble(tblDTLINV.getValueAt(i,intTB1_INVQT).toString())==0)
					break;
				intRUNCL = 0;
				for(k=1;k<j;k++)
				{
					O_DOUT.writeBytes(padSTRING(arrGTTHDR_PAD[k],tblDTLINV.getValueAt(i,k).toString().trim(),arrGTTHDR_WD[k]/8)+" "); intRUNCL += 1;
					if(intRUNCL>intGTTCL)
					{
						O_DOUT.writeBytes("\n"+padSTRING('L'," ",arrGTTHDR_WD[1]/8)+" "); intRUNCL=0; intLINECT +=1;
	...
	private boolean exeTBLREFSH()
	{
		try
		{
			tbpMAIN.remove(pnlDTLIND);
			tbpMAIN.remove(pnlDTLINV);
			pnlDTLIND.removeAll();
			pnlDTLINV.removeAll();
			...
			tblDTLIND = crtTBLPNL1(pnlDTLIND,arrDTTHDR,L_intRECCT,2,1,9.1,7.9,arrDTTHDR_WD,new int[]{0});    //,"Amount"
			tblDTLINV = crtTBLPNL1(pnlDTLINV,arrGTTHDR,L_intRECCT,2,1,9.1,7.9,arrGTTHDR_WD,new int[]{0});    //,"Amount"
			tbpMAIN.addTab("Toppers (Distr)",pnlDTLIND);
			tbpMAIN.addTab("Toppers (Grade)",pnlDTLINV);
	...
	private String setWHRSTR(String LP_STRDT, String LP_ENDDT)
	{
		...
		try
		{
			L_strRETSTR = " IVT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND date(ivt_invdt) between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(LP_STRDT))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(LP_ENDDT))+"' and ivt_stsfl<>'X' and ifnull(ivt_invqt,0)>0  and ivt_mkttp='"+cmbMKTTP.getSelectedItem().toString().substring(0,2)+"'"
