...
class mr_rpbrn extends cl_rbase
{
...
/** To wirte report to a file */
	...
	private cl_JTable tblREPORT;
	private JRadioButton rdbPSALL;
	private JRadioButton rdbPSPRM;
	private JRadioButton rdbPSNPR;
	private JRadioButton rdbSPSAL;
	private JRadioButton rdbSALTP[];
	private JRadioButton rdbSTALL;
	 ...
	 */
	@SuppressWarnings("unchecked") mr_rpbrn()
	{
		super(2);
		try
		{
			...
			if(M_rstRSSET!=null)
			{
				...
				while(M_rstRSSET.next())
				{
					if(M_rstRSSET.getString("CMT_CGSTP").equals("FGXXPKG"))
						...
						hstRGNDT.put(M_rstRSSET.getString("CMT_CODCD"),new String[]{ M_rstRSSET.getString("CMT_CODDS"),M_rstRSSET.getString("CMT_SHRDS")});
					else if(M_rstRSSET.getString("CMT_CGSTP").equals("MR00ZON"))
						hstZONDT.put(M_rstRSSET.getString("CMT_CODCD"),new  String[]{ M_rstRSSET.getString("CMT_CHP02"),M_rstRSSET.getString("CMT_CODDS")} );
					else if(M_rstRSSET.getString("CMT_CGSTP").equals("MR00SAL"))
					{
						...
						hstSALDT.put(M_rstRSSET.getString("CMT_CODDS"),M_rstRSSET.getString("CMT_CODCD"));
					}
					else if(M_rstRSSET.getString("CMT_CGSTP").equals("MR01IND"))
						hstDDCVL.put(M_rstRSSET.getString("CMT_CODCD"),M_rstRSSET.getString("CMT_NMP01"));
				}
	...
	@SuppressWarnings("unchecked") void exePRINT()
	{
		...
		//FORMING SBSCD FILTER
			...
			String L_strSALTP="and IN_SALTP in (";
			if(rdbSTALL.isSelected())
			{
				Enumeration L_enmSTPSL=hstSALDT.elements();
				while(L_enmSTPSL.hasMoreElements())
					L_strSALTP+="'"+L_enmSTPSL.nextElement()+"',";
				L_strSALTP=L_strSALTP.substring(0,L_strSALTP.length()-1)+")";
			}
			else if(rdbDOMDE.isSelected())
				L_strSALTP+="'"+hstSALDT.get("DOMESTIC").toString()+"','"+hstSALDT.get("DEEMED EXPORT").toString()+"')";
			else
			{
				for(int i=0;i<rdbSALTP.length;i++)
					if(rdbSALTP[i].isSelected())
						L_strSALTP=" and IN_SALTP='"+hstSALDT.get( rdbSALTP[i].getText())+"'";
			}
			//M_strSQLQRY="Select IN_INDNO,IN_ZONCD,IN_SALTP,INT_PRDCD,INT_PRDDS, INT_PKGWT, INT_INDQT, INT_BASRT, INT_INDQT*INT_BASRT INT_BASVL,(isnull(INT_CDCVL,0)+isnull(INT_TDCVL,0)+isnull(INT_DDCVL,0))*INT_INDQT INT_TDCVL,isnull(IN_APTVL,0)*INT_INDQT*INT_BASRT INT_TPTVL from MR_INMST,MR_INTRN where IN_INDNO=INT_INDNO and IN_BKGDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"' and IN_SBSCD1 in ("+L_strSBSCD+") and IN_STSFL not in ('0','X') and INT_STSFL not in ('0','X')"+L_strSALTP;
			M_strSQLQRY="Select IN_INDNO,IN_ZONCD,IN_SALTP,INT_PRDCD,INT_PRDDS, INT_PKGWT, (isnull(int_indqt,0)-isnull(int_fcmqt,0)) INT_INDQT, INT_RSNVL, isnull(IN_APTVL,0)*INT_RSNVL INT_TPTVL from VW_INTRN where INT_CMPCD = '"+cl_dat.M_strCMPCD_pbst+"' and IN_BKGDT between '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtFMDAT.getText()))+"' and '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(M_txtTODAT.getText()))+"'  and INT_RSNVL>0 and IN_STSFL not in ('0','X') and INT_STSFL not in ('0','X')"+L_strSALTP+"  and INT_SBSCD1 in "+M_strSBSLS;
			...
			while(M_rstRSSET.next())
			{
				L_strRGNCD=((String[])hstZONDT.get(M_rstRSSET.getString("IN_ZONCD")))[0];
				...
				if(L_strRGNCD.equals("03"))
					flgEXP=true;
				if((rdbDOMDE.isSelected()&&M_rstRSSET.getString("IN_SALTP").equals(hstSALDT.get("DOMESTIC"))) || !rdbDOMDE.isSelected())
				{
					if(hstRPDAT.containsKey(M_rstRSSET.getString("INT_PRDDS")+"|"+M_rstRSSET.getString("INT_PKGWT")))
					{
						L_hstTEMP=(Hashtable)hstRPDAT.get(M_rstRSSET.getString("INT_PRDDS")+"|"+M_rstRSSET.getString("INT_PKGWT"));
					...
					L_flaTEMP[intINDQT]+=M_rstRSSET.getFloat("INT_INDQT");
					L_flaTEMP[intBASVL]+=M_rstRSSET.getFloat("INT_RSNVL");
					L_flaTEMP[2]+=M_rstRSSET.getFloat("INT_TPTVL");
					L_hstTEMP.put(L_strRGNCD,L_flaTEMP);
					hstRPDAT.put(M_rstRSSET.getString("INT_PRDDS")+"|"+M_rstRSSET.getString("INT_PKGWT"),L_hstTEMP);
				}
				if(rdbDOMDE.isSelected() && M_rstRSSET.getString("IN_SALTP").equals(hstSALDT.get("DEEMED EXPORT")))
				{
					if(hstDEDAT.containsKey(M_rstRSSET.getString("INT_PRDDS")+"|"+M_rstRSSET.getString("INT_PKGWT")))
					{
						L_hstTEMP=(Hashtable)hstDEDAT.get(M_rstRSSET.getString("INT_PRDDS")+"|"+M_rstRSSET.getString("INT_PKGWT"));
						if(L_hstTEMP.containsKey(L_strRGNCD))
							L_flaTEMP=(float[])L_hstTEMP.get(L_strRGNCD);
						else
							L_flaTEMP=new float[]{0.0f,0.0f,0.0f};
					...
					L_flaTEMP[2]+=M_rstRSSET.getFloat("INT_TPTVL");
					L_hstTEMP.put(L_strRGNCD,L_flaTEMP);
					hstDEDAT.put(M_rstRSSET.getString("INT_PRDDS")+"|"+M_rstRSSET.getString("INT_PKGWT"),L_hstTEMP);
				}
			}
			int L_intCOLCT=3;
			if(flgWC)
		...
		//WRITE TO FILE
			...
			{//DISPLAY HTML REPORT IN BROWSER
					...
					if(M_rdbTEXT.isSelected())
						p  = r.exec("c:\\windows\\wordpad.exe "+cl_dat.M_strREPSTR_pbst+"mr_rpbrn.doc"); 
					else
						p  = r.exec("c:\\program files\\internet explorer\\iexplore.exe "+cl_dat.M_strREPSTR_pbst+"mr_rpbrn.htm");
			}
		//PRINT TO PRINTER IN PRINT OPTION IN TEXT FORMAT
				...
				doPRINT(cl_dat.M_strREPSTR_pbst+"mr_rpbrn.doc");
			...
			//E_MAIL THE RESPECTIVE FILE TO RECEIPENTS, ONE BY ONE
				for(int i=0;i<M_cmbDESTN.getItemCount();i++)
					((cl_eml)Class.forName("cl_eml").newInstance()).sendfile(M_cmbDESTN.getItemAt(i).toString(),cl_dat.M_strREPSTR_pbst+"mr_rpbrn."+(M_rdbTEXT.isSelected() ? "doc" : "htm"),"Grade wise stock status on "+M_txtTODAT.getText(),"");
	...
	private void exePRNRPT()
	{
		try
		{
			...
			if(M_rdbHTML.isSelected())
				filREPORT=new File(cl_dat.M_strREPSTR_pbst+"mr_rpbrn.htm");
			else
				filREPORT=new File(cl_dat.M_strREPSTR_pbst+"mr_rpbrn.doc");
			FileOutputStream filOUT=new FileOutputStream(filREPORT);
			dosREPORT=new DataOutputStream(filOUT);

			String L_strREPNM="Grade wise Booking Realisation (Based On Order Booking)";//+(rdbDAILY.isSelected() ? "Day" : "Month");
			...
			if(M_rdbHTML.isSelected())
			{
				...
				dosREPORT.writeBytes("<BODY bgColor=ghostwhite><P><HR><TABLE border=0 cellPadding=0 cellSpacing=0  width=\"100%\"> <TR><TD><IMG src=\"file://f:\\exec\\splerp2\\spllogo_old.gif\" style=\"HEIGHT: 68px; LEFT: 0px; TOP: 0px; WIDTH: 68px\"></TD><TD><P align=left><STRONG><FONT face=Arial size=5>SUPREME PETROCHEM LIMITED</FONT></STRONG></P><STRONG><FONT face=Arial size=4><p align=left> "+L_strREPNM+"</font><STRONG><FONT face=Arial size=3><p align=left> "+L_strREPTP+"</font><STRONG><FONT face=Arial size=3><p align=left> "+L_strREPCT+"</font> </TD><TD><p><FONT face=Arial size=2>Date : "+cl_dat.M_txtCLKDT_pbst.getText()+"</p><p><FONT face=Arial size=2>Page No. : 1</P><TD> </TR></TABLE><HR><FONT face=\"Comic Sans MS\">");
			}
			else
			{
	...
	private void prnHEADR() throws Exception
	{
		...
		{
			if(M_rdbTEXT.isSelected())
				dosREPORT.writeBytes("    "+padSTRING('C',"----------- West & Central ------------",46));
			else
		...
		{
			if(M_rdbTEXT.isSelected())
				dosREPORT.writeBytes("    "+padSTRING('C',"------------ North, East & South ---------",46));
			else
		...
		{
			if(M_rdbTEXT.isSelected())
				dosREPORT.writeBytes("    "+padSTRING('C',"--------------- TOTAL ---------------",46));
			else
		...
		{
			if(M_rdbTEXT.isSelected())
				dosREPORT.writeBytes("    "+padSTRING('C',"--------------- Export -----------------",46));
			else
