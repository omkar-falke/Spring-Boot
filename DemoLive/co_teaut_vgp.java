...
class co_teaut extends cl_pbase implements ChangeListener
{
	...
	private JButton btnRPDC1,btnRPDC2,btnRPDC3,btnRPDC4;
	private JTabbedPane jtpTRNDT;
	private JPanel pnlGPAPR,pnlGPAUT,pnlINAUT,pnlCLSDT,pnlRCLDT,pnlPTFDT,pnlVSTGP;
	private cl_JTable tblGPAPR,tblGPAUT,tblINAUT,tblITDT1,tblITDT2,tblITDT3,tblVSTAUT;
	...
	private int TB4_STATS = 0;
	private int TB4_LOTNO = 1;
	private int TB4_RCLNO = 2;
	private int TB4_PKGTP = 3;
	private int TB4_TPRCD = 4;
	private int TB4_CPRCD = 5;
	private int TB4_UCLQT = 6;
	private int TB4_REMDS = 7;
	private int TB4_WRHTP = 8;
	private int TB4_PRDTP = 9;
	private int TB4_PKGCD = 10;
	private int TB4_PRDCD = 11;
	private int TB4_OPRCD = 12;
	...
	private final int TB5_REMDS =11;
	private final int TB5_VSCAT=12;
	private final int TB5_VSPNO =13;
	...
	co_teaut()
	{
	   ...
	   try
	   {
		...
		add(btnRPDC4=new JButton("Show Document"),12,7,1,1.5,pnlVSTGP,'L');
...
/// Visitor GP Approval
		String[] L_strCOLHD = {"Select","DSP","Guest Name","Orgainsation","No.of Guests","To Visit","Purpose","Area","Area Desc","From Date","To Date","Remarks","Guest Category","Guest Pass No"};
		int[] L_intCOLSZ4 = {20,20,100,80,40,70,100,40,60,90,90,100,40,60};	  
		     tblVSTAUT = crtTBLPNL1(pnlVSTGP,L_strCOLHD,200,2,1,9,11.6,L_intCOLSZ4,new int[]{0,1});
		...
		try
		{
			...
			if(M_objSOURC == btnRPDC4)
			{
				...
				for(int i=0;i<tblVSTAUT.getRowCount();i++)
				{
					...
					{
						L_strVSPNO = tblVSTAUT.getValueAt(i,TB5_VSPNO).toString();
						break;
					}
				}
				hr_rpvgp objVSTGP = new hr_rpvgp(M_strSBSCD);
				objVSTGP.getDATA(L_strVSPNO,L_strVSPNO);
				try
				{
					Runtime r = Runtime.getRuntime();
					Process p = null;
	...
	void exeSAVE()
	{
		try
		{
			...
			if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPAUT_pbst))
			{
				...
				else if((jtpTRNDT.getSelectedIndex() ==intFNLCL)||(jtpTRNDT.getSelectedIndex() ==intPRVCL))
				{
	            	if(exeCLSREC())
				    {
				        String L_strLOTNO =txtLOTNO.getText().trim();
			      	...
			      	for(int i=0;i<tblVSTAUT.getRowCount();i++)
	    				{
	    					...
	    					//	M_strSQLQRY += "VS_VSTNM = " + "'" + tblVSTAUT.getValueAt(i,TB5_VSTNM).toString().trim() +"',";
	    					//	M_strSQLQRY += "VS_VSORG = " + "'" + tblVSTAUT.getValueAt(i,TB5_VSORG).toString().trim() +"',";
	    					//	M_strSQLQRY += "VS_VSTCT =" + "'"+tblVSTAUT.getValueAt(i,TB5_VSTCT).toString().trim() +"'," ;
	    					//	M_strSQLQRY += "VS_PERVS = "+ "'"+tblVSTAUT.getValueAt(i,TB5_PERVS).toString().trim() +"',";
	    					//    M_strSQLQRY += "VS_PURPS ="+ "'"+tblVSTAUT.getValueAt(i,TB5_PURPS).toString().trim() +"',";
	    			 		//    M_strSQLQRY += "VS_ARADS ="+ "'"+tblVSTAUT.getValueAt(i,TB5_ARADS).toString().trim() +"',";
	    					//	M_strSQLQRY += "VS_VSITM ="+ "'"+M_fmtDBDTM.format(M_fmtLCDTM.parse(tblVSTAUT.getValueAt(i,TB5_VSTDT).toString().trim() ))+"',";
	    					//	M_strSQLQRY += "VS_VLDTM ="+ "'"+M_fmtDBDTM.format(M_fmtLCDTM.parse(tblVSTAUT.getValueAt(i,TB5_VLDTM).toString().trim() ))+"',";
	    					//	M_strSQLQRY += "VS_VSARA ="+ "'"+tblVSTAUT.getValueAt(i,TB5_VSARA).toString().trim() +"',";
	    					...
	    					//	M_strSQLQRY += "VS_VSTTP = '01'";
	    						M_strSQLQRY += " WHERE  ";
	    						M_strSQLQRY += "  VS_VSPNO = " + "'" + tblVSTAUT.getValueAt(i,TB5_VSPNO).toString().trim()+"'";
	    						System.out.println(M_strSQLQRY);
	    						cl_dat.exeSQLUPD(M_strSQLQRY,"setLCLUPD");
	    						if(tblVSTAUT.getValueAt(i,TB5_REMDS).toString().trim().length() >0)
	    							...
	    							M_strSQLQRY += " where RM_TRNTP ='HC' and RM_TRNCD ='01' and RM_DOCNO = '" + tblVSTAUT.getValueAt(i,TB5_VSPNO).toString().trim()+"'";
	    							cl_dat.exeSQLUPD(M_strSQLQRY,"setLCLUPD");
								System.out.println(M_strSQLQRY);
	    			                	}
	    					}
...
private void modLINCNT()
{
	...
	{        
		...
		{    
			...
			M_strSQLQRY += " where VS_STSFL = '1' and VS_VSOTM IS NULL and VS_VSTTP='01' order by VS_VSPNO";
			System.out.println(M_strSQLQRY);
			M_rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY);
			if(M_rstRSSET !=null)
			{
