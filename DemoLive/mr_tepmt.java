...
import java.util.StringTokenizer;import java.util.Hashtable;import java.util.Enumeration;
...
public class mr_tepmt extends cl_pbase
{
	...
	private JTextField txtMOPCD,txtCHQNO,txtCHQDT,txtBNKCD,txtBNKNM,txtCHQAM,txtDBKCD1,txtDPSNO1;
	...
	private JTextField txtDBKCD,txtDBKNM,txtDPSNO;
	...
	private final int TB1_DPSNO=7;
	private final int TB1_DBKCD=8;
	private final int TB1_DBKNM=9;
	...
	public mr_tepmt()
	{
		...
		try
		{
			...
			cstPLTRN_PMR =cl_dat.M_conSPDBA_pbst.prepareCall("{call updPLTRN_PMR(?,?,?)}");
			...
			add(new JLabel("Party "),2,5,1,1,this,'L');
			add(txtPRTCD = new TxtLimit(8),2,6,1,1,this,'L');
			add(txtPRTNM = new TxtLimit(30),2,7,1,3,this,'L');
			...
			+ " AND CMT_CGSTP = 'MRXXPTT' AND CMT_CODCD like '"+strYRDGT+"1%' AND isnull(CMT_STSFL,'') <> 'X'";

			//	+ " AND CMT_CGSTP = 'MRXXPTT' AND CMT_CODCD like '"+cl_dat.M_strFNNYR1_pbst.substring(3)+"1%' AND isnull(CMT_STSFL,'') <> 'X'";
			M_rstRSSET = cl_dat.exeSQLQRY(M_strSQLQRY);
			if(M_rstRSSET!=null)
			{
				while(M_rstRSSET.next())
			...
			add(txtCHQNO1 = new TxtLimit(10),6,3,1,1,pnlINVWS,'L');
			...
			add(new JLabel("Deposited In"),7,2,1,1,pnlINVWS,'L');
			add(txtDBKCD = new TxtLimit(5),7,3,1,1,pnlINVWS,'L');
			add(txtDBKNM = new TxtLimit(40),7,4,1,2,pnlINVWS,'L');
			add(new JLabel("Deposit Slip No."),7,6,1,1.5,pnlINVWS,'L');
			add(txtDPSNO = new TxtLimit(15),7,8,1,1,pnlINVWS,'L');
			
			add(new JLabel("Consignee"),8,2,1,1,pnlINVWS,'L');
			add(txtCNSCD = new TxtLimit(5),8,3,1,1,pnlINVWS,'L');
			
			add(txtCNSNM = new TxtLimit(40),8,4,1,4,pnlINVWS,'L');
			...
			String[] L_strTBLHD1 = {" ","Pmt. Mode","Cheque No","Cheque Date","Bank Code ","Bank Name","Amount","Dep.slip","Dep.In","D.B.Name"};
			int[] L_intCOLSZ1 = {15,40,70,70,90,200,100,60,50,100};
			...
			txtDBKCD1 = new TxtLimit(5);
			txtDPSNO1 = new TxtLimit(15);
			...
			txtDBKCD1.addFocusListener(this);
			txtDBKCD1.addKeyListener(this);
			
			txtDPSNO1.addFocusListener(this);
			txtDPSNO1.addKeyListener(this);
			...
			tblRECEN.setCellEditor(TB1_DBKCD,txtDBKCD1);
			tblRECEN.setCellEditor(TB1_DPSNO,txtDPSNO1);
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		if(L_KE.getKeyCode() == L_KE.VK_F1)
		{
			...
			if(M_objSOURC==txtDSRCD)
			{
				...
				//if(getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))
					M_strSQLQRY="Select distinct PT_PRTCD,PT_SHRNM,PT_PRTNM from CO_PTMST where  PT_PRTTP ='"+((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+"' ";
				//else
			...
			if(M_objSOURC==txtPRTCD)
			{
				...
				{
					M_strSQLQRY="Select distinct PT_PRTCD,PT_PRTNM,PT_YTDVL,PT_YTDFL from CO_PTMST,MR_PLTRN  where  PT_PRTTP='C'  AND PL_PRTTP=PT_PRTTP AND PL_PRTCD=PT_PRTCD AND PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND (isnull(PL_DOCVL,0)-isnull(PL_ADJVL,0))>0 ";
				...
				//if(!getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04"))
				if(txtDSRCD.getText().trim().length()==5)
				{
					if(!txtDSRCD.getText().trim().substring(1,5).equals("8888"))
					{
						M_strSQLQRY += " AND "+((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "PT_CNSRF" : "PT_DSRCD")+"='"+txtDSRCD.getText().trim()+"'";					
					}
				}
		...
		if(M_objSOURC==txtDBKCD)
			{
				M_strHLPFLD="txtDBKCD";
				M_strSQLQRY="Select PT_PRTCD,PT_PRTNM from CO_PTMST where  PT_PRTTP='B' AND isnull(PT_STSFL,'')<> 'X'";
				if(txtDBKCD.getText().trim().length()>0)
				{
					M_strSQLQRY += "AND PT_PRTCD LIKE '"+txtDBKCD.getText().trim().toUpperCase()+"%'"; 
				}
				cl_hlp(M_strSQLQRY,2,1,new String[]{"Party Code", "Party Name"},2,"CT");
			}
			if(M_objSOURC==txtDBKCD1)
			{
				M_strHLPFLD="txtDBKCD1";
				M_strSQLQRY="Select PT_PRTCD,PT_PRTNM from CO_PTMST where  PT_PRTTP='B' AND isnull(PT_STSFL,'')<> 'X'";
				if(txtBNKCD.getText().trim().length()>0)
				{
					M_strSQLQRY += "AND PT_PRTCD LIKE '"+txtDBKCD1.getText().trim().toUpperCase()+"%'"; 
				}
				cl_hlp(M_strSQLQRY,2,1,new String[]{"Party Code", "Party Name"},2,"CT");
			}
			if(M_objSOURC==txtMOPCD)
			...
			if(M_objSOURC==txtINVNO)
			{
				...
				if(rdbINVIC.isSelected())
				{
					M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(isnull(PL_DOCVL,0) -isnull(PL_ADJVL,0)) PT_AVLVL,IVT_CNSCD,IVT_DSRCD,IVT_BYRCD ,isnull(PL_ADJVL,0) PL_ADJVL,PL_DOCTP,PL_CNSCD ";
					M_strSQLQRY +=" FROM MR_PLTRN,MR_IVTRN WHERE PL_DOCNO = IVT_INVNO AND PL_CMPCD=IVT_CMPCD AND PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND IVT_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' ";
					M_strSQLQRY += " AND PL_MKTTP='"+L_strMKTTP+"' AND (isnull(PL_DOCVL,0)-isnull(PL_ADJVL,0))>0 ";
				...
				if(rdbDEBIT.isSelected())
				{
					M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(isnull(PL_DOCVL,0) -isnull(PL_ADJVL,0)) PT_AVLVL,isnull(PL_ADJVL,0),PL_DOCTP  ";
					M_strSQLQRY +=" FROM MR_PLTRN WHERE PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' ";
					M_strSQLQRY += " AND PL_MKTTP='"+L_strMKTTP+"' AND (isnull(PL_DOCVL,0) -isnull(PL_ADJVL,0))>0 ";
					M_strSQLQRY += " AND PL_PRTCD ='"+txtPRTCD.getText().trim() +"' ";
					M_strSQLQRY += " AND PL_DOCTP like '3%'";
					...
					//System.out.println(M_strSQLQRY);
					cl_hlp(M_strSQLQRY,1,1,new String[]{"Doc No","Doc Date","Doc. Value","Outstanding","Adj. Value","Doc Type"},6,"CT");
				}
			}
			...
			if(M_objSOURC==txtINVNO1)
			{
				...
				String L_strMKTTP=cmbMKTTP.getSelectedItem().toString();
				L_strMKTTP=L_strMKTTP.substring(0,2);
				M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(isnull(PL_DOCVL,0) -isnull(PL_ADJVL,0)) PT_AVLVL,PL_CNSCD,IVT_DSRCD,IVT_BYRCD ,isnull(PL_ADJVL,0),PL_DOCTP ";
				M_strSQLQRY +=" FROM MR_PLTRN,MR_IVTRN WHERE PL_DOCNO = IVT_INVNO AND PL_CMPCD=IVT_CMPCD AND PL_DOCTP like '"+strDOCTP+"%'  AND (isnull(PL_DOCVL,0) -isnull(PL_ADJVL,0))>0";
				M_strSQLQRY +="	AND PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"'";
		...
		if(L_KE.getKeyCode() == L_KE.VK_ENTER)
		{
			try
			{
				...
				if(M_objSOURC==txtDBKCD)
				{		
					txtDBKNM.setText((String)hstPARTY.get("C"+txtDBKCD.getText()));		
					txtDPSNO.requestFocus();
				}
				if(M_objSOURC==txtPRTCD)
				{				
				...
				{
					txtDBKCD.requestFocus();
				}
				if(M_objSOURC==txtDPSNO)
				{
	...
	*/
	void exeHLPOK()
	{
		...
		try
		{
			...
			if(M_strHLPFLD == "txtDBKCD1")
			{
				StringTokenizer L_STRTKN=new StringTokenizer(cl_dat.M_strHELP_pbst,"|");
				L_STRTKN.nextToken();
				txtDBKCD1.setText(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),0)).trim());	
				tblRECEN.setValueAt(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),1)).trim(),tblRECEN.getSelectedRow(),TB1_DBKNM);
			}
			if(M_strHLPFLD == "txtDBKCD")
			{
				StringTokenizer L_STRTKN=new StringTokenizer(cl_dat.M_strHELP_pbst,"|");
				L_STRTKN.nextToken();
				txtDBKCD.setText(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),0)).trim());
				txtDBKNM.setText(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),1)).trim());	
			}
			...
			if(M_strHLPFLD == "txtINVNO")
			{
				...
				if(rdbINVIC.isSelected())
				{
					...
					//tblRECAD.setValueAt((String)hstPARTY.get(((cmbSALTP.getSelectedItem().toString()).equals("04") || cmbSALTP.getSelectedItem().toString()).equals("14")) ? "G" : "D")+String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),5))),tblRECAD.getSelectedRow(),TB2_DSRNM);
					System.out.println(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),5)));
					tblRECAD.setValueAt((String)hstPARTY.get(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),5))),tblRECAD.getSelectedRow(),TB2_DSRNM);
				...
				else
				{
					...
					tblRECAD.setValueAt(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),4)).trim(),tblRECAD.getSelectedRow(),TB2_ADJSH);
					tblRECAD.setValueAt(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),5)).trim(),tblRECAD.getSelectedRow(),TB2_DOCTP);
					tblRECAD.setValueAt( new Boolean(true),tblRECAD.getSelectedRow(),TB2_CHKFL);
			...
			if(M_strHLPFLD == "txtINVNO1")
			{
				...
				txtDSRCD.setText(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),5)).trim());
				txtPRTCD.setText(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),6)).trim());
		
				txtDSRNM.setText((String)hstPARTY.get(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+txtDSRCD.getText().trim()));
				txtPRTNM.setText((String)hstPARTY.get("C"+txtPRTCD.getText().trim()));
				
				strCNSCD=(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),4)).trim());	
				if(strCNSCD.trim().length()==6)
				{
				//txtCNSCD.setText(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),4)).trim());
					txtCNSCD.setText(strCNSCD.substring(1,6));
					if(txtCNSCD.getText().trim().length()==5)
						txtCNSNM.setText((String)hstPARTY.get(strCNSCD));
	...
	*/
	void exeSAVE()
	{
		try
		{
			...
			if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPDEL_pbst))
			{
				// New method written only for cancellation on 18/03/07, as old method is not working
				///delPAYMT();
				if(flgPAYADJ) // payment is adjusted
				{
					delPAYMT();
					// 1.  Do cancellation without reversal entry (working)
					// 2. put a reversal entry
			...
			else if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPADD_pbst))
			{
				...
				if(L_strDOCNO !=null)
				{
					...
					if(tbpMAIN.getSelectedIndex()==1)
					{
						...
						double dblCHQAMT_NEW = Double.parseDouble(txtCHQAM1.getText());
						//if(chkREVFL.isSelected() && cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPDEL_pbst))
						//	dblCHQAMT_NEW = dblCHQAMT_NEW*(-1);
						M_strSQLQRY="Insert into MR_PRTRN(PR_CMPCD,PR_PRTTP,PR_MKTTP,PR_PRTCD,PR_GRPCD,PR_DOCTP,PR_DOCNO,PR_DOCDT,PR_BNKCD,PR_CHQNO,PR_CHQDT,PR_MOPCD,PR_RCTVL,PR_SBSCD,PR_SBSCD1,PR_CNSCD,PR_TRNFL,PR_STSFL,PR_LUSBY,PR_LUPDT,PR_DBKCD,PR_DPSNO ) values ( ";
						...
						M_strSQLQRY+=""+setNumberFormat(dblCHQAMT_NEW,2)+",";
						M_strSQLQRY+="'"+M_strSBSCD+"',";
						M_strSQLQRY+="'"+M_strSBSCD.substring(0,2)+"XX00"+"',";
						M_strSQLQRY+="'"+nvlSTRVL(strCNSCD,"")+"',";
							
					///	M_strSQLQRY += getUSGDTL("PR_",'I',strSTSFL)+")";
						
						M_strSQLQRY += getUSGDTL("PR_",'I',strSTSFL)+",";
						M_strSQLQRY+="'"+txtDBKCD.getText().trim()+"',";
						M_strSQLQRY+="'"+txtDPSNO.getText().trim()+"')";
				...
				if(cl_dat.M_flgLCUPD_pbst)
				{
					...
					if(cl_dat.exeDBCMT("exeSAVE"))
					{
						...
						cstPLTRN_PMR.setString(1,cl_dat.M_strCMPCD_pbst);
						cstPLTRN_PMR.setString(2,L_strMKTTP);
						cstPLTRN_PMR.setString(3,L_strDOCNO);
						//System.out.println(4);
						cstPLTRN_PMR.executeUpdate();
						//System.out.println(5);
						JOptionPane.showMessageDialog(this,"Please Note down Document No :  "+L_strDOCNO,"Message",JOptionPane.INFORMATION_MESSAGE);
	...
	private void insRCTDATA(int P_intROWNO)
	{
		try
		{
			...
			if(chkREVFL.isSelected() && cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPDEL_pbst) && txtCHQNO_REV.getText().length() > 2)
			{
				dblCHQAMT1 = dblCHQAMT1*(-1);
			///	System.out.println("Reversal :"+dblCHQAMT1);
			///	System.out.println("Ref Doc No. :"+strDOCNO_REF);
				L_strDOCNO = strDOCNO_REF;
				L_strREFNO = strDOCNO_ORG;
			///	System.out.println(L_strDOCNO +" : "+L_strREFNO);
				strSTSFL = "1";
				L_strDOCDT = M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_strLOGDT_pbst));
			}
			...
			if(chkREVFL.isSelected() && cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPDEL_pbst) &&   txtCHQNO_REV.getText().length() > 2)
			{
				String L_strMOPCD = nvlSTRVL(tblRECEN.getValueAt(P_intROWNO,TB1_MOPCD).toString(),"");
				String L_strCHQNO = nvlSTRVL(tblRECEN.getValueAt(P_intROWNO,TB1_CHQNO).toString(),"");
				String L_strCHQDT = nvlSTRVL(tblRECEN.getValueAt(P_intROWNO,TB1_CHQDT).toString(),"");
				String L_strBNKCD = nvlSTRVL(tblRECEN.getValueAt(P_intROWNO,TB1_BNKCD).toString(),"");
				
				M_strSQLQRY="UPDATE MR_PRTRN SET PR_REFNO = '"+strDOCNO_REF+"' , ";
				...
				M_strSQLQRY += " AND PR_PRTCD = '" +txtPRTCD.getText().trim() + "'";
				M_strSQLQRY += " AND PR_DOCNO = '" +strDOCNO_ORG + "'";
				...
				M_strSQLQRY += " AND isnull(PR_REFNO,'')  = '' ";
				System.out.println("MR_PATRN = "+M_strSQLQRY);
				cl_dat.exeSQLUPD(M_strSQLQRY,"setLCLUPD");

				cstPATRN_REV.setString(1,cl_dat.M_strCMPCD_pbst);
				cstPATRN_REV.setString(2,"C");
				cstPATRN_REV.setString(3,txtPRTCD.getText().trim());
	...
	private void shoDATA()
	{
		try
		{
			...
			//M_strSQLQRY="Select distinct PT_YTDVL,PT_YTDFL from CO_PTMST,MR_PLTRN  where  PT_PRTTP='C' And PT_PRTCD='"+ txtPRTCD.getText().trim()+"'";
             M_strSQLQRY="Select distinct PT_YTDVL,PT_YTDFL from CO_PTMST  where  PT_PRTTP='C' And PT_PRTCD='"+ txtPRTCD.getText().trim()+"'";
			ResultSet L_rstRSSET=cl_dat.exeSQLQRY(M_strSQLQRY);
	...
	*/
	private void dspDATA()
	{
		...
		try
		{
			...
			M_strSQLQRY +="PR_BNKCD,PR_CHQNO,PR_CHQDT,PR_MOPCD,PR_RCTVL,PR_ADJVL,"+((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "PT_CNSRF" : "PT_DSRCD")+" PT_DSRCD,PT_YTDVL,PT_YTDFL from MR_PRTRN,CO_PTMST ";
			M_strSQLQRY +="WHERE PR_CMPCD ='"+cl_dat.M_strCMPCD_pbst+"' and PR_PRTTP='C' and PR_DOCNO ='"+txtDOCNO.getText().trim()+ "' ";
			...
			if(M_rstRSSET !=null)
			{
				while(M_rstRSSET.next())
				{
					...
					txtDSRCD.setText(L_strDSRCD);
					txtDSRNM.setText(nvlSTRVL((hstPARTY.get(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+L_strDSRCD)).toString(),""));
		...
		*/
			...
			if(M_rstRSSET !=null)
			{
				while(M_rstRSSET.next())
				{
					...
					tblRECAD.setValueAt((String)hstPARTY.get(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+M_rstRSSET.getString("IVT_DSRCD")),i,TB2_DSRNM);
	...
	private void delPAYMT_NEW()
	{
	   try
	   {
		...
		if(!flgPAYADJ)// payment is not adjusted
		{
			...
			{
				M_strSQLQRY = "SELECT PL_SEQNO FROM MR_PLTRN WHERE PL_CMPCD='"+cl_dat.M_strCMPCD_pbst+"' AND pl_prtcd ='"+txtPRTCD.getText().trim() +"' AND PL_DOCNO ='"+txtDOCNO.getText().trim() +"'";
				M_rstRSSET = cl_dat.exeSQLQRY(M_strSQLQRY);
				if(M_rstRSSET !=null)
				{
					if(M_rstRSSET.next())
			...
			if(cl_dat.exeDBCMT("delPAYMT"))
			{
				...
				cstrwkPLTRN_TRN.setString(2,"C");
				cstrwkPLTRN_TRN.setString(3,txtPRTCD.getText().trim());
	...
	private void delPAYMT()
	{
		...
		try
		{
			...
			if(chkREVFL.isSelected() &&  txtCHQNO_REV.getText().length() > 2)
			{
				...
				////
				cstPLTRN_PMR.setString(1,cl_dat.M_strCMPCD_pbst);
				cstPLTRN_PMR.setString(2,cmbMKTTP.getSelectedItem().toString().substring(0,2));
				cstPLTRN_PMR.setString(3,strDOCNO_REF);
				cstPLTRN_PMR.executeUpdate();
			...
			if(!chkREVFL.isSelected())
			{
				...
				if(cl_dat.exeDBCMT("delPAYMT"))
				{
					...
					cstPLTRN_PMR.setString(1,cl_dat.M_strCMPCD_pbst);
					cstPLTRN_PMR.setString(2,cmbMKTTP.getSelectedItem().toString().substring(0,2));
					cstPLTRN_PMR.setString(3,strDOCNO_ORG);
					cstPLTRN_PMR.executeUpdate();
	...
	*/
	private boolean chkDOCNO()
	{
		try
		{
			M_strSQLQRY="Select PR_DOCNO from MR_PRTRN  WHERE PR_CMPCD = '" +cl_dat.M_strCMPCD_pbst+ "' and PR_PRTTP='C' and PR_DOCNO = '"+txtDOCNO.getText().trim()+ "'  and  isnull(PR_STSFL,'') <> 'X'";
			M_rstRSSET = cl_dat.exeSQLQRY1(M_strSQLQRY );
			if (M_rstRSSET != null && M_rstRSSET.next())
			{
				M_rstRSSET.close();
			...
			try
			{
				...
				if(input==txtDSRCD)
				{
					...
					else
					{
						...
						//M_strSQLQRY = "Select PT_PRTNM from CO_PTMST where PT_PRTTP IN('D','G') AND  PT_PRTCD = '"+txtDSRCD.getText().trim().toUpperCase()+"'  "  ;
						//if(getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))
							M_strSQLQRY = "Select PT_PRTNM from CO_PTMST where PT_PRTTP ='"+((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14")) ? "G" : "D")+"' AND  PT_PRTCD = '"+txtDSRCD.getText().trim().toUpperCase()+"'  "  ;
						//else
				...
				if(input==txtINVNO1)
				{
					if(txtINVNO1.getText().trim().length()<8)
						return true;
					M_strSQLQRY ="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(isnull(PL_DOCVL,0) -isnull(PL_ADJVL,0)) PT_AVLVL,PL_CNSCD,IVT_DSRCD,IVT_BYRCD ,isnull(PL_ADJVL,0),PL_DOCTP FROM MR_PLTRN,MR_IVTRN";
					M_strSQLQRY +=" WHERE PL_DOCNO = IVT_INVNO  AND PL_CMPCD = IVT_CMPCD AND PL_DOCTP like '"+strDOCTP+"%' AND  PL_CMPCD='"+ cl_dat.M_strCMPCD_pbst +"' AND (isnull(PL_DOCVL,0)-isnull(PL_ADJVL,0))>0 ";
					...
					if(M_rstRSSET!=null &&M_rstRSSET.next())
					{
						...
						txtPLDOC.setText(M_rstRSSET.getString("PL_DOCTP"));	
						strCNSCD=nvlSTRVL(M_rstRSSET.getString("PL_CNSCD"),"");		
						if(strCNSCD.trim().length()==6)
						{
							//txtCNSCD.setText(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),4)).trim());
							txtCNSCD.setText(strCNSCD.substring(1,6));
						...
						txtPRTCD.setText(M_rstRSSET.getString("IVT_BYRCD"));
						if(hstPARTY.containsKey(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+txtDSRCD.getText().trim()))
						{   
							txtDSRNM.setText((String)hstPARTY.get(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+txtDSRCD.getText().trim()));
						}
						if(hstPARTY.containsKey("C"+txtPRTCD.getText().trim()))
						{ 
							txtPRTNM.setText((String)hstPARTY.get("C"+txtPRTCD.getText().trim()));
						}
						M_rstRSSET.close();
				...
				if(input==txtMOPCD1)
				{
					if(txtMOPCD1.getText().trim().length()<2)
						return true;
					M_strSQLQRY="Select CMT_CODCD,CMT_CODDS from CO_CDTRN where CMT_CGMTP ='SYS'";
					M_strSQLQRY += " AND CMT_CGSTP = 'MRXXPMM'  AND CMT_CODCD='"+txtMOPCD1.getText().trim()+"' AND isnull(CMT_STSFL,'') <> 'X'";
					M_rstRSSET=cl_dat.exeSQLQRY1(M_strSQLQRY);					
					if(M_rstRSSET!=null &&M_rstRSSET.next())
					{
						return true;
				...
				if(input==txtBNKCD1)
				{
					if(txtBNKCD1.getText().trim().length()!=5)
						return true;
					M_strSQLQRY = "Select PT_PRTNM from CO_PTMST where PT_PRTTP='B' AND  PT_PRTCD = '"+txtBNKCD1.getText().trim() +"'  AND isnull(PT_STSFL,'')<> 'X' "  ;
					ResultSet L_rstRSSET=cl_dat.exeSQLQRY(M_strSQLQRY);
					if(L_rstRSSET!=null &&L_rstRSSET.next())
					{
			...
			try
			{
				...
				if(getSource()==tblRECAD)
				{
					if(P_intCOLID==TB2_DOCNO)
					{
						...
						if(rdbINVIC.isSelected())
						{
							M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(isnull(PL_DOCVL,0)-isnull(PL_ADJVL,0)) PT_AVLVL,IVT_CNSCD,IVT_DSRCD,IVT_BYRCD ,isnull(PL_ADJVL,0) PL_ADJVL,PL_DOCTP FROM MR_PLTRN,MR_IVTRN WHERE PL_DOCNO = IVT_INVNO  AND PL_CMPCD = IVT_CMPCD AND PL_CMPCD='"+ cl_dat.M_strCMPCD_pbst +"' ";
							M_strSQLQRY += " AND IVT_INVNO ='"+strTEMP+"' AND PL_PRTCD='"+txtPRTCD.getText().trim()+"' AND PL_PRTTP= 'C'";
							System.out.println(M_strSQLQRY);
							M_rstRSSET=cl_dat.exeSQLQRY1(M_strSQLQRY);
							if(M_rstRSSET!=null &&M_rstRSSET.next())
							{							
								...
								tblRECAD.setValueAt((String)hstPARTY.get(((getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("04") || getCODCD("SYSMR00SAL"+cmbSALTP.getSelectedItem().toString()).equals("14"))==true ? "G" : "D")+M_rstRSSET.getString("IVT_DSRCD")),tblRECAD.getSelectedRow(),TB2_DSRNM);
						...
						{
							M_strSQLQRY="SELECT DISTINCT PL_DOCNO,PL_DOCDT,PL_DOCVL,(isnull(PL_DOCVL,0)-isnull(PL_ADJVL,0)) PT_AVLVL,PL_ADJVL,PL_DOCTP FROM MR_PLTRN WHERE PL_CMPCD='"+ cl_dat.M_strCMPCD_pbst +"' ";
							M_strSQLQRY += " AND PL_DOCNO ='"+strTEMP+"' AND PL_PRTCD='"+txtPRTCD.getText()+"' AND PL_PRTTP='C'";
							
							M_rstRSSET=cl_dat.exeSQLQRY1(M_strSQLQRY);
							//System.out.println("IN PUT= "+M_strSQLQRY);
							if(M_rstRSSET!=null &&M_rstRSSET.next())
							{							
								L_datTMPDT = M_rstRSSET.getDate("PL_DOCDT");
								if(L_datTMPDT !=null)
