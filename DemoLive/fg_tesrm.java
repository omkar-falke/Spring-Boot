...
{									/**JTextField for ware house type	 */
	...
	private JTextField txtPRTDS;	/**JTextField for Party Code */
	private JTextField txtPRTCD;    /**JTextField for remark description */
	...
	public fg_tesrm()
	{
		...
		add(chkAUTLT = new JCheckBox("Authorized Lot"),3,1,1,2,this,'L');
		add(new JLabel("Party"),3,3,1,1,this,'L');
		
		add(txtPRTDS = new TxtLimit(10),3,4,1,1,this,'L');
		add(txtPRTCD = new TxtLimit(40),3,5,1,3,this,'L');
		...
		tblPRDCD = crtTBLPNL1(this,new String[]{"","Lot Number","Reclassification No.","Remark","Location","Stock","Package Type","Party Code"},250,8,1,9,8.1,new int[]{20,100,60,200,100,80,80,110},new int[]{0});
	...
	public void dspCHKDS()
	{
		try
		{
			...
			M_strSQLQRY = "select distinct st_remds,st_lotno,pr_prdds  from fg_stmst, co_prmst where st_prdcd=pr_prdcd and st_stkqt>0 and len(rtrim(ltrim(st_remds)))>0 order by pr_prdds,st_remds";
	...
	public void actionPerformed(ActionEvent L_AE)
	{
		...
		if(M_objSOURC == chkAUTLT)
		{
			if(chkAUTLT.isSelected()==true)
				txtPRTDS.setEnabled(true);
			else
				txtPRTDS.setEnabled(false);
	...
	public void keyPressed(KeyEvent L_KE)
	{
		...
		if(L_KE.getKeyCode() == L_KE.VK_F1)
		{
			...
			if(M_objSOURC==txtPRTDS)
			{
				M_strHLPFLD = "txtPRTDS";
				M_strSQLQRY = "SELECT CMT_CODCD,CMT_CODDS FROM CO_CDTRN "
					+"WHERE CMT_CGMTP = 'SYS' AND "
					+"CMT_CGSTP = 'MRXXLAU' ORDER BY CMT_CODCD";
	...
	public void exeHLPOK()
	{
		...
		if(M_strHLPFLD == "txtPRTDS")
		{
			if(strPRTCD.length()>0)
				strPRTCD=strPRTCD+"_";
			txtPRTDS.setText(cl_dat.M_strHLPSTR_pbst);
			strPRTCD=strPRTCD+cl_dat.M_strHLPSTR_pbst;
			if(txtREMDS.getText().trim().length()>0)
				strREMDS=strREMDS+" / ";
			strREMDS=strREMDS+(String.valueOf(cl_dat.M_tblHELP_pbst.getValueAt(cl_dat.M_tblHELP_pbst.getSelectedRow(),1)).trim());
			txtREMDS.setText(strREMDS.toString());
			txtPRTCD.setText(strPRTCD.toString());
			System.out.println(" strPRTCD = "+strPRTCD);
			//txtPRDCD.requestFocus();
		}
	}
	...
	public void getPRDDT()
	{
		try
		{
			...
			if(vldDATA())
			{
				M_strSQLQRY = "SELECT ST_LOTNO,ST_RCLNO,ST_MNLCD,ST_STKQT,ST_REMDS,ST_PKGTP,LT_AUTLS FROM FG_STMST,PR_LTMST WHERE  LT_PRDTP=ST_PRDTP  AND LT_LOTNO=ST_LOTNO AND LT_RCLNO=ST_RCLNO AND  "
				...
				if(M_rstRSSET != null)
				{
					...
					while(M_rstRSSET.next())
					{
						...
						tblPRDCD.setValueAt(nvlSTRVL(M_rstRSSET.getString("LT_AUTLS"),""),i,intPRTCD);
						i++;
					}
					intROWNO = i;
				}
		...
		if(M_objSOURC == tblPRDCD)
		{
			if(tblPRDCD.getSelectedColumn() == intCHKFL)
			{
				...
				if(rdbLOTNO.isSelected())
				{
					...
					for(int i = L_intSELRW ;i<tblPRDCD.getRowCount();i++)
					{
						if(strLOTNO.equals(tblPRDCD.getValueAt(i,intLOTNO)))
						{
							if(rdbADDRM.isSelected())
							{
								...
								if(String.valueOf(tblPRDCD.getValueAt(i,intPRTCD)).length()>0)
									tblPRDCD.setValueAt(tblPRDCD.getValueAt(i,intPRTCD)+"_"+txtPRTCD.getText().trim(),i,intPRTCD);
								else
									tblPRDCD.setValueAt(tblPRDCD.getValueAt(i,intPRTCD)+txtPRTCD.getText().trim(),i,intPRTCD);
							}
							else
							{
								tblPRDCD.setValueAt(txtREMDS.getText().trim(),i,intREMDS);								
								tblPRDCD.setValueAt(txtPRTCD.getText().trim(),i,intPRTCD);
							}
				...
				else
				{
					if(rdbADDRM.isSelected())
					{
						...
						if(String.valueOf(tblPRDCD.getValueAt(L_intSELRW,intPRTCD)).length()>0)
							tblPRDCD.setValueAt(tblPRDCD.getValueAt(L_intSELRW,intPRTCD)+"_"+txtPRTCD.getText().trim(),L_intSELRW,intPRTCD);
						else
							tblPRDCD.setValueAt(tblPRDCD.getValueAt(L_intSELRW,intPRTCD)+txtPRTCD.getText().trim(),L_intSELRW,intPRTCD);
					}
					else
					{
						tblPRDCD.setValueAt(txtREMDS.getText().trim(),L_intSELRW,intREMDS);
						tblPRDCD.setValueAt(txtPRTCD.getText().trim(),L_intSELRW,intPRTCD);
		...
		try
		{
			...
			if(chkAUTLT.isSelected()==true)
			{
				for(int i = 0; i < intROWNO;i++)
				{
					if(tblPRDCD.getValueAt(i,intCHKFL).toString().equals("true"))
					{
						strLOTNO1=tblPRDCD.getValueAt(i,intLOTNO).toString();
						if(!strLOTNO1.equals(strPLOTNO))
						{
							if(cl_dat.M_flgLCUPD_pbst)		//Update AUTLS  in PR_LTMST
							{
								M_strSQLQRY =  "UPDATE PR_LTMST SET LT_AUTLS = '"+tblPRDCD.getValueAt(i,intPRTCD)+"',";
								M_strSQLQRY += " LT_LUSBY = '"+cl_dat.M_strUSRCD_pbst+"',";
								M_strSQLQRY += "LT_LUPDT = '"+M_fmtDBDAT.format(M_fmtLCDAT.parse(cl_dat.M_strLOGDT_pbst.toString()))+"' ";
								M_strSQLQRY += " WHERE  LT_PRDTP = '"+txtPRDTP.getText().trim()+"' ";
								M_strSQLQRY += " AND LT_LOTNO = '"+tblPRDCD.getValueAt(i,intLOTNO)+"' ";
								M_strSQLQRY += " AND LT_RCLNO = '"+tblPRDCD.getValueAt(i,intRCLNO)+"' ";
								System.out.println(" chkAUTLT = "+ M_strSQLQRY);
								cl_dat.exeSQLUPD(M_strSQLQRY,"setLCLUPD");
								strPLOTNO=strLOTNO1;
							}
						}
