...
public class fr_log extends JFrame implements ActionListener,FocusListener,ItemListener,KeyListener,MouseListener,SwingConstants,Runnable,MenuListener
{
	...
	private Vector	vtrPRGCD,vtrSBSCD,vtrSHRDS,vtrCHP01,vtrCHP02,vtrADDFL,vtrMODFL,vtrDELFL,vtrENQFL,vtrAUTFL;/** Hashtable for form details	 */
	private Hashtable hstUSRRT;/** Result set object	 */
	private ResultSet rstRSSET;/** Standard application, menu bar*/
	private JMenuBar mbrMNUBR;/** Main menu	 */
	private JMenu mnuTMRSRC,mnuMASTR,mnuTRANS,mnuREPRT,mnuHSKPG,mnuOTHER,mnuEXIT;/**Menus to be used as separators in main menu	 */
	private JMenu mnuSEPTR1,mnuSEPTR2,mnuSEPTR3,mnuSEPTR4,mnuSEPTR5;/**Menuitem for scrolling effect	 */
	private JMenuItem mitTMRSRC,mitNWITM,mitMSTUP,mitMSTDN,mitTRNUP,mitTRNDN,mitRPTUP,mitRPTDN,mitOTHUP,mitOTHDN,mitHSKUP,mitHSKDN;/**Font of "login" Text in login screen	 */
...
//	private Thread thrTEMP;
	...
	fr_log()
	{
		...
		imgSPL = this.getToolkit().createImage("spllogo_old.gif");
...
//imgSPL1.flush();
		...
		this.show();this.setCursor(cl_dat.M_curWTSTS_pbst);
	...
	//MAKING LOGIN SCREEN ******************
		...
		JLabel lblLOGIN = crtLBL(cl_dat.M_pnlLOGIN_pbst,"LOGIN",387,100,100,40);
		lblLOGIN.setFont(fntLOGIN);
		lblLOGIN.setForeground(Color.blue);
		JLabel lblIMGIC = new JLabel(imiSPL);
		lblIMGIC.setBounds(new Double(370*cl_dat.M_dblWIDTH).intValue(),new Double(145*cl_dat.M_dblHIGHT).intValue(),new Double(100*cl_dat.M_dblWIDTH).intValue(),new Double(100*cl_dat.M_dblHIGHT).intValue());
		cl_dat.M_pnlLOGIN_pbst.add(lblIMGIC);
		JLabel lblUSRNM = crtLBL(cl_dat.M_pnlLOGIN_pbst,"User Name",325,260,75,20);
		cl_dat.M_txtUSRCD_pbst = crtTXT(cl_dat.M_pnlLOGIN_pbst,JTextField.LEFT,410,260,120,20);
		JLabel lblPASWD = crtLBL(cl_dat.M_pnlLOGIN_pbst,"Password",325,290,75,20);
		cl_dat.M_txtPASWD_pbst = new JPasswordField(10);
		cl_dat.M_txtPASWD_pbst.setBounds(new Double(410*cl_dat.M_dblWIDTH).intValue(),new Double(290*cl_dat.M_dblHIGHT).intValue(),new Double(120*cl_dat.M_dblWIDTH).intValue(),new Double(20*cl_dat.M_dblHIGHT).intValue());
   ...
   public void actionPerformed(ActionEvent L_AE)
   {
	   this.setCursor(cl_dat.M_curWTSTS_pbst);
		...
		//TO CHANGE LENGTH OF MENU, SET VALUE OF intMNULN_fn IN DECLARATION.
	        ...
	        else if(L_LEFT02.equals("FG") ||L_LEFT02.equals("QC")|| L_LEFT02.equals("CO") || L_LEFT02.equals("MS") || L_LEFT02.equals("PR") || L_LEFT02.equals("MM") || L_LEFT02.equals("MR") || L_LEFT02.equals("SA") || L_LEFT02.equals("QA")||L_LEFT02.equals("SA")||L_LEFT02.equals("HR")||L_LEFT02.equals("PR")||L_LEFT02.equals("MR"))
	        {
				...
				*/		
					...
					if(strACTCMD.substring(5).equals("EXT"))
	                {
						//strCURLC="";
	                    exeCLRMNU();
						tmrCLOCK.cancel();
						...
						cl_dat.M_txtCURDT_pbst.setEnabled(true);cl_dat.M_txtCURDT_pbst.setText("");
					}
					else
	                {
						...
						if (!exeSPTRN(strACTCMD))
					        return;
						strPRGCD=strACTCMD;
/*from pbase*/			lblMODUL.setText(L_STRTMP[intPRGDS_fn]+" " + L_STRTMP[intVERNO_fn]);
...
//TO CLOSE CONNECTION AND TEMINATE APPLICATION
	...
	private void crtMENU(){
		...
		for(int i=0;i<vtrPRGCD.size();i++)//ADDING MENU ITEMS
		{
			...
			{
				crtMNUITM(mnuTRANS,L_STRTMP[intSYSCD_fn]+" : "+L_STRTMP[intPRGDS_fn],key,L_STRTMP[intPRGDS_fn].charAt(0));
				//** ADDING "UP" OPTION IF No. OF MENUITEMS IS MORE
				if(mnuTRANS.getItemCount()>intMNULN_fn&&mitTRNUP==null)
				{
					mnuTRANS.insert(mitTRNUP=new JMenuItem("UP"),0);
			...
			{
				crtMNUITM(mnuREPRT,L_STRTMP[intSYSCD_fn]+" : "+L_STRTMP[intPRGDS_fn],key,L_STRTMP[intPRGDS_fn].charAt(0));
				//** ADDING "UP" OPTION IF No. OF MENUITEMS IS MORE
				if(mnuREPRT.getItemCount()>intMNULN_fn&&mitRPTUP==null)
				{
					mnuREPRT.insert(mitRPTUP=new JMenuItem("UP"),0);
			...
			{
				crtMNUITM(mnuOTHER,L_STRTMP[intSYSCD_fn]+" : "+L_STRTMP[intPRGDS_fn],key,L_STRTMP[intPRGDS_fn].charAt(0));
				//** ADDING "UP" OPTION IF No. OF MENUITEMS IS MORE
				if(mnuOTHER.getItemCount()>intMNULN_fn&&mitOTHUP==null)
				{
					mnuOTHER.insert(mitOTHUP=new JMenuItem("UP"),0);
		...
		{
			mnuTRANS.add(mitTRNDN=new JMenuItem("Down"));
			mitTRNDN.addMouseListener(this);
			mitTRNDN.setForeground(Color.blue );
			mitTRNDN.setBorder(BorderFactory.createBevelBorder(0));
			for(int i=1;i<mnuTRANS.getItemCount()-1;i++)
			{
				if(mnuTRANS.isMenuComponent(mnuTRANS.getItem(i))&&i>intMNULN_fn-2)
					mnuTRANS.getItem(i).setVisible(false);
			}
		}
		addSeparator(mnuTRANS);//ADD SYSTEM WISE SEPARATORS
		if(mitRPTUP!=null)//IF MENU COUNT > MENU LENGTH, ADD DOWN MENU AND HIDE ADDITIONAL MENU
		{
			mnuREPRT.add(mitRPTDN=new JMenuItem("Down"));
			mitRPTDN.addMouseListener(this);
			mitRPTDN.setForeground(Color.blue );
			mitRPTDN.setBorder(BorderFactory.createBevelBorder(0));
			for(int i=1;i<mnuREPRT.getItemCount()-1;i++)
			{
				if(mnuREPRT.isMenuComponent(mnuREPRT.getItem(i))&&i>intMNULN_fn-2)
					mnuREPRT.getItem(i).setVisible(false);
			}
		}
		addSeparator(mnuREPRT);//ADD SYSTEM WISE SEPARATORS
		if(mitOTHUP!=null)//IF MENU COUNT > MENU LENGTH, ADD DOWN MENU AND HIDE ADDITIONAL MENU
		{
			mnuOTHER.add(mitOTHDN=new JMenuItem("Down"));
			mitOTHDN.addMouseListener(this);
			mitOTHDN.setForeground(Color.blue );
			mitOTHDN.setBorder(BorderFactory.createBevelBorder(0));
			for(int i=1;i<mnuOTHER.getItemCount()-1;i++)
			{
				if(mnuOTHER.isMenuComponent(mnuOTHER.getItem(i))&&i>intMNULN_fn-2)
					mnuOTHER.getItem(i).setVisible(false);
			}
		}
		addSeparator(mnuOTHER);//ADD SYSTEM WISE SEPARATORS
	...
	private void exeCLRMNU(){
		...
		mnuOTHER.setVisible(false);
		mnuEXIT.setVisible(false);
		mitMSTDN=null;mitTRNDN=null;mitRPTDN=null;mitHSKDN=null;mitOTHDN=null;
		mitMSTUP=null;mitTRNUP=null;mitRPTUP=null;mitHSKUP=null;mitOTHUP=null;
	}
	...
	private void chkUSRVLD()
		...
		try
		{
			...
			strSQLQRY = "select US_USRPW,US_PWMFL,US_STSFL,UST_USRTP,US_EMLRF,current_timestamp curtm,current_date us_curdt,current_time us_curtm,PPR_SYSCD,PPR_SBSCD,PPR_PRGCD,SP_SYSNM,SP_SYSLC,SP_DTBLB,  SP_YSTDT, SP_YENDT,PP_PRGDS,PP_VERNO,PP_PRGLC,CMT_SHRDS,CMT_CHP01,CMT_CHP02,PPR_USRTP,PPR_ADDFL,PPR_MODFL,PPR_DELFL,PPR_ENQFL,PPR_AUTFL,PP_PRGTP,SP_SYSCD FROM SA_SPMST,SA_PPMST,SA_USMST,SA_USTRN,SA_PPRTR,CO_CDTRN "
			+" where PP_SYSCD=SP_SYSCD and PPR_PRGCD=PP_PRGCD and UST_USRCD=US_USRCD and PPR_USRTP=UST_USRTP and CMT_CODCD=PPR_SBSCD and CMT_CGSTP='COXXSBS' and US_USRCD='"+cl_dat.M_txtUSRCD_pbst.getText().toUpperCase() + "' order by SP_SYSCD,IFNULL(PP_ORDBY,PP_PRGCD) ";
			if(rstRSSET!=null)
				rstRSSET.close();
			...
			if(rstRSSET!=null)
			{
				hstUSRRT=new Hashtable(10,0.2f);
				vtrPRGCD=new Vector(10,2);
				vtrSBSCD=new Vector(10,2);
				vtrSHRDS=new Vector(10,2);
				vtrCHP01=new Vector(10,2);
				vtrCHP02=new Vector(10,2);
				vtrADDFL=new Vector(10,2);
				vtrMODFL=new Vector(10,2);
				vtrDELFL=new Vector(10,2);
				vtrENQFL=new Vector(10,2);
				vtrAUTFL=new Vector(10,2);//ADDED ON 02/04/2004 BY AAP FOR AUTHORISATION OPTION
						...
						if(!rstRSSET.getString("US_USRPW").trim().equalsIgnoreCase(cl_dat.M_strUSRPW_pbst.trim()))
						{
							...
							{//DISABLE USER AFTER THREE UNSUCCESSFUL ATTEMPTS
								...
								strSQLQRY="UPDATE SA_USMST SET US_STSFL='D', "
									...
									+ "' WHERE US_USRCD='"+cl_dat.M_txtUSRCD_pbst.getText().toUpperCase()+"'";
								cl_dat.exeSQLUPD(strSQLQRY ,"setLCLUPD");
								cl_dat.exeDBCMT("chkUSRVLD");
	...
	private void setHITCNT()
	{
		try
		{
			String L_STRSQL="UPDATE SA_PPMST SET "
				+"PP_HITCT=PP_HITCT+1,"
				+"PP_LHTBY='"+cl_dat.M_strUSRCD_pbst+"',"
				+"PP_LHTDT=CURRENT_DATE "
...
private void crtHSTMKT()
{
    try
    {
        ...
        int L_intCNT =0;
        cl_dat.M_hstMKTCD_pbst = new Hashtable();
		...
		"(SELECT B.CMT_CODCD FROM CO_CDTRN B WHERE B.CMT_CGMTP='CAT' AND B.CMT_CGSTP = 'COXXHST' AND IFNULL(CMT_STSFL,' ') <> 'X') order by cmt_cgmtp,cmt_cgstp,cmt_codcd";
...
current_date us_curdt,PPR_SYSCD,PPR_SBSCD,
