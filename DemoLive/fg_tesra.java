...
class fg_tesra extends cl_pbase implements ChangeListener//,PopupMenuListener
{
	...
	private Hashtable<String,String> hstCODDS;			// Code decription as key  & Code as a value 
	private Hashtable<String,String[]> hstPRMST;			// Details of product master
	private Hashtable hstPTMST;			// Details of product master
	private Hashtable<String,String[]> hstSTMST;			// Details of stock master related fields
	private Hashtable<String,String[]> hstRSTRN;
	private Hashtable<String,String[]> hstLTMST;

	
	private Vector<String> vtrPRTNM;			// Party Name Vector
	...
	private JCheckBox chkREVFL;
	private JCheckBox chkEXTFL;
	...
	private double dblRESQT_LOT = 0.000;
	private double dblRESQT1_LOT = 0.000;
	...
	private cl_JTable tblODRSV;			//atc
	...
	private int intTB0_NEWDT = 12;		//atc
	...
	fg_tesra()
	{
		...
		try
		{
			...
			hstSTMST = new Hashtable<String,String[]>();
			hstLTMST = new Hashtable<String,String[]>();
			hstRSTRN = new Hashtable<String,String[]>();

			
			vtrPRTNM = new Vector<String>();
			lstPRTNM = new JList();
			...
			tblODRSV.setCellEditor(intTB0_REVFL,chkREVFL = new JCheckBox());
			tblODRSV.setCellEditor(intTB0_EXTFL,chkEXTFL = new JCheckBox());
			...
			//Added by atc on 01/10/2004 for initialising mouse listener to table order reservation 
			/**intTB0_REVFL intTB0_EXTFL
			 */
			((JCheckBox) tblODRSV.cmpEDITR[intTB0_REVFL]).addFocusListener(this);
			((JCheckBox) tblODRSV.cmpEDITR[intTB0_REVFL]).addMouseListener(this);
			
			((JCheckBox) tblODRSV.cmpEDITR[intTB0_EXTFL]).addFocusListener(this);
			((JCheckBox) tblODRSV.cmpEDITR[intTB0_EXTFL]).addMouseListener(this);
			///
			
			
			tblSTMST.setInputVerifier(oTBLINPVF=new TBLINPVF());
			tblODRSV.setInputVerifier(oTBLINPVF=new TBLINPVF());
	...
	boolean vldDATA()
	{
		try
		{
			if(!flgEXPIRED_RES)
			{
				if(!chkEMPTY_TXT(txtGRPCD,"Enter Group Code ..."))    return false;
				if(!chkEMPTY_TXT(txtPRDCD,"Enter Product Code ..."))    return false;
				if(!chkEMPTY_TXT(txtPKGTP,"Enter Pkg.Type ..."))    return false;
			}
...
private boolean chkRESNO()
{
	...
	public void mousePressed(MouseEvent L_ME)
	{
		...
		try
		{
			...
			if (M_objSOURC==chkREVFL || M_objSOURC==chkEXTFL)
				oTBLINPVF.setSource(tblODRSV);
		}
	...
	public void actionPerformed(ActionEvent L_AE)
	{
		...
		try
		{
			if(M_objSOURC==cl_dat.M_cmbOPTN_pbst&&cl_dat.M_cmbOPTN_pbst.getSelectedIndex()!=0)
			{
				...
				if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPADD_pbst))
				{
					getODRSV();	//Enabled true by atc on 01102004
					btnPRINT.setEnabled(false);  
					chkRESNO();
				}
			}
	...
	void setENBL(boolean L_STAT)
	{
		try
		{
			...
			tblSTMST.setEnabled(false);
			tblRSTRN.setEnabled(false);
			tblSTMST.cmpEDITR[intTB1_CHKFL].setEnabled(true);
			tblSTMST.cmpEDITR[intTB1_ADDFL].setEnabled(true);
			tblSTMST.cmpEDITR[intTB1_ADJQT].setEnabled(true);
			tblSTMST.cmpEDITR[intTB1_DEDFL].setEnabled(true);
		}
	...
	void setDSBL()
	{
		...
		catch(Exception L_EX) {	setMSG(L_EX,"setDSBL");}
}
		...
		try
		{
			...
			if(L_intKEYCD==L_KE.VK_ENTER)
			{
				if(M_objSOURC==txtWRHTP)
					...
					{dspPRTNM(); txtRESDS.requestFocus();}isnull
			...
			if(L_intKEYCD==L_KE.VK_F1)
			{
				...
				{
					String strSQLQRY="select PR_PRDCD, PR_PRDDS from CO_PRMST where SUBSTRING(PR_PRDCD,1,2) in ('51','52','53') and PR_PRDCD in (select ST_PRDCD from fg_stmst where ifnull(st_stkqt,0)>0) ORDER BY PR_PRDCD";
					M_strHLPFLD = "txtPRDCD";
					cl_hlp(strSQLQRY,2,1,new String[] {"Code","Grade"},2 ,"CT");
				}
				else if(M_objSOURC==txtRESNO)
				{
					String strSQLQRY="select distinct RST_RESNO,RST_RESDS from MR_RSTRN where RST_STSFL<>'X' order by RST_RESNO";
					M_strHLPFLD = "txtRESNO";
					cl_hlp(strSQLQRY,2,1,new String[] {"Code","Grade"},2 ,"CT");
				}isnull
				else if(M_objSOURC==txtPKGTP)isnull
				{
				...
				{
					//String strSQLQRY="select PT_GRPCD, PT_PRTNM from CO_PTMST where PT_PRTTP='C' and length(ifnull(pt_grpcd,''))=5  and pt_grpcd in (select ivt_byrcd from mr_ivtrn where ivt_prdcd = '"+txtPRDCD.getText()+"') ORDER BY PT_GRPCD";
					String strSQLQRY="select distinct PT_GRPCD, PT_PRTNM from CO_PTMST where PT_PRTTP='C' and length(ifnull(pt_grpcd,''))=5  ORDER BY PT_GRPCD";
					M_strHLPFLD = "txtGRPCD";
					cl_hlp(strSQLQRY ,2,1,new String[] {"Code","Name"},2 ,"CT");
				}
			}
...
/** One time data capturing for Stock Master detailsisnullisnullisnull
*	into the Hash Table
*/
private boolean crtSTMST()
{
    ...
    try
    {
		...
		strWHRSTMST = "ifnull(ST_STKQT,0)>0 and ST_PRDCD='"+txtPRDCD.getText()+"' and ST_PKGTP = '"+txtPKGTP.getText()+"' and ST_PRDTP = '"+strPRDTP+"'";
		hstSTMST.clear();
		L_strSQLQRY = "select ST_PRDTP,ST_LOTNO,ST_RCLNO,ST_MNLCD,ifnull(ST_STKQT,0) ST_STKQT,ifnull(ST_RESQT,0) ST_RESQT,ST_REMDS from FG_STMST where "+strWHRSTMST+"  and ifnull(st_stkqt,0)>0.025 order by ST_PRDTP,ST_LOTNO,ST_RCLNO,ST_MNLCD";
		//System.out.println(L_strSQLQRY);
		ResultSet L_rstRSSET = cl_dat.exeSQLQRY1(L_strSQLQRY);
		 if(L_rstRSSET == null || !L_rstRSSET.next())
		 {
...
{isnullisnull
	...
	if(cl_dat.M_cmbOPTN_pbst.getSelectedItem().toString().equals(cl_dat.M_OPADD_pbst))
		return;
	cl_dat.exeSQLUPD("delete from mr_rstrn where ifnull(rst_resqt,0)=0 and ifnull(rst_issqt,0)=0 and rst_resno='"+txtRESNO.getText()+"'","");
	if(cl_dat.exeDBCMT("exeSAVE"))
		setMSG("",'N');
	String L_strSQLQRY = "";
    try
			...
			try
			{
				if(getSource()==tblSTMST)
				{
					if(P_intCOLID==intTB1_ADDFL || P_intCOLID==intTB1_DEDFL || P_intCOLID==intTB1_ADJQT)
					{
						...
						double L_dblRESQT_EFF = Double.parseDouble(getRSTRN(tblSTMST.getValueAt(P_intROWID,intTB1_LOTNO).toString()+tblSTMST.getValueAt(P_intROWID,intTB1_RCLNO).toString(),"RST_CURQT")); // effectiv qty for deduction operation
						L_dblRESQT_EFF = L_dblRESQT_EFF < dblRESQT ? L_dblRESQT_EFF : dblRESQT;
						System.out.println("L_dblRESQT_EFF : "+L_dblRESQT_EFF);
						...
						if(P_intCOLID==intTB1_ADDFL &&  chkADDFL.isSelected() && dblAVLQT>0)
								...
								setTB1_QTY(P_intROWID,L_dblRESQT_EFF*-1,"A",L_dblRESQT_EFF,"A",L_dblRESQT_EFF*-1,"A","F","T");
				...
				if(getSource() == tblODRSV)
				{
					...
					{
						System.out.println("Table Input Verifier");
						if(P_intCOLID==intTB0_EXTFL)
						{
							if(tblODRSV.getValueAt(P_intROWID,intTB0_EXTFL).toString().equals("true"))
							{
								...
								tblODRSV.setValueAt(setMONEND(cl_dat.M_txtCLKDT_pbst.getText().toString()),P_intROWID,intTB0_NEWDT);
							}
						}
						...
						tblODRSV.setValueAt(Boolean.TRUE,P_intROWID,intTB0_CHKFL);
					}
				}
				setMSG("",'N');
			}
...
/**
 */	
private void setRESQT_LOT(String LP_LOTNO, String LP_RCLNO)
{
	try
	{
		dblRESQT_LOT = 0.000;
		...
		double L_dblADJQT1_LOT = 0.000;
		double L_dblREFQT1_LOT = 0.000;
		for(int i=0;i<tblSTMST.getRowCount();i++)
		{
			//if(tblSTMST.getValueAt(i,intTB1_LOTNO).toString().length()==0)
			//	break;
			if(tblSTMST.getValueAt(i,intTB1_LOTNO).toString().equals(LP_LOTNO) && tblSTMST.getValueAt(i,intTB1_RCLNO).toString().equals(LP_RCLNO))
			{
				System.out.println(LP_LOTNO+"/"+LP_RCLNO);
				//String L_strSTMST_KEY = tblSTMST.getValueAt(i,intTB1_LOTNO).toString()+tblSTMST.getValueAt(i,intTB1_RCLNO).toString()+tblSTMST.getValueAt(i,intTB1_MNLCD).toString();
				dblRESQT_LOT +=	Double.parseDouble(nvlSTRVL(tblSTMST.getValueAt(i,intTB1_RESQT).toString(),"0.000"));
				L_dblADJQT1_LOT += Double.parseDouble(nvlSTRVL(tblSTMST.getValueAt(i,intTB1_ADJQT).toString(),"0.000"));
			}
		}
		L_dblREFQT1_LOT += (getRSTRN(LP_LOTNO+LP_RCLNO,"RST_REFQT").length()>0) ? Double.parseDouble(getRSTRN(LP_LOTNO+LP_RCLNO,"RST_REFQT")) : 0.000;
		dblRESQT1_LOT = L_dblADJQT1_LOT + L_dblREFQT1_LOT;
		dblRESQT1_LOT = Double.parseDouble(setNumberFormat(dblRESQT1_LOT,3));
		System.out.println("L_dblADJQT1_LOT : "+L_dblADJQT1_LOT);
		System.out.println("L_dblREFQT1_LOT : "+L_dblREFQT1_LOT+"\n");
		System.out.println("dblRESQT_LOT : "+dblRESQT_LOT);
		System.out.println("dblRESQT1_LOT : "+dblRESQT1_LOT);
	}
	catch(Exception L_EX){setMSG(L_EX,"setRESQT_LOT");}
}
...
private void setTB1_QTY(int LP_ROWID,double LP_RESQT,String LP_RESFL, double LP_AVLQT,String LP_AVLFL, double LP_ADJQT,String LP_ADJFL, String LP_ADDFL, String LP_DEDFL)
{
	try
	{
		System.out.println("setTB1_QTY : "+LP_RESQT+"/"+LP_AVLQT+"/"+LP_ADJQT+"/");
 ...
 * @param	LP_FLDNM	Field name to be inserted
 * @param	LP_FLDVL	isnullt / value of the field to be isnulled
 * @param	LP_FLDTP	Type of the field to be inserted
 */
private String setUPDSTR(String LP_FLDNM, String LP_FLDVL, String LP_FLDTP)isnullisnull
try {
	//System.out.println(LP_FLDNM+" : "+LP_FLDVL+" : "+LP_FLDTP);
	if (LP_FLDTP.equals("C"))
...
private boolean chkEXIST(String LP_TBLNM, String LP_WHRSTR)
{
	...
	/** Method to get order reservation
	 */
	private void getODRSV()
	{
		try
		{
			...
			M_strSQLQRY = "SELECT RST_RESNO,RST_RESDT,RST_PRDCD,RST_PKGTP,RST_GRPCD,RST_REXDT,"
				+"PR_PRDDS,sum(ifnull(RST_RESQT,0)) RST_RESQT,sum(ifnull(RST_ISSQT,0)) RST_ISSQT FROM MR_RSTRN,"
				...
				+"RST_GRPCD,RST_REXDT,PR_PRDDS ORDER BY RST_RESNO,RST_PRDCD having sum(ifnull(rst_resqt,0))>sum(ifnull(rst_issqt,0)) ";
